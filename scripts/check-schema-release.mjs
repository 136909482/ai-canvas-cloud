...
export function loadSchemaReleaseManifest() {
  if (!existsSync(manifestPath))
    throw new Error("Missing server/db/migrations/release-manifest.json");
  const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
  if (
    manifest.manifestVersion !== 1 ||
    JSON.stringify(manifest.releaseOrder) !==
      JSON.stringify(["expand", "migrate", "contract"])
  ) {
    throw new Error(
      "Schema release manifest version or releaseOrder is invalid",
    );
  }
  return manifest;
}

export function validateSchemaReleaseManifest(
  ...
  for (const [index, migration] of manifest.migrations.entries()) {
    ...
    if (
      ...
      phases.get(migration.phase) === undefined
    )
    ...
    if (releaseTrain !== previousTrain) {
      ...
      if (completedTrains.has(releaseTrain))
        throw new Error(
          `Schema release train reappeared at ${migration.version}`,
        );
    ...
    if (phase < previousPhase)
      throw new Error(
        `Schema release phase moved backwards at ${migration.version}`,
      );
    previousPhase = phase;
    if (
      ![true, false].includes(migration.oldAppReadable) ||
      ...
      !Number.isInteger(migration.statementTimeoutMs) ||
      migration.statementTimeoutMs < 1000 ||
      migration.statementTimeoutMs > 300000 ||
      typeof migration.rollback !== "string" ||
      migration.rollback.length < 10 ||
      typeof migration.forwardRepair !== "string" ||
      migration.forwardRepair.length < 10 ||
      typeof migration.backupRequired !== "boolean"
    ) {
      throw new Error(
        `Incomplete schema release metadata for ${migration.version}`,
      );
    }
    const sql = readFileSync(join(migrationsDirectory, fileName), "utf8");
    if (
      /\bDROP\s+(?:TABLE|COLUMN|DATABASE)\b/i.test(sql) &&
      migration.backupRequired !== true
    ) {
      throw new Error(
        `Destructive migration ${migration.version} must require a backup`,
      );
    }
  }
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  try {
    ...
    console.log(
      JSON.stringify({
        event: "schema_release_manifest_valid",
        migrations: result.files.length,
      }),
    );
  } catch (error) {
    console.error(
      JSON.stringify({
        event: "schema_release_manifest_invalid",
