...
export function parseEncryptionKey(value) {
  ...
  if (key.byteLength !== 32 || key.toString('base64') !== value) {
    throw new Error('BACKUP_ENCRYPTION_KEY must be a canonical base64 32-byte key')
  }
  return key
}
...
export function assertRestoreIsolation(env) {
  ...
  for (const [sourceKey, restoreKey] of pairs) {
    ...
    if (source === restore || !/restore/i.test(restore)) {
      throw new Error(`${restoreKey} must be a distinct restore-only identifier`)
    }
  }
  const target = new URL(requiredEnv(env, 'RESTORE_DATABASE_URL'))
  if (!['postgres:', 'postgresql:'].includes(target.protocol)) throw new Error('RESTORE_DATABASE_URL must use PostgreSQL')
  if (/^(localhost|127\.0\.0\.1|::1)$/i.test(target.hostname)) throw new Error('RESTORE_DATABASE_URL must not target localhost')
  if (!/restore/i.test(target.hostname)) throw new Error('RESTORE_DATABASE_URL host must be restore-only')
  if (env.RESTORE_RESET_CONFIRMED !== 'true') throw new Error('RESTORE_RESET_CONFIRMED must be true')
  return true
}
...
export async function runCommand(command, args, options = {}) {
  await new Promise((resolvePromise, reject) => {
    ...
    child.once('error', () => reject(new Error(`${basename(command)} could not start`)))
    child.once('exit', (code) => code === 0
      ? resolvePromise()
      : reject(new Error(`${basename(command)} failed with exit code ${code ?? 'unknown'}`)))
  })
}
...
export async function encryptBackup(sourcePath, targetPath, key) {
  ...
  async function* encrypted() {
    ...
    for await (const chunk of createReadStream(sourcePath).pipe(cipher)) yield chunk
    yield cipher.getAuthTag()
  }
  await pipeline(Readable.from(encrypted()), createWriteStream(targetPath, { mode: 0o600 }))
}

export async function decryptBackup(sourcePath, targetPath, key) {
  const stat = await fs.stat(sourcePath)
  if (stat.size <= BACKUP_MAGIC.length + IV_BYTES + TAG_BYTES) throw new Error('Encrypted backup is truncated')
  ...
  try {
    ...
    if (!header.subarray(0, BACKUP_MAGIC.length).equals(BACKUP_MAGIC)) throw new Error('Encrypted backup format is invalid')
    const decipher = createDecipheriv('aes-256-gcm', key, header.subarray(BACKUP_MAGIC.length))
    decipher.setAuthTag(tag)
    await pipeline(
      createReadStream(sourcePath, { start: header.length, end: stat.size - TAG_BYTES - 1 }),
      decipher,
...
export async function pruneBackupFiles(directory, retentionDays, now = Date.now()) {
  ...
  for (const entry of await fs.readdir(root, { withFileTypes: true })) {
    if (!entry.isFile() || !/^[0-9TZ-]+\.(?:dump\.enc|manifest\.json|source-guard\.json)$/.test(entry.name)) continue
    const path = resolve(join(root, entry.name))
    if (!path.startsWith(`${root}\\`) && !path.startsWith(`${root}/`)) throw new Error('Backup retention path escaped its root')
...
export async function createRecoveryFingerprint(client) {
  ...
  `)
  return { counts, digest: digestResult.rows[0]?.digest ?? createHash('sha256').update('').digest('hex') }
}
...
export async function pushBackupMetrics(url, input) {
  ...
  const response = await fetch(new URL(`/metrics/job/staging_backup/outcome/${input.success ? 'success' : 'failure'}`, url), {
    method: 'PUT',
