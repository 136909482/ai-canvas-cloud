...
import { scanWebBundle, validateCleanupAudit } from './staging-release-gate.mjs'
...
test('web bundle accepts production-safe static content', () => {
  assert.deepEqual(scanWebBundle(bundle('fetch("/api/health/live")')), { files: 2 })
})

test('cleanup audit requires zero unreconciled staging state', () => {
  assert.equal(validateCleanupAudit({ orphanFormalAssets: 0, permanentRunningTasks: 0, duplicateCharges: 0, unreclaimableStagingObjects: 0 }), true)
  assert.throws(() => validateCleanupAudit({ orphanFormalAssets: 1 }), /orphanFormalAssets/)
