...
export function parseAssetMaintenanceArgs(args) {
  ...
  if (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > ASSET_MAINTENANCE_MAX_BATCH_SIZE) {
    throw new Error(`--batch-size must be between 1 and ${ASSET_MAINTENANCE_MAX_BATCH_SIZE}`)
  }
  if (!Number.isInteger(graceHours) || graceHours < 1 || graceHours > ASSET_GC_MAX_GRACE_HOURS) {
    throw new Error(`--grace-hours must be between 1 and ${ASSET_GC_MAX_GRACE_HOURS}`)
  }
...
function requireEnvironment(name) {
  ...
  if (!value) {
    throw new Error(`Missing ${name}. Asset maintenance requires PostgreSQL and S3-compatible storage.`)
  }
  return value
}
...
async function main() {
  ...
  loadDotEnv()
  const pool = createPostgresPool({ connectionString: requireEnvironment('DATABASE_URL') })
  ...
  try {
    ...
    do {
      ...
      for (const item of batch.items) {
        ...
        console.log(JSON.stringify({ event: 'asset_maintenance_database_item', runId, mode, ...item }))
      }
    ...
    do {
      ...
      for (const item of page.items) {
        scannedObjects += 1
        counts[item.action] = (counts[item.action] ?? 0) + 1
        console.log(JSON.stringify({ event: 'asset_maintenance_object_item', runId, mode, ...item }))
      }
