...
export function scanWebBundle(directory) {
  if (!existsSync(directory))
    throw new Error(
      "Web production bundle is missing; run the production build first",
    );
  const files = [];
  const visit = (current) => {
  ...
  if (!files.some((file) => file.endsWith("index.html")))
    throw new Error("Web production bundle has no index.html");
  for (const file of files) {
    const content = readFileSync(file, "utf8");
    for (const pattern of WEB_FORBIDDEN) {
      if (pattern.test(content))
        throw new Error(
          `Web bundle contains a forbidden development or secret marker (${pattern.source})`,
        );
    }
  }
...
export function validateReleaseArtifacts(root = process.cwd()) {
  ...
  const compose = readFileSync(
    resolve(root, "infra/deploy/staging/docker-compose.yml"),
    "utf8",
  );
  const dockerfile = readFileSync(resolve(root, "Dockerfile"), "utf8");
  const nginx = readFileSync(
    resolve(root, "infra/deploy/staging/web.nginx.conf"),
    "utf8",
  );
  const alerts = readFileSync(
    resolve(root, "infra/deploy/staging/alerts.yml"),
    "utf8",
  );
  if (
    !/profiles:\s*\["release"\]/.test(compose) ||
    /apply-migrations\.mjs.*api/.test(compose)
  )
    throw new Error("Migrations must be an explicit release-only step");
  if (!/AS api[\s\S]*USER node/.test(dockerfile) || !/AS web/.test(dockerfile))
    throw new Error(
      "API and Web runtime artifacts must run as non-root images",
    );
  if (
    !/Content-Security-Policy/.test(nginx) ||
    !/frame-ancestors 'none'/.test(nginx) ||
    /unsafe-eval/.test(nginx)
  )
    throw new Error("Web security headers are incomplete");
  if (
    !/AiCanvasDependencyDown/.test(alerts) ||
    !/AiCanvasBackupMissing/.test(alerts)
  )
    throw new Error("Dependency and backup alerts are required");
  return { requiredArtifacts: required.length };
}

export function validateCleanupAudit(report) {
  if (!report || typeof report !== "object")
    throw new Error("Cleanup audit report must be an object");
  const fields = [
    "orphanFormalAssets",
    "permanentRunningTasks",
    "duplicateCharges",
    "unreclaimableStagingObjects",
  ];
  for (const field of fields) {
    if (!Object.prototype.hasOwnProperty.call(report, field))
      throw new Error(`Cleanup audit field ${field} is required`);
    const value = Number(report[field]);
    if (!Number.isInteger(value) || value < 0)
      throw new Error(
        `Cleanup audit field ${field} must be a non-negative integer`,
      );
...
async function probe(url, path, expectedStatus = 200) {
  ...
  if (response.status !== expectedStatus)
    throw new Error(
      `Endpoint probe failed (${path}, status ${response.status})`,
    );
...
export async function runGate({
  ...
  const env = readEnvFile(
    resolve(root, envFile ?? "infra/deploy/staging/staging.env"),
  );
  ...
  if (auditFile)
    validateCleanupAudit(
      JSON.parse(readFileSync(resolve(root, auditFile), "utf8")),
    );
  const probes = [];
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    args[key] = argv[index + 1]?.startsWith("--")
      ? true
      : (argv[index + 1] ?? true);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  ...
  runGate({
    ...
    .then((result) => {
      console.log(
        JSON.stringify({
          event: "staging_release_gate_passed",
          artifactCount: result.artifacts.requiredArtifacts,
          webFiles: result.web.files,
    ...
    .catch((error) => {
      console.error(
        JSON.stringify({
          event: "staging_release_gate_failed",
