...
try {
  ...
  await client.query(`REVOKE ALL ON public."user", public."session", public.workspaces, public.workspace_members, public.assets, public.migration_import_asset_uploads FROM ${admin}`)
  await client.query(`GRANT SELECT (id, user_no, name, email, email_verified, status, created_at, updated_at) ON public."user" TO ${admin}`)
  await client.query(`GRANT UPDATE (status, updated_at) ON public."user" TO ${admin}`)
  await client.query(`GRANT SELECT (id, user_id, expires_at, created_at, updated_at) ON public."session" TO ${admin}`)
  await client.query(`GRANT DELETE ON public."session" TO ${admin}`)
  await client.query(`GRANT SELECT (id, type, name, owner_user_id, status, plan_key, storage_quota_bytes, created_at, updated_at) ON public.workspaces TO ${admin}`)
  await client.query(`GRANT SELECT (workspace_id, user_id, role, joined_at) ON public.workspace_members TO ${admin}`)
  await client.query(`GRANT SELECT (id, workspace_id, byte_size, status, deleted_at) ON public.assets TO ${admin}`)
  await client.query(`GRANT SELECT (workspace_id, expected_byte_size, status, committed_asset_id) ON public.migration_import_asset_uploads TO ${admin}`)
