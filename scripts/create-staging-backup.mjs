...
export async function createStagingBackup(env = process.env) {
  ...
  try {
    ...
    await runCommand('pg_dump', [
      ...
      '--snapshot', snapshot.rows[0].snapshot, '--file', temporaryPath,
  ...
  try {
    ...
    await runCommand('mc', ['mirror', '--overwrite', '--preserve', `primary/${sourceBucket}`, `backup/${backupBucket}/${objectPrefix}`], {
      env: {
        ...env,
        MC_HOST_primary: mcHost(requiredEnv(env, 'S3_ENDPOINT'), requiredEnv(env, 'S3_ACCESS_KEY_ID'), requiredEnv(env, 'S3_SECRET_ACCESS_KEY')),
    ...
    await runCommand('mc', ['cp', manifestPath, `backup/${backupBucket}/manifests/${backupId}.manifest.json`], {
      env: {
        ...env,
        MC_HOST_backup: mcHost(requiredEnv(env, 'BACKUP_S3_ENDPOINT'), requiredEnv(env, 'BACKUP_S3_ACCESS_KEY_ID'), requiredEnv(env, 'BACKUP_S3_SECRET_ACCESS_KEY')),
      },
    ...
    console.log(JSON.stringify({ event: 'staging_backup_completed', backupId, encryptedBytes: stat.size }))
    return manifest
  } catch (error) {
    return fail(error)
  }
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  createStagingBackup().catch((error) => {
    console.error(JSON.stringify({ event: 'staging_backup_failed', error: error instanceof Error ? error.name : 'UnknownError' }))
    process.exitCode = 1
  })
}
