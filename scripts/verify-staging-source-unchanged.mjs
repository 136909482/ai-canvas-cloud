...
export async function verifyStagingSourceUnchanged(env = process.env) {
  ...
  if (!/^[0-9TZ-]+$/.test(backupId))
    throw new Error("RESTORE_BACKUP_ID is invalid");
  const mode = requiredEnv(env, "SOURCE_GUARD_MODE");
  if (!["record", "verify"].includes(mode))
    throw new Error("SOURCE_GUARD_MODE must be record or verify");
  ...
  try {
    ...
    if (mode === "record") {
      ...
      (await fs.readFile(guardPath, "utf8").then((value) => value.trim()))
    ) {
    ...
    console.log(
      JSON.stringify({
        event:
          mode === "record"
            ? "staging_source_guard_recorded"
            : "staging_source_unchanged",
        backupId,
      }),
    );
  } finally {
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  verifyStagingSourceUnchanged().catch((error) => {
    console.error(
      JSON.stringify({
        event: "staging_source_verification_failed",
