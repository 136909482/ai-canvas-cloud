...
function loadMigrations() {
  ...
  return files.map((fileName, index) => {
    ...
    if (Number(version) !== index + 1) {
      throw new Error(
        `Migration versions must be contiguous from 0001: ${fileName}`,
      );
    }
...
async function assertAssetGovernanceSchema(client, schemaName) {
  ...
  if (tableResult.rowCount !== tables.length) {
    throw new Error(
      "Asset governance migration did not create all required tables",
    );
  }
  ...
  if (constraintResult.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Asset governance migration is missing required tenant constraints",
    );
  }
...
async function assertGenerationTaskLegacyReferenceGuard(client, migrationSql) {
  ...
  try {
    ...
    if (
      ...
      error.message.includes("legacy task asset references exist")
    ) {
      return;
    }
    throw error;
  ...
  await client.query("RELEASE SAVEPOINT generation_task_legacy_guard");
  throw new Error(
    "Generation task migration accepted an unowned legacy task asset reference",
  );
}
...
async function assertGenerationTaskSchema(client, schemaName) {
  ...
  if (tableResult.rowCount !== tables.length) {
    throw new Error(
      "Generation task migration did not create all required tables",
    );
  }
  ...
  if (constraintResult.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Generation task migration is missing required tenant constraints",
    );
  }
  ...
  if (indexResult.rowCount !== requiredIndexes.length) {
    throw new Error(
      "Generation task migration is missing queue or history indexes",
    );
  }
  ...
  if (taskIdType.rows[0]?.udt_name !== "uuid") {
    throw new Error(
      "Generation task migration did not normalize asset task references to UUID",
    );
  }
...
async function assertProviderCredentialSchema(client, schemaName) {
  ...
  if (constraintResult.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Provider credential migration is missing required constraints",
    );
  }
  ...
  if (indexResult.rowCount !== 1) {
    throw new Error(
      "Provider credential migration is missing its user status index",
    );
  }
  ...
  if (backfilledWebsite.rows[0]?.website_url !== "https://openai.com") {
    throw new Error(
      "Provider website URL migration did not backfill the known provider website",
    );
  }
...
async function assertTaskCommandSchema(client, schemaName) {
  ...
  if (constraintResult.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Task command migration is missing required tenant or idempotency constraints",
    );
  }
  const indexResult = await client.query(
...
async function assertProjectGraphSchema(client, schemaName) {
  ...
  if (tableResult.rowCount !== tables.length) {
    throw new Error(
      "Project graph migration did not create all required tables",
    );
  }
  ...
  if (constraintResult.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Project graph migration is missing required tenant or graph constraints",
    );
  }
  ...
  if (indexResult.rowCount !== requiredIndexes.length) {
    throw new Error(
      "Project graph migration is missing required list or history indexes",
    );
  }
...
async function assertAuthDeviceSchema(client, schemaName) {
  ...
  await client.query(
    `DELETE FROM "session" WHERE id = 'migration-device-session'`,
  );
  ...
  `);
  if (
    deduplicated.rowCount !== 1 ||
    deduplicated.rows[0]?.device_key !== "persistent-device-id"
  ) {
    throw new Error(
      "Auth device legacy dedup migration did not preserve only the persistent device",
    );
  }
}
...
async function assertTaskQueueOutboxSchema(client, schemaName) {
  ...
  if (constraints.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Task queue outbox migration is missing required constraints",
    );
  }
  ...
  `,
    [
      schemaName,
      ["task_queue_outbox_pending_idx", "task_queue_outbox_task_idx"],
    ],
  );
  ...
  `);
  if (
    backfilled.rows[0]?.dispatch_key !==
    "run:cccccccc-cccc-4ccc-8ccc-ccccccccccce:1"
  ) {
    throw new Error(
      "Task queue outbox migration did not backfill a queued task",
    );
  }
  await expectRejected(
    client,
...
async function assertProviderSubmissionSchema(client, schemaName) {
  ...
  if (columns.rowCount !== 3) {
    throw new Error(
      "Provider submission migration is missing attempt submission columns",
    );
  }
  const requiredConstraints = [
  ...
  if (constraints.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Provider submission migration is missing submission constraints",
    );
  }
  ...
  if (index.rowCount !== 1) {
    throw new Error(
      "Provider submission migration is missing its recovery index",
    );
  }
  const upgraded = await client.query(`
  ...
  `);
  if (
    upgraded.rows[0]?.submission_key !==
      "provider-submission:dddddddd-dddd-4ddd-8ddd-dddddddddddf" ||
    upgraded.rows[0]?.submission_stage !== "submitted" ||
    upgraded.rows[0]?.remote_task_id !== "legacy-remote-task"
  ) {
    throw new Error(
      "Provider submission migration did not preserve an in-flight remote task",
    );
  }
  await expectRejected(
    client,
...
async function assertGenerationTaskEventSchema(client, schemaName) {
  ...
  if (constraints.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Generation task event migration is missing tenant or event constraints",
    );
  }
  const indexes = await client.query(
    `
  ...
  if (indexes.rowCount !== 3) {
    throw new Error(
      "Generation task event migration is missing polling indexes",
    );
  }
  const backfilled = await client.query(`
  ...
  if (backfilled.rows[0]?.status !== "queued") {
    throw new Error(
      "Generation task event migration did not backfill an existing task",
    );
  }
  await client.query(`
    UPDATE generation_tasks
  ...
  if (
    ...
    throw new Error(
      "Generation task event trigger did not persist a sanitized projection",
    );
  }
  await expectRejected(
    client,
...
async function assertMigrationImportSchema(client, schemaName) {
  ...
  if (constraints.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Migration import migration is missing tenant, lifecycle, or payload constraints",
    );
  }
  const indexes = await client.query(
    `
  ...
  `,
    [
      schemaName,
      [
        "migration_imports_workspace_status_updated_idx",
        "migration_imports_expiry_idx",
      ],
    ],
  );
  if (indexes.rowCount !== 2) {
    throw new Error(
      "Migration import migration is missing status or expiry indexes",
    );
  }
  await client.query(`
    INSERT INTO "user" (id, name, username, display_username, email, email_verified)
...
async function assertMigrationAssetUploadSchema(client, schemaName) {
  ...
  if (table.rowCount !== 1) {
    throw new Error(
      "Migration asset upload migration did not create its table",
    );
  }
  const requiredConstraints = [
  ...
  if (constraints.rowCount !== requiredConstraints.length) {
    throw new Error(
      "Migration asset upload migration is missing lifecycle, tenant, or payload constraints",
    );
  }
  const indexes = await client.query(
    `
  ...
  `,
    [
      schemaName,
      [
        "migration_import_asset_uploads_import_status_idx",
        "migration_import_asset_uploads_expiry_idx",
      ],
    ],
  );
  if (indexes.rowCount !== 2) {
    throw new Error(
      "Migration asset upload migration is missing status or expiry indexes",
    );
  }
  await client.query(`
...
async function assertMigrationCommitSchema(client, schemaName) {
  ...
  if (columns.rowCount !== 5) {
    throw new Error(
      "Migration commit migration is missing commit idempotency columns",
    );
  }
  const constraints = await client.query(
    `
  ...
  if (constraints.rowCount !== 5) {
    throw new Error(
      "Migration commit migration is missing commit state constraints",
    );
  }
  await expectRejected(
    client,
...
async function assertMigrationExportSchema(client, schemaName) {
  ...
  if (table.rowCount !== 1) {
    throw new Error(
      "Migration export migration did not create migration_exports",
    );
  }
  const columns = await client.query(
    `
  ...
  if (columns.rowCount !== 1) {
    throw new Error(
      "Migration lifecycle retry migration is missing export retry_count",
    );
  }
  const constraints = await client.query(
    `
  ...
  if (constraints.rowCount !== 9) {
    throw new Error(
      "Migration export migration is missing lifecycle or tenant constraints",
    );
  }
  const indexes = await client.query(
    `
  ...
  `,
    [
      schemaName,
      [
        "migration_exports_workspace_status_updated_idx",
        "migration_exports_expiry_idx",
        "migration_exports_retryable_idx",
      ],
    ],
  );
  if (indexes.rowCount !== 3) {
...
async function assertUserNumberSchema(client, schemaName) {
  ...
  if (
    ...
    throw new Error(
      "User number migration did not backfill stable sequential numbers",
    );
  }

  const maximumBeforeInsert = await client.query(
    'SELECT max(user_no) AS maximum FROM "user"',
  );
  ...
  if (Number(inserted.rows[0]?.user_no) !== expectedNextNumber) {
    throw new Error(
      "User number sequence did not continue after the backfill maximum",
    );
  }
  ...
  if (constraints.rowCount !== 2) {
    throw new Error(
      "User number migration is missing uniqueness or range constraints",
    );
  }
...
async function assertAdminSecuritySchema(client) {
  ...
  if (tables.rowCount !== 8) {
    throw new Error(
      "Admin security migration did not create all isolated tables",
    );
  }
  const publicUsage = await client.query(
    `SELECT has_schema_privilege('public', 'admin', 'USAGE') AS allowed`,
  );
  ...
  `);
  if (
    captchaSetting.rowCount !== 1 ||
    captchaSetting.rows[0]?.captcha_enabled !== false
  ) {
  ...
  if (publicCaptchaRead.rows[0]?.allowed !== false) {
    throw new Error(
      "Admin CAPTCHA challenges are readable through PUBLIC privileges",
    );
  }
  await expectRejected(
    client,
...
async function assertSiteConfigurationSchema(client) {
  const adminTables = await client.query(
    ...
    WHERE table_schema = 'admin'
      AND table_name = ANY($1::text[])
  `,
    [["site_assets", "site_config_revisions", "site_config_current"]],
  );
  if (adminTables.rowCount !== 3) {
    throw new Error(
      "Site configuration migration did not create the Admin configuration tables",
    );
  }
  ...
  if (publication.rowCount !== 1) {
    throw new Error(
      "Site configuration migration did not create the public projection table",
    );
  }
  const publicRead = await client.query(
    `SELECT has_table_privilege('public', 'public.site_config_publications', 'SELECT') AS allowed`,
  );
  if (publicRead.rows[0]?.allowed !== false) {
    throw new Error(
      "Site configuration publication is readable through PUBLIC privileges",
    );
  }
  ...
  `,
    [
      administratorId,
      `${administratorId}@example.invalid`,
      `site_${randomUUID().replaceAll("-", "").slice(0, 20)}`,
    ],
  );
  await client.query(
    `
...
async function seedLegacyOfficialSiteConfigFixture(client) {
  ...
  const legacyConfig = {
    ...
    home: {
      ...
      description:
        "Verifies the P8-4 active site configuration forward repair.",
  ...
  `,
    [
      administratorId,
      `${administratorId}@example.invalid`,
      `legacy_site_${randomUUID().replaceAll("-", "").slice(0, 16)}`,
    ],
  );
  await client.query(
    `
...
async function assertServerGenerationRemoved(client, schemaName) {
  ...
  assert.equal(
    removedTables.rowCount,
    0,
    "server generation tables still exist after contract migration",
  );
  ...
  assert.equal(
    taskColumn.rowCount,
    0,
    "asset_references.task_id still exists after contract migration",
  );
  ...
  `);
  assert.equal(
    activeSiteConfig.rowCount,
    1,
    "active site configuration is missing after contract migration",
  );
  assert.notEqual(
    activeSiteConfig.rows[0]?.revision_id,
    legacyOfficialSiteConfigRevisionId,
  );
  ...
  assert.equal(
    legacyRevision.rows[0]?.has_official_mode,
    true,
    "immutable legacy site configuration revision was not preserved",
  );
}
...
if (!databaseUrl) {
  throw new Error(
    "Missing MIGRATION_DATABASE_URL or DATABASE_URL. Migration tests require a disposable PostgreSQL schema.",
  );
}
...
try {
  ...
  console.log(
    `Checked and upgraded ${migrations.length} migration file(s) in an isolated PostgreSQL schema.`,
  );
} finally {
