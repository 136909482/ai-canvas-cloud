...
function cwdProof(token, pid, cwd) {
  return createHash("sha256")
    .update(`${token}\0${pid}\0${normalizePath(cwd)}`)
    .digest("hex");
}
...
function inspectProcProcess(pid) {
  try {
    return {
      ...
      commandLine: readFileSync(`/proc/${pid}/cmdline`, "utf8").replaceAll(
        "\0",
        " ",
      ),
    };
...
export function inspectProcess(pid) {
  ...
  return process.platform === "win32"
    ...
    : inspectProcProcess(pid);
}

export function validateManagedProcess(
  manifest,
  processInfo,
  service,
  repoRoot = REPO_ROOT,
  ...
  if (
    ...
    basename(processInfo.executablePath).toLowerCase() !==
  ...
  if (service === "web" || service === "admin-web") {
    ...
    return {
      ...
      args: [
        ...
        "--host",
        (isAdmin ? process.env.ADMIN_WEB_HOST : process.env.WEB_HOST) ??
          "127.0.0.1",
        "--port",
        (isAdmin ? process.env.ADMIN_WEB_PORT : process.env.WEB_PORT) ??
          (isAdmin ? "5174" : "5173"),
        "--strictPort",
      ],
      ipc: false,
...
async function runService(service, token) {
  if (!SERVICE_NAMES.includes(service) || !/^[0-9a-f-]{36}$/i.test(token))
    process.exit(2);
...
async function getManagedState(service, inspector = inspectProcess) {
  ...
  if (!validation.ok)
    return { state: "unverified", manifest, reason: validation.reason };
...
async function startService(service) {
  ...
  if (current.state === "unverified") {
    throw new Error(
      `${service}: refusing to replace unverified PID record (${current.reason})`,
    );
  }
  ...
  throw new Error(
    `${service}: failed to start; inspect ${logPath(service, "stderr")}`,
  );
}
...
async function stopService(service) {
  ...
  if (current.state !== "running") {
    throw new Error(
      `${service}: refusing to stop unverified PID ${current.manifest?.pid ?? "unknown"} (${current.reason})`,
    );
  }
  ...
  const validation = validateManagedProcess(
    current.manifest,
    processInfo,
    service,
  );
  if (!validation.ok)
    throw new Error(
      `${service}: identity changed while stopping; refusing force stop`,
    );
...
async function startAll() {
  ...
  console.log(
    `web: http://${process.env.WEB_HOST ?? "127.0.0.1"}:${process.env.WEB_PORT ?? "5173"}`,
  );
  console.log(
    `api: http://${process.env.API_HOST ?? "127.0.0.1"}:${process.env.API_PORT ?? "8787"}`,
  );
  console.log(
    `admin web: http://${process.env.ADMIN_WEB_HOST ?? "127.0.0.1"}:${process.env.ADMIN_WEB_PORT ?? "5174"}`,
  );
  console.log(
    `admin api: http://${process.env.ADMIN_API_HOST ?? "127.0.0.1"}:${process.env.ADMIN_API_PORT ?? "8788"}`,
  );
}
...
async function statusAll() {
  ...
  for (const service of SERVICE_NAMES) {
    const state = await getManagedState(service);
    if (state.state === "running")
      console.log(`${service}: running (pid ${state.manifest.pid})`);
    else if (state.state === "stopped") console.log(`${service}: stopped`);
    else if (state.state === "stale")
      console.log(`${service}: stopped (stale record)`);
    else {
      unhealthy = true;
...
async function main() {
  ...
  throw new Error(
    "Usage: node scripts/dev-process.mjs <start|stop|restart|status>",
  );
}
  ...
  main().catch((error) => {
    console.error(
      error instanceof Error
        ? error.message
        : "Development process command failed",
    );
    process.exitCode = 1;
  });
}
