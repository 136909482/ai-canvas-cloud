...
test('migration interruption rollback and rerun leave a known schema version', {
  ...
  try {
    ...
    const newAppOnOldSchema = await client.query('SELECT id, status, completed_file_count FROM migration_exports')
    assert.deepEqual(newAppOnOldSchema.fields.map((field) => field.name), ['id', 'status', 'completed_file_count'])
    ...
    const adminUsernameColumn = await client.query(`SELECT 1 FROM information_schema.columns WHERE table_schema = 'admin' AND table_name = 'user' AND column_name = 'username'`)
    assert.equal(adminUsernameColumn.rowCount, 1)
    const adminCaptchaSettings = await client.query(`SELECT 1 FROM information_schema.tables WHERE table_schema = 'admin' AND table_name = 'login_security_settings'`)
    assert.equal(adminCaptchaSettings.rowCount, 1)
    const removedTables = await client.query(`
      SELECT table_name FROM information_schema.tables
      WHERE table_schema = $1 AND table_name = ANY($2::text[])
    `, [schema, ['generation_tasks', 'task_attempts', 'task_commands', 'task_queue_outbox', 'generation_task_events', 'usage_ledger', 'provider_credentials']])
    assert.equal(removedTables.rowCount, 0)
    const taskReferenceColumn = await client.query(`SELECT 1 FROM information_schema.columns WHERE table_schema = $1 AND table_name = 'asset_references' AND column_name = 'task_id'`, [schema])
