...
export function parseCheckpointAssetRepairArgs(args) {
  ...
  if (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > CHECKPOINT_ASSET_REPAIR_MAX_BATCH_SIZE) {
    throw new Error(`--batch-size must be between 1 and ${CHECKPOINT_ASSET_REPAIR_MAX_BATCH_SIZE}`)
  }
...
async function main() {
  ...
  if (!process.env.DATABASE_URL) {
    throw new Error('Missing DATABASE_URL. The checkpoint asset repair requires PostgreSQL.')
  }
  ...
  try {
    do {
      const batch = options.apply
        ...
        : await service.preflightBatch({ cursor, batchSize: options.batchSize })
