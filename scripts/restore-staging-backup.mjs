...
async function selectManifest(directory, requestedId) {
  const files = (await fs.readdir(directory)).filter((name) => /^[0-9TZ-]+\.manifest\.json$/.test(name)).sort()
  const fileName = requestedId ? `${requestedId}.manifest.json` : files.at(-1)
  if (!fileName || !files.includes(fileName)) throw new Error('Requested backup manifest was not found')
  const root = resolve(directory)
  const path = resolve(join(root, fileName))
  if (!path.startsWith(`${root}\\`) && !path.startsWith(`${root}/`)) throw new Error('Backup manifest escaped its root')
  const manifest = JSON.parse(await fs.readFile(path, 'utf8'))
  if (manifest.formatVersion !== 1 || manifest.backupId !== fileName.slice(0, -'.manifest.json'.length)) {
    throw new Error('Backup manifest format is invalid')
  }
  if (manifest.database?.encryptedFile !== `${manifest.backupId}.dump.enc`
...
function quoteIdentifier(value) {
  if (!/^[a-z][a-z0-9_]{2,62}$/.test(value)) throw new Error('Restore database identifier is invalid')
  return `"${value}"`
}
...
async function resetRestoreDatabase(databaseUrl) {
  ...
  try {
    ...
    await admin.query(`CREATE DATABASE ${quoteIdentifier(databaseName)} OWNER ${quoteIdentifier(owner)}`)
  } finally {
    await admin.end()
  }
}
...
export async function restoreStagingBackup(env = process.env) {
  ...
  const manifest = await selectManifest(directory, env.RESTORE_BACKUP_ID?.trim())
  if (manifest.objects.bucket !== requiredEnv(env, 'BACKUP_S3_BUCKET')) throw new Error('Backup manifest bucket does not match the isolated backup resource')
  const encryptedPath = join(directory, manifest.database.encryptedFile)
  if (await sha256File(encryptedPath) !== manifest.database.encryptedSha256) throw new Error('Encrypted backup checksum mismatch')
  ...
  try {
    await decryptBackup(encryptedPath, decryptedPath, parseEncryptionKey(requiredEnv(env, 'BACKUP_ENCRYPTION_KEY')))
    await runCommand('pg_restore', ['--exit-on-error', '--no-owner', '--no-acl', '--dbname', requiredEnv(env, 'RESTORE_POSTGRES_DB'), decryptedPath], {
    ...
    await runCommand('mc', [
      ...
      env: {
        ...env,
        MC_HOST_backup: mcHost(requiredEnv(env, 'BACKUP_S3_ENDPOINT'), requiredEnv(env, 'BACKUP_S3_ACCESS_KEY_ID'), requiredEnv(env, 'BACKUP_S3_SECRET_ACCESS_KEY')),
        MC_HOST_restore: mcHost(requiredEnv(env, 'RESTORE_S3_ENDPOINT'), requiredEnv(env, 'RESTORE_S3_ACCESS_KEY_ID'), requiredEnv(env, 'RESTORE_S3_SECRET_ACCESS_KEY')),
      },
    })
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  restoreStagingBackup().catch((error) => {
    console.error(JSON.stringify({ event: 'staging_restore_drill_failed', error: error instanceof Error ? error.name : 'UnknownError' }))
    process.exitCode = 1
  })
}
