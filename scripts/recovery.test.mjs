...
} from "./configure-object-lifecycle.mjs";
import {
...
function restoreEnv(overrides = {}) {
  return {
    ...
    RESTORE_DATABASE_URL:
      "postgres://restore_user:secret@restore-postgres:5432/staging_restore",
...
test("restore isolation requires distinct restore-only database, Redis, and bucket resources", () => {
  ...
  assert.throws(
    () =>
      assertRestoreIsolation(
        restoreEnv({ RESTORE_S3_BUCKET: "staging-assets" }),
      ),
    /restore-only/,
  );
  assert.throws(
    () =>
      assertRestoreIsolation(
        restoreEnv({
          RESTORE_DATABASE_URL: "postgres://user:secret@postgres:5432/staging",
        }),
      ),
    /restore-only/,
  );
  assert.throws(
    () =>
      assertRestoreIsolation(restoreEnv({ RESTORE_RESET_CONFIRMED: "false" })),
    /must be true/,
  );
  assert.throws(
...
test("object lifecycle retains current formal assets and expires backup snapshots separately", () => {
  ...
  assert.equal(
    primary.some((rule) => rule.Expiration?.Days),
    false,
  );
  assert.equal(
    primary.find((rule) => rule.ID === "retain-noncurrent-formal-assets")
