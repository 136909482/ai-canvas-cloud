...
function positiveDays(value, key, maximum = 3650) {
  ...
  if (!Number.isInteger(days) || days < 1 || days > maximum) throw new Error(`${key} must be between 1 and ${maximum}`)
  return days
}
...
export async function configureObjectLifecycle(env = process.env) {
  ...
  const primaryClient = createClient(
    requiredEnv(env, 'S3_ENDPOINT'), region,
    requiredEnv(env, 'S3_ACCESS_KEY_ID'), requiredEnv(env, 'S3_SECRET_ACCESS_KEY'),
  )
  const backupClient = createClient(
    requiredEnv(env, 'BACKUP_S3_ENDPOINT'), region,
    requiredEnv(env, 'BACKUP_S3_ACCESS_KEY_ID'), requiredEnv(env, 'BACKUP_S3_SECRET_ACCESS_KEY'),
  )
  try {
    await configureBucket(
      primaryClient,
      requiredEnv(env, 'S3_BUCKET'),
      primaryLifecycleRules(positiveDays(requiredEnv(env, 'OBJECT_NONCURRENT_RETENTION_DAYS'), 'OBJECT_NONCURRENT_RETENTION_DAYS')),
    )
    await configureBucket(
      backupClient,
      requiredEnv(env, 'BACKUP_S3_BUCKET'),
      backupLifecycleRules(positiveDays(requiredEnv(env, 'BACKUP_RETENTION_DAYS'), 'BACKUP_RETENTION_DAYS', 365)),
    )
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  configureObjectLifecycle().catch((error) => {
    console.error(JSON.stringify({ event: 'object_lifecycle_failed', error: error instanceof Error ? error.name : 'UnknownError' }))
    process.exitCode = 1
  })
}
