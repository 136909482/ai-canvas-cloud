...
async function applyMigration(client, migration) {
  ...
  try {
    const lockTimeoutMs =
      migration.release.lockRisk === "low"
        ? 1000
        : migration.release.lockRisk === "medium"
          ? 5000
          : 10000;
    await client.query(`SET LOCAL lock_timeout = '${lockTimeoutMs}ms'`);
    await client.query(
      `SET LOCAL statement_timeout = '${migration.release.statementTimeoutMs}ms'`,
    );
...
if (
  ...
  const { validateProtectedDeploymentEnvironment } =
...
try {
  ...
  console.log(
    `Database migrations complete. Applied ${appliedCount}, skipped ${skippedCount}.`,
  );
} finally {
  await client.end();
}
