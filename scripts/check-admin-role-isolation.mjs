...
for (const key of [
  ...
  assert.equal(Boolean(process.env[key]?.trim()), false, `${key} must be removed`)
}
...
const expectedMigrationVersions = Array.from({ length: 29 }, (_, index) => String(index + 1).padStart(4, '0'))
...
for (const [connection, connectionString] of Object.entries(connections)) {
  ...
  try {
    ...
    assert.deepEqual(removedRelations.rows, [], 'server generation relations must be removed')
    ...
    assert.deepEqual(removedFunctions.rows, [], 'server generation functions must be removed')
    ...
    `)
    assert.equal(taskReferenceColumn.rowCount, 0, 'asset_references.task_id must be removed')
    if (connection === 'app') {
      const appliedMigrations = await client.query('SELECT version FROM schema_migrations ORDER BY version')
      assert.deepEqual(appliedMigrations.rows.map((row) => row.version), expectedMigrationVersions, 'live migration history must match the 29-file release manifest')
    }
    ...
    `)
    assert.equal(activePublication.rows[0]?.has_official_mode ?? false, false, 'active site publication must not expose retired official mode')
    if (connection === 'admin') {
      ...
      `)
      assert.equal(activeRevision.rows[0]?.has_official_mode ?? false, false, 'active Admin site revision must not retain retired official mode')
    }
