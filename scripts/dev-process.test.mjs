...
function manifest(overrides = {}) {
  ...
  return {
    ...
    cwd,
    scriptPath,
    cwdProof: createHash("sha256")
      .update(`${token}\0${pid}\0${proofCwd}`)
      .digest("hex"),
...
test("managed process identity requires PID, repository cwd, command line, and ownership token", () => {
  ...
  assert.deepEqual(validateManagedProcess(value, processInfo(value), "api"), {
    ok: true,
  });
});
...
test("managed process identity rejects a different working directory or command line", () => {
  ...
  assert.equal(
    validateManagedProcess(wrongCwd, processInfo(wrongCwd), "api").reason,
...
test("managed process identity rejects PID reuse and another service token", () => {
  ...
  assert.equal(
    validateManagedProcess(
      value,
      { ...processInfo(value), pid: value.pid + 1 },
      "api",
    ).reason,
    "pid_or_service_mismatch",
  );
  assert.equal(
    validateManagedProcess(
      value,
