...
浏览器 Vault 与任务缓存没有 Cloud HTTP 资源。配置 Vault 和任务缓存均为 `schemaVersion=2`、`cipherVersion=1`，IndexedDB 数据库版本为 2，使用不可导出的 WebCrypto AES-256-GCM `CryptoKey`；AAD 绑定版本、Origin 和可信 session 用户 ID，任务缓存还绑定项目 ID。Provider 配置不含 Key，Key 按 `providerProfileId` 存在独立凭据槽，模型条目以 `modelEntryId` 为唯一身份。Provider 与模型配置保存后固定把密文与 Key 写入当前 Origin 的 IndexedDB，不提供 persistence 或单独删除入口；登出/session 失效/换账号清空内存但不删除设备记录；清除当前网站数据会由浏览器删除密文、Key、模型绑定和本地任务缓存。当前内测环境没有旧数据，不支持历史 localStorage 或旧密文迁移。
...
服务商和模型选择全部在浏览器内完成，不存在对应 Cloud API。节点仅列出当前类别下可执行的服务商与模型；未绑定、已删除、缺失、停用或凭据无效的当前引用仍可见但请求在浏览器适配器前被拒绝。匿名引用绑定后，执行请求和本地任务使用该绑定解析出的 `modelEntryId`，而项目图字段继续保留 `local:<uuid>`。
