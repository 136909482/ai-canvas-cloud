...
- 首发策略调整为同账号单活跃会话：新登录成功后旧设备 session 必须失效；前端首屏恢复一次 session，可见页面每 5 分钟心跳一次，窗口重新聚焦/可见只在距上次检查已满 5 分钟时触发，业务 API 返回未授权时立即清理登录态，同一标签页并发检查合并为一个请求。开发/测试环境已支持把邮箱验证和密码重置链接输出到日志；生产真实邮件供应商、账号删除申请、浏览器级两账号隔离 E2E、新登录踢旧设备 E2E 和更完整限流审计待后续批次接入。
