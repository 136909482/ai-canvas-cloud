...
`community` 领域当前已实现独立公开昵称和版本化投稿授权。登录用户通过 `/api/v1/community/profile` 读取或更新自己的社区资料；登录用户名继续不可修改，公开昵称不进入认证身份。资料服务再次锁定并验证账号为 `active`，`disabled|deleted` 用户不能读取、修改或取得投稿资格。撤回授权会清除同意版本和时间，`canPost` 立即变为 false；账号注销事务立即删除公开资料并释放昵称。
