...
AI Canvas Cloud 是账号制 AI 画布 SaaS，提供账号、个人空间、云端项目图、私有媒体资产、目录包迁移、官方图片模型和独立 Admin。它不承载 Electron、本地 SQLite 或 File System Access API，也不代理用户自定义 Provider；官方 Provider 是平台预设、服务端加密保存并通过持久任务执行的独立能力。
...
`community` 领域当前已实现独立公开昵称（设置页账号信息区显示为「用户昵称」）。登录用户通过 `/api/v1/community/profile` 读取或更新自己的社区资料；登录用户名继续不可修改，公开昵称不进入认证身份。资料服务再次锁定并验证账号为 `active`，`disabled|deleted` 用户不能读取、修改或取得投稿资格。投稿资格只要求资料状态为 `active` 且已设置公开昵称：主动投稿动作本身就是展示授权，设置页不再要求勾选授权协议；`community_consent_*` 字段保留用于兼容与记录，不再参与 `canPost` 判定，清除昵称才会回收投稿资格。账号注销事务立即删除公开资料并释放昵称。

社区是 Cloud 的公开展示域，但不改变个人 workspace 的私有属性。用户必须主动选择图片投稿，现有私有资产不会自动公开。第一版规划只展示图片、标题、标签、发布时间和独立公开昵称；Prompt、Provider、endpoint、真实模型 ID、API Key、项目节点、工作流结构、object key 和签名 URL 不进入社区响应。

投稿只能引用当前可信用户在当前 workspace 中拥有的 `completed` 图片资产。作者可编辑待审核、已拒绝和已发布投稿的标题与标签：待审核保持待审核，已拒绝和已发布编辑后回到 `pending_review` 重新排队（已发布内容编辑期间不公开，重新审核通过后重新公开），已撤回/已下架不可编辑。社区发布、撤回、编辑、审核、举报和下架必须由独立领域服务负责，HTTP 路由不得直接写社区、资产或项目表。已发布社区内容对资产形成保护引用，必须参与资产 GC 判断；撤回、拒绝或下架后才释放该保护。Admin 只读取审核所需的最小内容，不读取 Provider 配置或项目正文。
...
- OpenAI Compatible 图片模型的真实模型 ID 原样透传；可识别的 GPT Image、Gemini 和 Nano Banana 家族使用各自扩展参数，服务商自定义且无法识别的图片模型 ID 使用最小通用 Images 请求，不以官方模型 ID 白名单阻断执行。模型目录发现会将 `nano-banana`、`nano_banana` 和 `nanobanana` 变体归类为图片；已导入条目的类别仍保留用户选择。

### 官方图片生成与积分

- 节点模型引用按来源解析：`official:<modelUuid>` 表示官方模型，旧有/普通模型条目 ID 表示浏览器 Vault 自定义模型。选择器固定分为“官方模型”和“我的模型”，两者不自动回退。
- 新官方任务要求站点发布配置 `officialGenerationEnabled=true` 且用户偏好开启；关闭个人开关只阻止新任务。全站关闭会拒绝新任务并由执行器退回排队任务积分，运行中任务继续结算。
- API 进程内执行器通过 PostgreSQL `FOR UPDATE SKIP LOCKED` 领取任务。请求参数和 Provider 凭据用不同用途的 AES-256-GCM 信封暂存，终态清除请求载荷；Prompt、上游正文、endpoint、密钥和真实模型 ID不得进入日志或普通任务记录。
- 积分提交时从可用转为预留，资产成功落库后扣除预留，失败或排队取消时退回。平台不提供充值、支付、订单、提现、积分转赠或现金价值功能。
