...
平台不提供官方模型、积分、计费或服务器代生成。平台负责账号、项目图、私有资产存储和管理端；用户 Provider、endpoint、模型 ID 与 API Key 只存在于登录用户当前浏览器。

P8-4 将删除当前未提交的官方目录/积分实现和旧服务器 BYOK/生成路径。在清理完成前，这些代码只视为待移除的过渡实现，不再扩展其接口或数据模型。

### 浏览器 Vault

Provider、Key、模型和绑定必须进入同一个版本化 IndexedDB Vault，并按 Origin 与可信 session 用户 ID 分区。默认“记住此设备”使用 WebCrypto 生成的不可导出 AES-GCM `CryptoKey`、随机 IV 和包含版本、Origin、用户 ID、用途的 AAD；“仅本次会话”只使用内存。

登出或会话失效立即清除内存明文，但不擅自删除用户选择保留的密文。“忘记此设备”删除密文、CryptoKey、模型绑定和本地任务缓存。旧 localStorage 配置迁移成功后必须删除原明文。Vault 只保护离线缓存，不宣称抵御正在执行的 XSS 或恶意浏览器扩展。

生产自定义 endpoint 只允许 HTTPS，并拒绝 URL 用户名、密码和 fragment。所有请求错误、Provider 响应、调试输出和诊断在进入日志、节点或缓存前必须移除 Authorization、API Key、query 凭据和用户正文。

### 浏览器执行与云端结果

Web 端只使用受控协议 adapter 调用用户 Provider，首期支持 OpenAI Compatible 与阿里 DashScope 的 chat/image/video。允许用户填写 endpoint 和模型 ID，但不接受任意 JavaScript、自由 Header/Body 模板或平台转发 target URL。

Provider 必须支持浏览器 CORS，或由用户提供固定目标的自建 CORS 网关。平台不提供任意公网代理，也不替浏览器抓取任意结果 URL。Base64 或二进制结果在浏览器转换为 Blob；URL 结果必须能由浏览器读取。

媒体结果沿用 Cloud 资产上传边界：可信 session 创建一次性上传会话，浏览器使用短期签名 URL 无 Cookie 直传，API 完成确认后通过项目图领域操作写入节点。聊天文本通过正常图增量保存。前端不得读取对象存储管理凭据，API 路由不得直接修改项目图表。

云端节点只保存匿名 `local:<uuid>` 模型引用。真实 Provider、endpoint、模型 ID、显示名和绑定只在 Vault；另一设备打开项目时必须显式绑定本机模型，不得自动替换。

### 本地任务与服务端清理

浏览器任务只存在内存或加密 IndexedDB，不写 PostgreSQL。同步任务在页面关闭后中断；异步 Provider 若已返回 remote task ID，可在同一设备重新打开后继续轮询。仅本次会话不提供关闭后的恢复。

Redis 继续用于 API 分布式限流，但不再承载 BullMQ 生成队列。生成 Worker、Worker 数据库角色、Provider 密钥环、服务端任务 API 和对应任务表将在 P8 contract 清理中移除。

### Admin 隔离

`admin-web` 与 `admin-api` 使用独立域名、Origin allowlist、Cookie、Better Auth Secret、数据库角色和 `admin` schema。普通用户 session 不可用于管理端；普通 API 无权读取管理员身份、登录安全或审计表。

管理员固定角色为 `super_admin`、`operator`、`support`、`auditor`，使用独立账号和密码登录。图片验证码默认关闭，只能由 `super_admin` 开启。首个 `super_admin` 只能通过一次性交互式 bootstrap 创建，不通过环境变量自动 seed。

管理端不提供用户模拟。站点设置、用户状态、session 撤销和审计必须由 `server/modules/admin` 领域服务完成；路由不得直接修改身份表、项目图、资产或普通用户数据。审计不得包含密码、Token、验证码、用户正文或浏览器本地 Provider 配置。

P8-2 的 Admin 安全底座和 P8-3 的结构化网站设置继续保留。网站配置发布使用不可变修订、受控品牌资产和最小公开投影；公开端点失败时 Web 使用内置安全默认值。
...
P8 contract 清理完成后只保留 API readiness：PostgreSQL 使用 `SELECT 1`，Redis 使用 `PING`，对象存储使用 `HeadBucket`。共享测量层为单项检查设置有界超时并统一映射上述分类；未注入真实检查时必须以 `unknown` degraded 失败。`HeadBucket` 同时验证服务可达、Bucket 存在和当前服务端凭据具备 Bucket 权限。
