...
import { AuthServiceError, normalizeEmail, validatePassword, type AuthService } from './service.js'
...
export async function seedDevelopmentAdminAccount(options: DevelopmentAdminSeedOptions) {
  if (!options.enabled) {
    return
  }
  ...
  if (options.env === 'production') {
    options.logger.warn('auth.dev_admin_seed.disabled_in_production', { email })
    return
  }
