...
test('PostgreSQL checkpoints persist asset manifests and restore graph references atomically', {
  ...
  try {
    ...
    const actorA = { userId: 'snapshot-asset-user-a', workspaceId: WORKSPACE_A }
    const actorB = { userId: 'snapshot-asset-user-b', workspaceId: WORKSPACE_B }
    const projectA = (await projects.createProject({ name: 'Snapshot assets A' }, actorA)).project
    const projectB = (await projects.createProject({ name: 'Snapshot assets B' }, actorB)).project
    ...
    const sourceCheckpoint = await snapshots.createCheckpoint(projectA.id, {
      expectedVersion: 1,
      expectedSequence: 1,
    }, actorA)
    const repeatedCheckpoint = await snapshots.createCheckpoint(projectA.id, {
      expectedVersion: 1,
      expectedSequence: 1,
    }, actorA)
    assert.equal(repeatedCheckpoint.checkpoint.id, sourceCheckpoint.checkpoint.id)
    assert.deepEqual((await pool.query<{ asset_manifest_json: string[] }>(
      `SELECT asset_manifest_json FROM project_snapshots WHERE id = $1`,
      [sourceCheckpoint.checkpoint.id],
    )).rows[0]?.asset_manifest_json, [ASSET_A_SOURCE])
    ...
    const assertProjectAStillUsesResult = async () => {
      ...
      assert.deepEqual((await pool!.query(
        ...
        [projectA.id],
      )).rows, [{ asset_id: ASSET_A_RESULT, reference_role: 'result' }])
    }

    await pool.query(`UPDATE assets SET status = 'pending' WHERE id = $1`, [ASSET_A_SOURCE])
    const snapshotCountBeforeRejectedRestore = (await pool.query(
      `SELECT count(*)::integer AS count FROM project_snapshots WHERE project_id = $1`,
      [projectA.id],
    )).rows[0]!.count as number
    await assert.rejects(
      () => snapshots.restoreRevision(projectA.id, 1, {
        expectedVersion: 2,
        expectedSequence: 2,
      }, actorA),
    ...
    assert.equal((await pool.query(
      `SELECT count(*)::integer AS count FROM project_snapshots WHERE project_id = $1`,
      [projectA.id],
    )).rows[0]!.count, snapshotCountBeforeRejectedRestore)
    await assertProjectAStillUsesResult()

    await pool.query(`UPDATE assets SET status = 'completed' WHERE id = $1`, [ASSET_A_SOURCE])
    ...
    )).rows[0]?.asset_manifest_json, [ASSET_A_RESULT])
    assert.deepEqual((await pool.query(
      `SELECT asset_id::text, reference_role FROM asset_references WHERE project_id = $1`,
      [projectA.id],
    )).rows, [{ asset_id: ASSET_A_SOURCE, reference_role: 'source' }])
    const restoredGraph = await graphs.getGraph(projectA.id, actorA)
    assert.equal(restoredGraph.version, 3)
    assert.equal(
      ((restoredGraph.nodes[0]?.data.imageAsset as { assetId?: string } | undefined)?.assetId),
      ASSET_A_SOURCE,
    )

    const validationProject = (await projects.createProject({ name: 'Snapshot validation' }, actorA)).project
    await graphs.applyOperations(validationProject.id, {
      baseVersion: 0,
    ...
    const validationCheckpoint = await snapshots.createCheckpoint(validationProject.id, {
      expectedVersion: 1,
      expectedSequence: 1,
    }, actorA)
    await graphs.applyOperations(validationProject.id, {
    ...
    const assertValidationProjectUnchanged = async () => {
      ...
      )).rows[0], { version: '2', last_sequence: '2' })
      assert.equal((await pool!.query(
    ...
    await assert.rejects(
      () => snapshots.restoreRevision(validationProject.id, 1, {
        expectedVersion: 2,
        expectedSequence: 2,
      }, actorA),
      (error: unknown) => error instanceof AuthServiceError
        ...
        && error.message === 'Checkpoint asset manifest does not match its graph record',
    )
    await assertValidationProjectUnchanged()

    const targetRecord = (await pool.query<{ record_json: Record<string, unknown> }>(
    ...
    const setTargetAsset = async (assetId: string) => {
      ...
      await pool!.query(
        ...
        [validationCheckpoint.checkpoint.id, JSON.stringify(targetRecord), JSON.stringify([assetId])],
      )
    }
