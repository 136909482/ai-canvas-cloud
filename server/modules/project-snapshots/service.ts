...
export function validateCreateProjectCheckpointRequest(
  ...
  return {
    expectedVersion: requireNonNegativeSafeInteger(
      input.expectedVersion,
      "expectedVersion",
    ),
    expectedSequence: requireNonNegativeSafeInteger(
...
export function validateListProjectRevisionsInput(
  input: ListProjectRevisionsInput,
): Required<ListProjectRevisionsInput> {
  const limit = input.limit ?? PROJECT_REVISIONS_DEFAULT_LIMIT;

  if (
    !Number.isInteger(limit) ||
    limit < 1 ||
    limit > PROJECT_REVISIONS_MAX_LIMIT
  ) {
    return validationError(
      `Project revisions limit must be between 1 and ${PROJECT_REVISIONS_MAX_LIMIT}`,
    );
  }
...
export function validateProjectRevisionVersion(value: unknown) {
  ...
  if (!Number.isSafeInteger(version) || Number(version) < 0) {
    return validationError(
      "Project revision version must be a non-negative safe integer",
    );
  }
