...
export type CheckpointAssetManifestAssessment =
  ...
  | {
      status: "repairable";
      manifest: string[];
      reason: RepairableManifestReason;
    }
...
export function deriveCheckpointAssetManifest(
  record: unknown,
  projectId: string,
) {
  if (
  ...
  const nodes: ProjectGraphNode[] = record.canvas.nodes.map(
    validateProjectGraphNode,
  );
...
export function assessCheckpointAssetManifest(input: {
  ...
  try {
    derivedManifest = deriveCheckpointAssetManifest(
      input.record,
      input.projectId,
    );
  } catch {
  ...
  const reason: RepairableManifestReason =
    normalizedStoredManifest.length === 0 && derivedManifest.length > 0
      ? "empty"
