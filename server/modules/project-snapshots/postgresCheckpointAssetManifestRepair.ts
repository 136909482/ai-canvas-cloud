...
export interface PostgresCheckpointAssetManifestRepairService {
  ...
  ) => Promise<CheckpointAssetRepairBatch>;
  applyBatch: (
    input?: CheckpointAssetRepairBatchInput,
  ) => Promise<CheckpointAssetRepairBatch>;
}
...
function validateBatchSize(value: number | undefined) {
  ...
  if (
    ...
    throw new Error(
      `batchSize must be between 1 and ${CHECKPOINT_ASSET_REPAIR_MAX_BATCH_SIZE}`,
    );
  }
  return batchSize;
}
...
async function preflightRow(
  pool: DbPool,
  row: SnapshotRepairRow,
): Promise<CheckpointAssetRepairItem> {
...
async function markInvalid(
  ...
  if (row.is_valid) {
    await client.query(
      `UPDATE project_snapshots SET is_valid = false WHERE id = $1`,
      [row.id],
    );
  }
...
async function applyRow(
  ...
  return withTransaction(pool, async (client) => {
    ...
    try {
      await requireCompletedAssetIds(
        client,
        row.workspace_id,
        assessment.manifest,
      );
    ...
    return {
      ...
      projectId: row.project_id,
      action:
        assessment.status === "repairable" ? "manifest_repaired" : "unchanged",
      reason:
        assessment.status === "repairable"
          ? assessment.reason
          : "already_consistent",
