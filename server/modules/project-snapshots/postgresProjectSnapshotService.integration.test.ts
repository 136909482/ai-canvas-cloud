...
test('PostgreSQL checkpoints save the current graph and remain tenant scoped', {
  ...
  try {
    ...
    const actorA = { userId: 'snapshot-user-a', workspaceId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' }
    const actorB = { userId: 'snapshot-user-b', workspaceId: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb' }
    const projectA = (await projects.createProject({ name: 'Checkpoint A' }, actorA)).project
    const projectB = (await projects.createProject({ name: 'Checkpoint B' }, actorB)).project

    await graphs.applyOperations(projectA.id, {
      baseVersion: 0,
      clientId: 'browser_a',
    ...
    const stored = await pool.query<{
      ...
      record_json: {
        ...
        canvas: { nodes: Array<{ id: string; data: Record<string, unknown> }>; edges: unknown[] }
    ...
    assert.deepEqual(stored.rows[0]?.record_json.canvas.nodes[0]?.data, { text: 'saved' })
    ...
    const periodicCheckpoint = await snapshots.createCheckpoint(projectA.id, {
      expectedVersion: 2,
      expectedSequence: 2,
    ...
    const savedAfterPeriodic = await pool.query<{ saved_snapshot_id: string }>(
      `SELECT saved_snapshot_id::text FROM projects WHERE id = $1`,
      [projectA.id],
    )
    assert.equal(savedAfterPeriodic.rows[0]?.saved_snapshot_id, checkpoint.checkpoint.id)

    const secondCheckpoint = await snapshots.createCheckpoint(projectA.id, {
      expectedVersion: 2,
      expectedSequence: 2,
    }, actorA)
    const firstPage = await snapshots.listRevisions(projectA.id, { limit: 1 }, actorA)
    assert.deepEqual(firstPage.revisions.map((revision) => revision.id), [secondCheckpoint.checkpoint.id])
    assert(firstPage.nextCursor)
    assert.equal('recordJson' in firstPage.revisions[0]!, false)
    const secondPage = await snapshots.listRevisions(projectA.id, { cursor: firstPage.nextCursor, limit: 1 }, actorA)
    assert.deepEqual(secondPage.revisions.map((revision) => revision.id), [periodicCheckpoint.checkpoint.id])
    assert(secondPage.nextCursor)
    const thirdPage = await snapshots.listRevisions(projectA.id, { cursor: secondPage.nextCursor, limit: 1 }, actorA)
    assert.deepEqual(thirdPage.revisions.map((revision) => revision.id), [checkpoint.checkpoint.id])
    assert.equal(thirdPage.nextCursor, null)
    const revisionDetail = await snapshots.getRevision(projectA.id, 2, actorA)
    assert.equal(revisionDetail.checkpoint.id, secondCheckpoint.checkpoint.id)
    assert.equal(revisionDetail.record.project.id, projectA.id)
    assert.equal(revisionDetail.record.project.version, 2)
    assert.deepEqual(revisionDetail.record.canvas.nodes.map((node) => node.id), ['node-a', 'node-b'])

    const restored = await snapshots.restoreRevision(projectA.id, 1, {
      expectedVersion: 2,
      expectedSequence: 2,
    }, actorA)
    ...
    assert.deepEqual(restoredGraph.nodes.map((node) => node.id), ['node-a'])
    ...
    assert(restoreChanges.changes[0]?.operations.some((operation) => (
      operation.type === 'deleteNode' && operation.nodeId === 'node-b'
    )))

    const preRestoreDetail = await snapshots.getRevision(projectA.id, 2, actorA)
    assert.equal(preRestoreDetail.checkpoint.id, restored.preRestoreCheckpoint.id)
    assert.equal(preRestoreDetail.record.canvas.nodes.length, 2)

    await assert.rejects(
      () => snapshots.createCheckpoint(projectA.id, { expectedVersion: 0, expectedSequence: 0 }, actorA),
    ...
    await assert.rejects(
      () => snapshots.createCheckpoint(projectB.id, { expectedVersion: 0, expectedSequence: 0 }, actorA),
      (error: unknown) => error instanceof AuthServiceError
    ...
    await assert.rejects(
      () => snapshots.restoreRevision(projectB.id, 0, { expectedVersion: 0, expectedSequence: 0 }, actorA),
      (error: unknown) => error instanceof AuthServiceError
        && error.statusCode === 404
        && error.apiCode === 'RESOURCE_NOT_FOUND',
    )
    await assert.rejects(
      () => snapshots.restoreRevision(projectA.id, 99, { expectedVersion: 3, expectedSequence: 3 }, actorA),
      (error: unknown) => error instanceof AuthServiceError
        && error.statusCode === 404
        && error.apiCode === 'RESOURCE_NOT_FOUND',
    )
    await assert.rejects(
      () => snapshots.restoreRevision(projectA.id, 1, { expectedVersion: 2, expectedSequence: 2 }, actorA),
      (error: unknown) => error instanceof AuthServiceError
        && error.statusCode === 409
        && error.apiCode === 'PROJECT_VERSION_CONFLICT'
        && error.details?.currentVersion === 3,
    ...
    await assert.rejects(
      () => snapshots.createCheckpoint(projectA.id, { expectedVersion: 3, expectedSequence: 3 }, actorA),
      (error: unknown) => error instanceof AuthServiceError
        && error.statusCode === 403
        && error.apiCode === 'ACCESS_DENIED',
    )
    await assert.rejects(
      () => snapshots.restoreRevision(projectA.id, 1, { expectedVersion: 3, expectedSequence: 3 }, actorA),
      (error: unknown) => error instanceof AuthServiceError
        && error.statusCode === 403
        && error.apiCode === 'ACCESS_DENIED',
    )
