...
export type AssetGcStatus = 'pending' | 'completed' | 'failed' | 'quarantined' | 'deleted'
export type AssetGcRetentionReason = 'active_asset' | 'current_reference' | 'checkpoint_reference' | 'grace_period' | 'eligible'
...
export function validateAssetMaintenanceBatchSize(value: number | undefined) {
  ...
  if (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > ASSET_MAINTENANCE_MAX_BATCH_SIZE) {
    throw new Error(`batchSize must be between 1 and ${ASSET_MAINTENANCE_MAX_BATCH_SIZE}`)
  }
  return batchSize
}
...
export function validateAssetGcGraceHours(value: number | undefined) {
  ...
  if (!Number.isInteger(graceHours) || graceHours < 1 || graceHours > ASSET_GC_MAX_GRACE_HOURS) {
    throw new Error(`graceHours must be between 1 and ${ASSET_GC_MAX_GRACE_HOURS}`)
  }
  return graceHours
}
