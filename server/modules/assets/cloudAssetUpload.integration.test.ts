...
const hasCloudDependencies = Object.values(config).every((value) =>
  Boolean(value),
);

test(
  "PostgreSQL upload sessions complete a real private MinIO round trip",
  {
    skip: hasCloudDependencies
      ? false
      : "PostgreSQL and S3 object storage are not configured",
    timeout: 30_000,
  },
  async () => {
    ...
    `);
      ...
      const project = (
        await projects.createProject({ name: "Real MinIO upload" }, actor)
      ).project;
