...
test(
  "PostgreSQL asset maintenance protects references, diagnoses missing objects, and converges idempotently",
  {
  ...
  async () => {
    ...
    `,
      ...
      `,
          [
            ...
            WORKSPACE_A,
            PROJECT_A,
            key(WORKSPACE_A, PROJECT_A, assetId),
          ],
        );
      }
      await pool.query(
    ...
    `,
        [
          PROJECT_A,
          JSON.stringify([CHECKPOINT_ASSET]),
          PROJECT_B,
      ...
      const objects = new Map<
        string,
        { lastModified: string; byteSize: number }
      >();
      ...
      let failDeleteKey: string | null = key(
        WORKSPACE_A,
        PROJECT_A,
        ORPHAN_ASSET,
      );
      const storage: AssetMaintenanceObjectStorage = {
        ...
        async listObjectsPage(input) {
          ...
          const all = [...objects.entries()].sort(([left], [right]) =>
          ...
          const next =
            start + page.length < all.length
              ...
              : null;
          return {
            objects: page.map(([objectKey, metadata]) => ({
              objectKey,
              ...metadata,
            })),
            nextStartAfter: next,
      ...
      assert.equal(
        preflightItems.find((item) => item.assetId === CURRENT_ASSET)?.reason,
        "current_reference",
      );
      assert.equal(
      ...
      assert.equal(
        preflightItems.find((item) => item.assetId === MISSING_ASSET)?.action,
        "missing_object",
      );
      ...
      try {
        ...
        await referenceWriter.query(
          `SELECT id FROM assets WHERE id = $1 FOR SHARE`,
          [FAILURE_ASSET],
        );
        await referenceWriter.query(
          `
      ...
      `,
        ...
        assert.equal(
          applied.items.find((item) => item.assetId === FAILURE_ASSET)?.action,
          "skipped_locked",
        );
      ...
      assert.equal(
        applied.items.find((item) => item.assetId === ORPHAN_ASSET)?.action,
      ...
      const rerun = await service.maintainAssetBatch({
        cutoff,
        batchSize: 500,
        apply: true,
      });
      assert.equal(
        rerun.items.find((item) => item.assetId === ORPHAN_ASSET)?.action,
        "retained",
      );
      assert.equal(
        rerun.items.find((item) => item.assetId === FAILURE_ASSET)?.reason,
        "current_reference",
      );
      ...
      assert.equal(
        objectItems.find((item) => item.objectKey === bucketOrphanKey)?.action,
        "orphan_object_deleted",
      );
      assert.equal(
        objectItems.find((item) => item.objectKey === recentBucketOrphanKey)
          ?.reason,
        "grace_period",
      );
      assert.equal(
        objectItems.find((item) => item.objectKey === unrecognizedKey)?.action,
        "ignored_unmanaged",
      );
