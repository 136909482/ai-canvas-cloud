...
test('PostgreSQL asset reads require completed state and isolate two workspaces', {
  ...
  try {
    ...
    const objectStorage: AssetObjectStorage = {
      ...
      async calculateObjectSha256() {
        throw new Error('SHA-256 should not be requested when no digest was declared')
      },
    }

    const projects = createPostgresProjectService(pool)
    const assets = createPostgresAssetService(pool, { objectStorage, readUrlTtlSeconds: 300 })
    const actorA = { userId: 'asset-user-a', workspaceId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' }
    const viewerA = { userId: 'asset-viewer-a', workspaceId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa' }
    const actorB = { userId: 'asset-user-b', workspaceId: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb' }
    const projectA = (await projects.createProject({ name: 'Asset A' }, actorA)).project
