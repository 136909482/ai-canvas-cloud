...
test('PostgreSQL upload reservations enforce workspace quota atomically and idempotently', {
  ...
  try {
    ...
    const objectStorage: AssetObjectStorage = {
      ...
      async getObjectMetadata(objectKey) {
        return { byteSize: objectSizes.get(objectKey) ?? 0, mimeType: 'image/png' }
      },
    ...
    const projectA = (await projects.createProject({ name: 'Quota A' }, actorA)).project
    const projectB = (await projects.createProject({ name: 'Quota B' }, actorB)).project
    const request = (projectId: string, byteSize: number, idempotencyKey: string) => ({
      projectId,
    ...
    const first = await assets.createUpload(request(projectA.id, 60, 'quota-first'), actorA)
    const repeated = await assets.createUpload(request(projectA.id, 60, 'quota-first'), actorA)
    assert.equal(repeated.asset.id, first.asset.id)
    assert.equal((await usage.getCurrentUsage(actorA)).storage.reservedBytes, 60)
    ...
    await assets.createUpload(request(projectA.id, 40, 'quota-exact'), actorA)
    assert.equal((await usage.getCurrentUsage(actorA)).storage.totalBytes, 100)
    ...
    const concurrent = await Promise.allSettled([
      assets.createUpload(request(projectB.id, 60, 'quota-concurrent-a'), actorB),
      assets.createUpload(request(projectB.id, 60, 'quota-concurrent-b'), actorB),
    ])
    ...
    assert.equal((await usage.getCurrentUsage(actorB)).storage.totalBytes, 60)
  } finally {
    await pool?.end()
