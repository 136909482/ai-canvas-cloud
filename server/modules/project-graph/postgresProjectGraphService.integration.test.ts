...
test(
  "PostgreSQL graph batches are atomic, idempotent, versioned, and tenant isolated",
  {
  ...
  async () => {
    ...
    `);
      ...
      const projectA = (
        await projects.createProject({ name: "Graph A" }, actorA)
      ).project;
      const projectB = (
        await projects.createProject({ name: "Graph B" }, actorB)
      ).project;
      const firstBatch = {
        baseVersion: 0,
        clientId: "browser_a",
      ...
      const accepted = await graphs.applyOperations(
        projectA.id,
        firstBatch,
        actorA,
      );
      ...
      assert.deepEqual(
        await graphs.applyOperations(projectA.id, firstBatch, actorA),
        accepted,
      );
      ...
      assert.equal(
        graph.nodes.find((node) => node.id === "node-b")?.parentNodeId,
        "node-a",
      );
      ...
      assert.deepEqual(
        changesAfterFirst.changes.map((change) => change.sequence),
        [2],
      );
      const afterDelete = await graphs.getGraph(projectA.id, actorA);
      assert.deepEqual(
        afterDelete.nodes.map((node) => node.id),
        ["node-a"],
      );
      ...
      assert.deepEqual(
        await graphs.applyOperations(projectA.id, secondBatch, actorA),
        deleted,
      );
      await assert.rejects(
        () =>
      ...
      const concurrentProject = (
        await projects.createProject({ name: "Concurrent" }, actorA)
      ).project;
