...
export function validateProjectGraphNode(value: unknown): ProjectGraphNode {
  ...
  if (value.zIndex !== undefined) {
    if (
      ...
      Number(value.zIndex) < -2_147_483_648 ||
      Number(value.zIndex) > 2_147_483_647
    ) {
...
export function validateProjectGraphEdge(value: unknown): ProjectGraphEdge {
  ...
  return {
    ...
    edgeType:
      optionalString(value.edgeType, "edge.edgeType", ENTITY_ID_MAX_LENGTH) ??
      null,
...
function validateOperation(value: unknown): ProjectGraphOperation {
  ...
  if (value.type === "deleteNode") {
    return {
      type: "deleteNode",
      nodeId: requireString(value.nodeId, "nodeId", ENTITY_ID_MAX_LENGTH),
    };
  }
  ...
  if (value.type === "deleteEdge") {
    return {
      type: "deleteEdge",
      edgeId: requireString(value.edgeId, "edgeId", ENTITY_ID_MAX_LENGTH),
    };
  }
...
export function validateApplyProjectGraphOperationsRequest(
  ...
  if (
    ...
    input.operations.length > PROJECT_GRAPH_MAX_OPERATIONS
  ) {
    return validationError(
      `operations must contain between 1 and ${PROJECT_GRAPH_MAX_OPERATIONS} items`,
    );
  }
  ...
  for (const operation of operations) {
    const ids =
      operation.type === "upsertNode" || operation.type === "deleteNode"
        ? nodeIds
        : edgeIds;
    const id =
    ...
    if (ids.has(id)) {
      return validationError(
        `Entity ${id} is changed more than once in the same batch`,
      );
    }
    ids.add(id);
  }
