...
test('graph request validation rejects duplicate entities and invalid bounds', () => {
  ...
  assert.throws(() => validateApplyProjectGraphOperationsRequest(duplicate), AuthServiceError)
  assert.throws(
    () => validateApplyProjectGraphOperationsRequest({ ...validRequest(), baseVersion: -1 }),
    AuthServiceError,
  )
  assert.throws(
    () => validateApplyProjectGraphOperationsRequest({
      ...validRequest(),
      operations: Array.from({ length: PROJECT_GRAPH_MAX_OPERATIONS + 1 }, (_, index) => ({
...
test('graph request validation rejects invalid geometry and edge data', () => {
  ...
  assert.throws(
    () => validateApplyProjectGraphOperationsRequest({
      ...
      operations: [{
        type: 'upsertEdge',
        edge: { id: 'edge_1', source: 'node_1', target: 'node_2', data: [] },
      }],
    } as never),
    AuthServiceError,
  )
