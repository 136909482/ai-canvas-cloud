...
function parseCloudAssetLocator(value: unknown, field: string) {
  if (
    typeof value !== "string" ||
    !value.toLowerCase().startsWith(CLOUD_ASSET_PREFIX)
  ) {
    return null;
  }
...
function getPrimaryReferenceRole(
  ...
  if (
    value.assetKind === "generated" ||
    value.assetKind === "edit" ||
    value.assetKind === "crop"
  ) {
    return "result";
  }
...
export function extractNodeAssetReferences(
  ...
  const pending: Array<{ value: unknown; path: readonly string[] }> = [
  ...
  while (pending.length > 0) {
    ...
    if (Array.isArray(value)) {
      ...
      value.forEach((entry, index) =>
        pending.push({ value: entry, path: [...path, String(index)] }),
      );
      continue;
    }
    ...
    const relativePathAssetId = parseCloudAssetLocator(
      value.relativePath,
      "relativePath",
    );
    if (
      explicitAssetId &&
      relativePathAssetId &&
      explicitAssetId !== relativePathAssetId
    ) {
      throw new Error(
        "Node assetId and relativePath refer to different Cloud assets",
      );
    }
    ...
    if (primaryAssetId) {
      addReference(
        primaryAssetId,
        getPrimaryReferenceRole(node.nodeType, value, path),
      );
    }

    const thumbnailAssetId = parseCloudAssetLocator(
    ...
    const previewAssetId = parseCloudAssetLocator(
...
export function normalizeAssetManifest(value: unknown) {
  ...
  const assetIds = value.map((assetId) => {
    if (typeof assetId !== "string" || !UUID_PATTERN.test(assetId)) {
      throw new Error(
        "Checkpoint asset manifest must contain valid asset UUIDs",
      );
    }
    return assetId.toLowerCase();
  });
