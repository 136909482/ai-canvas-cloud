...
test('PostgreSQL graph transactions validate and replace node asset references atomically', {
  ...
  try {
    ...
    await pool.query(`
      ...
      VALUES
        ('graph-asset-user-a', 'A', 'a-graph-assets@example.com', true),
        ('graph-asset-user-b', 'B', 'b-graph-assets@example.com', true)
    `)
    await pool.query(`
    ...
    const projectA = (await projects.createProject({ name: 'Asset graph A' }, actorA)).project
    const projectB = (await projects.createProject({ name: 'Asset graph B' }, actorB)).project
    ...
    await insertAsset({ id: COMPLETED_SOURCE, workspaceId: WORKSPACE_A, projectId: projectA.id, userId: actorA.userId, status: 'completed' })
    await insertAsset({ id: COMPLETED_RESULT, workspaceId: WORKSPACE_A, projectId: projectA.id, userId: actorA.userId, status: 'completed', kind: 'generated' })
    await insertAsset({ id: COMPLETED_THUMBNAIL, workspaceId: WORKSPACE_A, projectId: projectA.id, userId: actorA.userId, status: 'completed', kind: 'thumbnail' })
    await insertAsset({ id: PENDING_ASSET, workspaceId: WORKSPACE_A, projectId: projectA.id, userId: actorA.userId, status: 'pending' })
    await insertAsset({ id: FAILED_ASSET, workspaceId: WORKSPACE_A, projectId: projectA.id, userId: actorA.userId, status: 'failed' })
    await insertAsset({ id: QUARANTINED_ASSET, workspaceId: WORKSPACE_A, projectId: projectA.id, userId: actorA.userId, status: 'quarantined' })
    await insertAsset({ id: CROSS_WORKSPACE_ASSET, workspaceId: WORKSPACE_B, projectId: projectB.id, userId: actorB.userId, status: 'completed' })
    ...
    const firstAccepted = await graphs.applyOperations(projectA.id, firstBatch, actorA)
    ...
    const replacementAccepted = await graphs.applyOperations(projectA.id, replacementBatch, actorA)
    assert.equal(replacementAccepted.version, 2)
    assert.deepEqual(await graphs.applyOperations(projectA.id, replacementBatch, actorA), replacementAccepted)
    assert.deepEqual((await pool.query(
      `SELECT asset_id::text, reference_role FROM asset_references WHERE project_id = $1`,
      [projectA.id],
    )).rows, [{ asset_id: COMPLETED_RESULT, reference_role: 'result' }])
    ...
    const assertUnchangedAtVersionTwo = async () => {
      ...
      assert.equal((await pool!.query(`SELECT 1 FROM project_changes WHERE project_id = $1`, [projectA.id])).rowCount, 2)
      assert.deepEqual((await pool!.query(
        `SELECT asset_id::text, reference_role FROM asset_references WHERE project_id = $1`,
        [projectA.id],
      )).rows, [{ asset_id: COMPLETED_RESULT, reference_role: 'result' }])
    ...
    await assert.rejects(
      () => graphs.applyOperations(projectA.id, {
        ...
        operations: [
          ...
          { type: 'upsertEdge', edge: { id: 'edge-invalid', source: 'node-media', target: 'node-invalid' } },
        ],
      }, actorA),
    ...
    assert.equal((await pool.query(`SELECT 1 FROM project_nodes WHERE project_id = $1 AND node_id = 'node-invalid'`, [projectA.id])).rowCount, 0)
    assert.equal((await pool.query(`SELECT 1 FROM project_edges WHERE project_id = $1 AND edge_id = 'edge-invalid'`, [projectA.id])).rowCount, 0)
    await assertUnchangedAtVersionTwo()

    for (const [assetId, suffix] of [[FAILED_ASSET, 'failed'], [QUARANTINED_ASSET, 'quarantined']] as const) {
      await assert.rejects(
        () => graphs.applyOperations(projectA.id, {
          baseVersion: 2,
          clientId: 'asset-browser-a',
    ...
    for (const [assetId, suffix] of [[CROSS_WORKSPACE_ASSET, 'cross'], [MISSING_ASSET, 'missing']] as const) {
      await assert.rejects(
        () => graphs.applyOperations(projectA.id, {
          baseVersion: 2,
          clientId: 'asset-browser-a',
    ...
    const deleted = await graphs.applyOperations(projectA.id, {
      baseVersion: 2,
      clientId: 'asset-browser-a',
    ...
    assert.equal((await pool.query(`SELECT 1 FROM asset_references WHERE project_id = $1`, [projectA.id])).rowCount, 0)
  } finally {
    await pool?.end()
