...
function prepareRequest(
  ...
  const files = [
    {
      path: "assets.json",
      byteSize: Buffer.byteLength(jsonFiles[0]!.content),
      sha256: hash(jsonFiles[0]!.content),
    },
    { path: `assets/${ASSET_ID}.png`, byteSize: 20, sha256: ASSET_HASH },
    {
      path: "graph.json",
      byteSize: Buffer.byteLength(jsonFiles[1]!.content),
      sha256: hash(jsonFiles[1]!.content),
    },
    {
      path: "project.json",
      byteSize: Buffer.byteLength(jsonFiles[2]!.content),
      sha256: hash(jsonFiles[2]!.content),
    },
  ];
    ...
    archiveEntries: [
      {
        path: "assets.json",
        kind: "file" as const,
        uncompressedSize: files[0]!.byteSize,
        compressedSize: files[0]!.byteSize,
      ...
      },
      {
        path: "graph.json",
        kind: "file" as const,
        uncompressedSize: files[2]!.byteSize,
        compressedSize: files[2]!.byteSize,
      ...
      },
      {
        path: "project.json",
        kind: "file" as const,
        uncompressedSize: files[3]!.byteSize,
        compressedSize: files[3]!.byteSize,
...
test(
  "PostgreSQL migration prepare is durable, idempotent, quota-safe, and workspace isolated",
  {
  ...
  async () => {
    ...
    `,
      ...
      assert.equal(
        (
          await pool.query(
            `SELECT count(*)::integer AS count FROM migration_imports WHERE workspace_id = $1`,
            [WORKSPACE_A],
          )
        ).rows[0]?.count,
        1,
      ...
      await assert.rejects(
        () =>
          service.prepareImport(
            prepareRequest("prepare-one", "Different valid package"),
            actorA,
          ),
        (error: unknown) =>
      ...
      assert.equal(
        (await service.getImport(prepared.import.id, actorA)).import.id,
        prepared.import.id,
      );
      ...
      const crossWorkspace = await service.prepareImport(
        prepareRequest("cross-workspace-id"),
        actorB,
      );
      assert.equal(
        crossWorkspace.import.conflict.type,
      ...
      await pool.query(
        `UPDATE workspaces SET storage_quota_bytes = 10 WHERE id = $1`,
        [WORKSPACE_B],
      );
      await assert.rejects(
      ...
      assert.equal(
        (
          await pool.query(
            `SELECT count(*)::integer AS count FROM migration_imports WHERE idempotency_key = 'quota-over'`,
          )
        ).rows[0]?.count,
        0,
      );
      ...
      assert.equal(
        (await service.cancelImport(prepared.import.id, actorA)).import.status,
        "canceled",
      );
      const expiring = await service.prepareImport(
        prepareRequest("prepare-expired"),
        actorA,
      );
      await pool.query(
        `
    ...
    `,
      ...
      assert.equal(
        (await service.getImport(expiring.import.id, actorA)).import.status,
        "expired",
      );
      ...
      assert.equal(
        (
          await pool.query(
            `SELECT count(*)::integer AS count FROM asset_references`,
          )
        ).rows[0]?.count,
        0,
      );
