...
interface SavedSnapshotRow {
  ...
  created_at: Date | string
  record_json: {
    project: { id: string; name: string; version: number; lastSequence: number }
...
function supportsRetryColumn(pool: DbPool) {
  ...
  const pending = pool.query<{ supported: boolean }>(`
    SELECT EXISTS (
...
function assertProjectId(projectId: string) {
  if (!UUID_PATTERN.test(projectId)) {
    throw new AuthServiceError({ statusCode: 400, apiCode: 'VALIDATION_FAILED', message: 'Invalid project id' })
  }
}

function normalizeExportId(exportId: unknown) {
  if (typeof exportId !== 'string' || !UUID_PATTERN.test(exportId)) {
    throw new AuthServiceError({ statusCode: 400, apiCode: 'VALIDATION_FAILED', message: 'Invalid migration export id' })
  }
  return exportId.toLowerCase()
}

function exportNotFound(): never {
  throw new AuthServiceError({ statusCode: 404, apiCode: 'RESOURCE_NOT_FOUND', message: 'Migration export not found' })
}
...
function toResponse(row: ExportRow): MigrationExportResponse {
  return {
    export: {
      ...
      archive: row.archive_byte_size !== null && row.archive_sha256
        ? { byteSize: Number(row.archive_byte_size), sha256: row.archive_sha256 }
        : null,
...
function normalizeRequest(input: unknown): PrepareMigrationExportRequest {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new AuthServiceError({ statusCode: 400, apiCode: 'VALIDATION_FAILED', message: 'Migration export request must be an object' })
  }
  const value = input as Record<string, unknown>
  const allowed = new Set(['idempotencyKey', 'expectedVersion', 'expectedSequence'])
  if (Object.keys(value).some((key) => !allowed.has(key))) {
    throw new AuthServiceError({ statusCode: 400, apiCode: 'VALIDATION_FAILED', message: 'Migration export request contains unsupported fields' })
  }
  if (typeof value.idempotencyKey !== 'string' || value.idempotencyKey.trim().length < 1 || value.idempotencyKey.length > 200) {
    throw new AuthServiceError({ statusCode: 400, apiCode: 'VALIDATION_FAILED', message: 'idempotencyKey must be between 1 and 200 characters' })
  }
  for (const field of ['expectedVersion', 'expectedSequence'] as const) {
    if (value[field] !== undefined && (!Number.isSafeInteger(value[field]) || Number(value[field]) < 0)) {
      throw new AuthServiceError({ statusCode: 400, apiCode: 'VALIDATION_FAILED', message: `${field} must be a non-negative safe integer` })
    }
  }
  return {
    idempotencyKey: value.idempotencyKey.trim(),
    ...(value.expectedVersion === undefined ? {} : { expectedVersion: Number(value.expectedVersion) }),
    ...(value.expectedSequence === undefined ? {} : { expectedSequence: Number(value.expectedSequence) }),
  }
}
...
function rewriteValue(value: unknown, key: string | null, byId: ReadonlyMap<string, ExportAssetFile>, byPath: ReadonlyMap<string, ExportAssetFile>): unknown {
  if (typeof value === 'string') {
    if (key === 'assetId') {
      return byId.get(value.toLowerCase())?.logicalAssetId ?? value
    }
    if (key === 'relativePath' || key === 'thumbnailRelativePath' || key === 'previewRelativePath') {
      return byPath.get(value)?.filePath ?? byPath.get(value.toLowerCase())?.filePath ?? value
    }
    return value
  }
...
function rewriteNodes(nodes: ProjectGraphNode[], byId: ReadonlyMap<string, ExportAssetFile>, byPath: ReadonlyMap<string, ExportAssetFile>) {
  return nodes.map((node) => ({
    ...node,
    data: rewriteValue(node.data, null, byId, byPath),
    ...(node.presentation ? { presentation: rewriteValue(node.presentation, null, byId, byPath) } : {}),
  }))
}

function rewriteEdges(edges: ProjectGraphEdge[], byId: ReadonlyMap<string, ExportAssetFile>, byPath: ReadonlyMap<string, ExportAssetFile>) {
...
function toMigrationSnapshot(
  ...
  return {
    schemaVersion: 1,
    canvas: {
      nodes: rewriteNodes(nodes, byId, byPath) as unknown as MigrationJsonObject[],
      edges: rewriteEdges(edges, byId, byPath) as unknown as MigrationJsonObject[],
    },
    taskQueue: { tasks: [] },
  }
}
...
function assetIdsFromNodes(nodes: ProjectGraphNode[]) {
  return collectAssetIdsFromNodeReferenceChanges(collectNodeAssetReferenceChangesForNodes(nodes))
}
...
function buildExportPackage(input: {
  ...
  for (const [path, body] of input.jsonFiles) {
    ...
    descriptorFiles.push({ path, byteSize: body.byteLength, sha256: createHash('sha256').update(body).digest('hex') })
  }
  for (const asset of input.assetManifest.assets) {
    const body = input.assetBodies.get(asset.logicalAssetId)
    if (!body) throw new Error(`Export asset body missing: ${asset.logicalAssetId}`)
    descriptorFiles.push({ path: asset.filePath, byteSize: body.byteLength, sha256: createHash('sha256').update(body).digest('hex') })
  }
  ...
  const manifest: MigrationPackageManifest = {
    ...
    totalByteSize: descriptorFiles.reduce((total, file) => total + file.byteSize, 0),
    files: descriptorFiles,
    contentSha256: createHash('sha256').update(createMigrationPackageContentDigestInput(descriptorFiles) as string).digest('hex'),
  }
  ...
  for (const descriptor of descriptorFiles) {
    const body = input.jsonFiles.get(descriptor.path) ?? input.assetBodies.get(input.assetManifest.assets.find((asset) => asset.filePath === descriptor.path)?.logicalAssetId ?? '')
    if (!body) throw new Error(`Export descriptor body missing: ${descriptor.path}`)
...
export function createPostgresMigrationExportService(
  ...
  async function findExport(client: Pick<DbClient, 'query'>, projectId: string, exportId: string, workspaceId: string, lock = false) {
  ...
  async function expireExport(client: Pick<DbClient, 'query'>, row: ExportRow) {
    if (new Date(row.expires_at).getTime() <= Date.now() && !['completed', 'canceled', 'expired'].includes(row.status)) {
      const result = await client.query<ExportRow>(
  ...
  return {
      ...
      const prepared = await withTransaction(pool, async (client) => {
        ...
        if (existing) {
          if (existing.request_fingerprint !== fingerprint) {
            throw new AuthServiceError({ statusCode: 409, apiCode: 'EXPORT_CONFLICT', message: 'Migration export idempotency key was reused with different content' })
          }
          return existing
        }
        ...
        if (project.archived_at) {
          throw new AuthServiceError({ statusCode: 403, apiCode: 'ACCESS_DENIED', message: 'Archived projects cannot be exported' })
        }
        ...
        if ((input.expectedVersion !== undefined && input.expectedVersion !== version)
          || (input.expectedSequence !== undefined && input.expectedSequence !== sequence)) {
        ...
        const savedAssetIds = saved ? assetIdsFromNodes(saved.record_json.canvas.nodes) : []
        const sourceNodes = saved ? [...graph.nodes_json, ...saved.record_json.canvas.nodes] : graph.nodes_json
        ...
        const assetsResult = assetIds.length > 0
          ...
          : { rows: [] as AssetRow[] }
        if (assetsResult.rows.length !== assetIds.length || assetsResult.rows.some((asset) => !asset.sha256 || !SHA256_PATTERN.test(asset.sha256))) {
          throw new AuthServiceError({ statusCode: 409, apiCode: 'ASSET_NOT_READY', message: 'Project contains an asset that cannot be exported' })
        }
        const exportAssets = createPortableAssetFiles(assetsResult.rows)
        const byId = new Map(assetsResult.rows.map((asset, index) => [asset.id.toLowerCase(), exportAssets[index]!]))
        const byPath = new Map(assetsResult.rows.map((asset, index) => [`cloud-assets/${asset.id.toLowerCase()}`, exportAssets[index]!]))
        const workingSnapshot = toMigrationSnapshot(graph.nodes_json, graph.edges_json, byId, byPath)
        const savedSnapshot = saved
          ? toMigrationSnapshot(saved.record_json.canvas.nodes, saved.record_json.canvas.edges, byId, byPath)
          : workingSnapshot
        ...
        const portableGraph: MigrationProjectGraph = {
          ...
          version,
          sequence,
          nodes: workingSnapshot.canvas.nodes as unknown as MigrationProjectGraph['nodes'],
          edges: workingSnapshot.canvas.edges as unknown as MigrationProjectGraph['edges'],
        }
        const portableAssets = assetsResult.rows.map((asset, index) => ({
        ...
        const checkpoint: MigrationPackageCheckpoint | null = saved
          ? {
              ...
              assetIds: savedAssetIds.map((assetId) => byId.get(assetId)?.logicalAssetId ?? assetId),
              record: {
                schemaVersion: 1,
                project: { id: project.id, name: project.name, version: Number(saved.project_version), lastSequence: Number(saved.last_sequence) },
                canvas: savedSnapshot.canvas as unknown as MigrationPackageCheckpoint['record']['canvas'],
        ...
        jsonFiles.set('assets.json', canonicalJson({ schemaVersion: 1, assets: portableAssets }))
        if (checkpoint) jsonFiles.set('checkpoint.json', canonicalJson(checkpoint))
        const descriptors = [...jsonFiles.entries()].map(([path, body]) => ({ path, byteSize: body.byteLength, sha256: createHash('sha256').update(body).digest('hex') }))
        descriptors.push(...portableAssets.map((asset) => ({ path: asset.filePath, byteSize: asset.byteSize, sha256: asset.sha256 })))
        ...
        const manifest: MigrationPackageManifest = {
          ...
          totalByteSize: descriptors.reduce((total, descriptor) => total + descriptor.byteSize, 0),
          contentSha256: createHash('sha256').update(createMigrationPackageContentDigestInput(descriptors) as string).digest('hex'),
          files: descriptors,
        }
        const manifestBytes = canonicalJson(manifest)
        const totalBytes = manifestBytes.byteLength + descriptors.reduce((total, descriptor) => total + descriptor.byteSize, 0)
        const exportAssetsJson = assetsResult.rows.map((_, index) => ({ ...exportAssets[index]! }))
        const inserted = await client.query<ExportRow>(
          ...
          [
            actor.workspaceId, actor.userId, project.id, input.idempotencyKey, fingerprint,
            project.name, version, sequence, descriptors.length + 1, totalBytes,
            JSON.stringify(manifest), JSON.stringify(projectRecord), JSON.stringify(portableGraph), JSON.stringify({ schemaVersion: 1, assets: portableAssets }), checkpoint ? JSON.stringify(checkpoint) : null, JSON.stringify(exportAssetsJson), MIGRATION_EXPORT_TTL_HOURS,
          ],
        )
    ...
    async getExport(rawProjectId, rawExportId, actor) {
      ...
      return withTransaction(pool, async (client) => {
        const current = await findExport(client, rawProjectId, exportId, actor.workspaceId, true)
    ...
    async cancelExport(rawProjectId, rawExportId, actor) {
      ...
      return withTransaction(pool, async (client) => {
        ...
        if (expired.status === 'completed') {
          throw new AuthServiceError({ statusCode: 409, apiCode: 'EXPORT_CONFLICT', message: 'Completed migration export cannot be canceled' })
        }
        if (['canceled', 'expired', 'failed'].includes(expired.status)) return toResponse(expired)
        const result = await client.query<ExportRow>(
    ...
    async retryExport(rawProjectId, rawExportId, actor) {
      ...
      const response = await withTransaction(pool, async (client) => {
        ...
        if (!['failed', 'canceled'].includes(current.status)) {
          throw new AuthServiceError({ statusCode: 409, apiCode: 'EXPORT_NOT_READY', message: 'Only failed or canceled migration exports can be retried' })
        }
        if (current.retry_count >= MIGRATION_EXPORT_MAX_RETRIES) {
          throw new AuthServiceError({ statusCode: 409, apiCode: 'EXPORT_RETRY_EXHAUSTED', message: 'Migration export retry limit has been reached' })
        }
      ...
      if (staleArchiveKey) {
        await objectStorage.deleteObject(staleArchiveKey).catch(() => undefined)
      }
    ...
    async downloadExport(rawProjectId, rawExportId, actor) {
      ...
      assertProjectId(rawProjectId)
      const exportId = normalizeExportId(rawExportId)
      const row = await pool.query<ExportRow>(`SELECT ${EXPORT_COLUMNS} FROM migration_exports WHERE id = $1 AND workspace_id = $2 AND project_id = $3`, [exportId, actor.workspaceId, rawProjectId])
      const current = row.rows[0] ?? exportNotFound()
      if (new Date(current.expires_at).getTime() <= Date.now() && current.status !== 'completed') {
        throw new AuthServiceError({ statusCode: 410, apiCode: 'EXPORT_EXPIRED', message: 'Migration export has expired' })
      }
      if (current.status !== 'completed' || !current.archive_object_key) {
        throw new AuthServiceError({ statusCode: 409, apiCode: 'EXPORT_NOT_READY', message: 'Migration export is not ready for download' })
      }
      const signed = await objectStorage.createPresignedDownload({ objectKey: current.archive_object_key, expiresInSeconds: MIGRATION_EXPORT_DOWNLOAD_TTL_SECONDS })
    ...
    async processExport(exportId) {
      const row = await withTransaction(pool, async (client) => {
        const result = await client.query<ExportRow>(`SELECT ${EXPORT_COLUMNS} FROM migration_exports WHERE id = $1 FOR UPDATE`, [exportId])
        const current = result.rows[0]
        if (!current || ['completed', 'canceled', 'expired', 'failed', 'generating'].includes(current.status)) return null
        const updated = await client.query<ExportRow>(`UPDATE migration_exports SET status = 'generating', updated_at = now() WHERE id = $1 AND status = 'prepared' RETURNING ${EXPORT_COLUMNS}`, [exportId])
      ...
      try {
        ...
        if (row.checkpoint_json) jsonFiles.set('checkpoint.json', canonicalJson(row.checkpoint_json))
        const assetBodies = new Map<string, Uint8Array>()
        let completedFileCount = 1 + jsonFiles.size
        let completedBytes = canonicalJson(row.manifest_json).byteLength
          + [...jsonFiles.values()].reduce((total, body) => total + body.byteLength, 0)
        await withTransaction(pool, async (client) => {
          await client.query(`UPDATE migration_exports SET completed_file_count = $2, completed_bytes = $3, updated_at = now() WHERE id = $1`, [exportId, completedFileCount, completedBytes])
        })
        for (const asset of row.export_assets_json) {
          const cancel = (await pool.query<{ cancel_requested_at: Date | null; status: ExportRow['status'] }>(`SELECT cancel_requested_at, status FROM migration_exports WHERE id = $1`, [exportId])).rows[0]
          if (!cancel || cancel.cancel_requested_at || cancel.status === 'canceled') {
            throw new AuthServiceError({ statusCode: 409, apiCode: 'EXPORT_CANCELED', message: 'Migration export was canceled' })
          }
          const body = await objectStorage.getObjectBytes({ objectKey: asset.objectKey, maxBytes: MAX_EXPORT_OBJECT_BYTES })
          if (body.byteLength !== asset.byteSize || createHash('sha256').update(body).digest('hex') !== asset.sha256) {
            throw new AuthServiceError({ statusCode: 422, apiCode: 'ASSET_VALIDATION_FAILED', message: 'Project asset changed before export generation' })
          }
          assetBodies.set(asset.logicalAssetId, body)
          completedFileCount += 1
          completedBytes += body.byteLength
          await pool.query(`UPDATE migration_exports SET completed_file_count = $2, completed_bytes = $3, updated_at = now() WHERE id = $1 AND status = 'generating'`, [exportId, completedFileCount, completedBytes])
        }
        ...
        await objectStorage.putObject({ objectKey, mimeType: 'application/zip', body: built.body })
        const archiveSha256 = createHash('sha256').update(built.body).digest('hex')
        const final = await withTransaction(pool, async (client) => {
          const current = (await client.query<ExportRow>(`SELECT ${EXPORT_COLUMNS} FROM migration_exports WHERE id = $1 FOR UPDATE`, [exportId])).rows[0]
          if (!current || current.cancel_requested_at || current.status === 'canceled') return 'canceled' as const
          await client.query(`UPDATE migration_exports SET status = 'completed', completed_file_count = $2, completed_bytes = $3, archive_object_key = $4, archive_byte_size = $5, archive_sha256 = $6, completed_at = now(), updated_at = now() WHERE id = $1`, [exportId, row.file_count, Number(row.total_bytes), objectKey, built.body.byteLength, archiveSha256])
          return 'completed' as const
        })
        if (final === 'canceled') {
          await objectStorage.deleteObject(objectKey).catch(() => undefined)
          await pool.query(`UPDATE migration_exports SET status = 'canceled', canceled_at = COALESCE(canceled_at, now()), updated_at = now() WHERE id = $1 AND status = 'generating'`, [exportId])
        }
      } catch (error) {
        const canceled = error instanceof AuthServiceError && error.apiCode === 'EXPORT_CANCELED'
        if (generatedObjectKey) {
          await objectStorage.deleteObject(generatedObjectKey).catch(() => undefined)
        }
        await pool.query(
          canceled
          ...
          canceled ? [exportId] : [exportId, error instanceof AuthServiceError ? error.apiCode : 'EXPORT_GENERATION_FAILED', error instanceof AuthServiceError ? error.message : 'Migration export generation failed'],
        )
      }
    },
    ...
    async maintainExports(options = {}) {
      ...
      if (!Number.isInteger(graceHours) || graceHours < 1 || graceHours > 8760 || !Number.isInteger(batchSize) || batchSize < 1 || batchSize > 500) {
        throw new AuthServiceError({ statusCode: 400, apiCode: 'VALIDATION_FAILED', message: 'Invalid migration export maintenance options' })
      }
      await pool.query(`
        UPDATE migration_exports
      ...
      `)
      const stale = await pool.query<{ id: string; archive_object_key: string | null }>(
      ...
      for (const row of stale.rows) {
        if (row.archive_object_key) {
          await objectStorage.deleteObject(row.archive_object_key).catch(() => undefined)
        }
        await pool.query(
          `UPDATE migration_exports
    ...
    async recoverExports() {
      ...
      await pool.query(`UPDATE migration_exports SET status = 'prepared', updated_at = now() WHERE status = 'generating' AND expires_at > now()`)
      const result = await pool.query<{ id: string }>(`SELECT id::text FROM migration_exports WHERE status IN ('prepared', 'generating') AND expires_at > now() ORDER BY created_at, id LIMIT 20`)
