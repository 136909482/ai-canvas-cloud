...
class FakeMigrationObjectStorage {
  objects = new Map<string, { byteSize: number; mimeType: string; sha256: string }>()
  lastObjectKey: string | null = null
  multipartUploadIds: string[] = []
  completedMultipart: Array<{ uploadId: string; parts: Array<{ partNumber: number; etag: string }> }> = []
  deletedKeys: string[] = []

  async createPresignedUpload(input: { objectKey: string; mimeType: string; byteSize: number; expiresInSeconds: number }) {
  ...
  async createPresignedUploadPart(input: { objectKey: string; uploadId: string; partNumber: number; byteSize: number; expiresInSeconds: number }) {
    this.lastObjectKey = input.objectKey
    return {
  ...
  async completeMultipartUpload(input: { objectKey: string; uploadId: string; parts: Array<{ partNumber: number; etag: string }> }) {
...
test('migration asset uploads are resumable, validated, isolated, and reservation-safe', {
  ...
  try {
    ...
    `, [IMPORT_ID, WORKSPACE_A, small.byteLength + largeSize, JSON.stringify(assetManifest)])
    ...
    const smallPrepared = await service.prepareAssetUpload(IMPORT_ID, 'small-asset', actorA)
    assert.equal(smallPrepared.upload.mode, 'single')
    assert.equal(smallPrepared.upload.parts.length, 0)
    assert(smallPrepared.upload.directUpload?.url.includes('storage.invalid'))
    assert.equal('objectKey' in smallPrepared.upload, false)
    assert.equal((await readWorkspaceStorageUsage(pool, WORKSPACE_A)).storage.reservedBytes, small.byteLength)

    const smallKey = storage.lastObjectKey!
    storage.objects.set(smallKey, { byteSize: small.byteLength, mimeType: 'image/png', sha256: sha256(small) })
    const smallCompleted = await service.completeAssetUpload(IMPORT_ID, 'small-asset', undefined, actorA)
    assert.equal(smallCompleted.upload.status, 'completed')
    assert.deepEqual((await service.completeAssetUpload(IMPORT_ID, 'small-asset', {}, actorA)).upload.completedParts, [])

    const largePrepared = await service.prepareAssetUpload(IMPORT_ID, 'large-asset', actorA)
    ...
    await service.completeAssetPart(IMPORT_ID, 'large-asset', 1, { etag: 'etag-1', byteSize: 8 * 1024 * 1024 }, actorA)
    const resumed = await service.getAssetUpload(IMPORT_ID, 'large-asset', actorA)
    assert.deepEqual(resumed.upload.completedParts, [1])
    assert.equal(resumed.upload.parts.length, 1)
    await service.completeAssetPart(IMPORT_ID, 'large-asset', 2, { etag: 'etag-2', byteSize: 1 }, actorA)
    storage.objects.set(largeKey, { byteSize: largeSize, mimeType: 'video/mp4', sha256: 'c'.repeat(64) })
    await assert.rejects(
      () => service.completeAssetUpload(IMPORT_ID, 'large-asset', undefined, actorA),
      (error: unknown) => error instanceof AuthServiceError && error.apiCode === 'ASSET_VALIDATION_FAILED',
    )
    assert.equal((await service.getAssetUpload(IMPORT_ID, 'large-asset', actorA)).upload.status, 'failed')
    const retried = await service.prepareAssetUpload(IMPORT_ID, 'large-asset', actorA)
    ...
    await service.completeAssetPart(IMPORT_ID, 'large-asset', 1, { etag: 'etag-retry-1', byteSize: 8 * 1024 * 1024 }, actorA)
    await service.completeAssetPart(IMPORT_ID, 'large-asset', 2, { etag: 'etag-retry-2', byteSize: 1 }, actorA)
    storage.objects.set(retriedLargeKey, { byteSize: largeSize, mimeType: 'video/mp4', sha256: largeHash })
    const largeCompleted = await service.completeAssetUpload(IMPORT_ID, 'large-asset', undefined, actorA)
    assert.equal(largeCompleted.upload.status, 'completed')
    assert.equal(storage.completedMultipart.length, 2)
    assert.deepEqual((await pool.query(`SELECT status FROM migration_imports WHERE id = $1`, [IMPORT_ID])).rows[0], { status: 'ready' })
    assert.equal((await pool.query(`SELECT count(*)::integer AS count FROM assets`)).rows[0]?.count, 0)
    assert.equal((await readWorkspaceStorageUsage(pool, WORKSPACE_A)).storage.reservedBytes, small.byteLength + largeSize)
    const deletedBeforeCompletedCancel = [...storage.deletedKeys]
    assert.equal((await service.cancelAssetUpload(IMPORT_ID, 'small-asset', actorA)).upload.status, 'completed')
    ...
    const canceled = await service.cancelAssetUpload(cancelImportId, 'cancel-asset', actorA)
