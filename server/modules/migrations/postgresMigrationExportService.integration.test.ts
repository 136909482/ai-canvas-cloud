...
function readStoredZip(body: Uint8Array) {
  ...
  while (offset + 4 <= buffer.byteLength && buffer.readUInt32LE(offset) === 0x04034b50) {
    ...
    files.push({ path: name, body: buffer.subarray(bodyStart, bodyStart + compressedSize) })
...
async function waitForCompletion(service: ReturnType<typeof createPostgresMigrationExportService>, exportId: string, actor: { userId: string; workspaceId: string }) {
  for (let attempt = 0; attempt < 50; attempt += 1) {
    const response = await service.getExport(PROJECT_ID, exportId, actor)
    if (response.export.status === 'completed' || response.export.status === 'failed' || response.export.status === 'canceled') {
      return response
    }
...
test('migration export freezes one project version and produces an import-compatible private archive', {
  ...
  const storage = {
    ...
    async createPresignedDownload(input: { objectKey: string }) {
      return { url: `https://storage.test/download/${encodeURIComponent(input.objectKey)}`, expiresAt: new Date(Date.now() + 300_000).toISOString() }
    },
  ...
  try {
    ...
    const migrations = (await readdir(join(process.cwd(), 'server', 'db', 'migrations'))).filter((fileName) => fileName.endsWith('.sql') && !/^(?:002[5-9]|0030)_/.test(fileName)).sort()
    ...
    `, [PROJECT_ID, JSON.stringify({ imageAsset: { assetId: ASSET_ID, relativePath: `cloud-assets/${ASSET_ID}` } })])
    await pool.query(`
      INSERT INTO assets (
    ...
    `, [ASSET_ID, WORKSPACE_A, PROJECT_ID, objectKey, assetBytes.byteLength, sha256(assetBytes)])
    await pool.query(`
    ...
    const snapshotId = (await pool.query<{ id: string }>(`
    ...
    `, [PROJECT_ID, JSON.stringify({
      schemaVersion: 1,
      project: { id: PROJECT_ID, name: 'Export project', version: 1, lastSequence: 1 },
      canvas: { nodes: [{ id: 'node-1', nodeType: 'imageNode', position: { x: 0, y: 0 }, dataSchemaVersion: 1, data: { imageAsset: { assetId: ASSET_ID, relativePath: `cloud-assets/${ASSET_ID}` } } }], edges: [] },
      taskQueue: { tasks: [] },
    }), JSON.stringify([ASSET_ID])])).rows[0]!.id
    await pool.query(`UPDATE projects SET saved_snapshot_id = $2 WHERE id = $1`, [PROJECT_ID, snapshotId])

    const service = createPostgresMigrationExportService(pool, storage)
    const prepared = await service.prepareExport(PROJECT_ID, { idempotencyKey: 'export-1', expectedVersion: 1, expectedSequence: 1 }, owner)
    assert.equal(prepared.export.status, 'prepared')
    assert.equal(prepared.export.project.version, 1)
    const replay = await service.prepareExport(PROJECT_ID, { idempotencyKey: 'export-1', expectedVersion: 1, expectedSequence: 1 }, owner)
    assert.equal(replay.export.id, prepared.export.id)
    await assert.rejects(
      () => service.prepareExport(PROJECT_ID, { idempotencyKey: 'export-1', expectedVersion: 0, expectedSequence: 1 }, owner),
      (error: unknown) => error instanceof AuthServiceError && error.apiCode === 'EXPORT_CONFLICT',
    )
    await pool.query(`UPDATE projects SET version = 2, last_sequence = 2 WHERE id = $1`, [PROJECT_ID])
    const completed = await waitForCompletion(service, prepared.export.id, owner)
    assert.equal(completed.export.status, 'completed', JSON.stringify(completed))
    assert.equal(completed.export.project.version, 1)
    assert(completed.export.archive)
    assert.equal(completed.export.progress.completedFileCount, completed.export.progress.fileCount)
    const download = await service.downloadExport(PROJECT_ID, prepared.export.id, owner)
    assert.equal(download.exportId, prepared.export.id)
    assert.equal(JSON.stringify(download).includes(`workspaces/${WORKSPACE_A}/migration-exports`), false)
    ...
    const manifest = JSON.parse(byPath.get('manifest.json')!.toString('utf8'))
    const projectRecord = JSON.parse(byPath.get('project.json')!.toString('utf8'))
    const graph = JSON.parse(byPath.get('graph.json')!.toString('utf8'))
    const assetManifest = JSON.parse(byPath.get('assets.json')!.toString('utf8'))
    const checkpoint = byPath.has('checkpoint.json') ? JSON.parse(byPath.get('checkpoint.json')!.toString('utf8')) : null
    const descriptors = manifest.files.map((file: { path: string; byteSize: number; sha256: string }) => ({ ...file }))
    assert.equal(manifest.project.version, 1)
    assert.equal(graph.nodes[0].data.imageAsset.assetId.startsWith('asset-'), true)
    assert.equal(graph.nodes[0].data.imageAsset.relativePath.startsWith('assets/'), true)
    assert.equal(JSON.stringify(projectRecord).includes(objectKey), false)
    assert.equal(JSON.stringify(projectRecord).includes(ASSET_ID), false, JSON.stringify(projectRecord))
    validateMigrationPackageContract({
      manifest,
      projectRecord,
      graph,
    ...
    assert.equal(manifest.contentSha256, sha256(createMigrationPackageContentDigestInput(descriptors)))
    await assert.rejects(
      () => service.getExport(PROJECT_ID, prepared.export.id, { userId: 'export-owner-b', workspaceId: WORKSPACE_B }),
      (error: unknown) => error instanceof AuthServiceError && error.apiCode === 'RESOURCE_NOT_FOUND',
    )
    await assert.rejects(
      () => service.getExport('22222222-2222-4222-8222-222222222222', prepared.export.id, owner),
      (error: unknown) => error instanceof AuthServiceError && error.apiCode === 'RESOURCE_NOT_FOUND',
    )

    objects.set(objectKey, Buffer.from('changed'))
    const failedPrepared = await service.prepareExport(PROJECT_ID, { idempotencyKey: 'export-failure' }, owner)
    const failed = await waitForCompletion(service, failedPrepared.export.id, owner)
    assert.equal(failed.export.status, 'failed')
    objects.set(objectKey, assetBytes)
    const retried = await service.retryExport(PROJECT_ID, failedPrepared.export.id, owner)
    assert.equal(retried.export.status, 'prepared')
    assert.equal(retried.export.progress.retryCount, 1)
    const recovered = await waitForCompletion(service, failedPrepared.export.id, owner)
    assert.equal(recovered.export.status, 'completed')
    assert.equal(recovered.export.progress.retryCount, 1)
    assert.equal(Number((await pool.query(`SELECT version, last_sequence FROM projects WHERE id = $1`, [PROJECT_ID])).rows[0]?.version), 2)
  } finally {
    await pool?.end()
    if (admin.readyForQuery) await admin.query(`DROP SCHEMA IF EXISTS "${schemaName}" CASCADE`)
    await admin.end()
  }
})
