...
export const MIGRATION_IMPORT_WRITE_ROLES = ['owner', 'admin', 'editor'] as const
const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i

export interface MigrationImportService {
  prepareImport: (input: unknown, actor: ProjectActor) => Promise<MigrationImportResponse>
  getImport: (importId: string, actor: ProjectActor) => Promise<MigrationImportResponse>
  cancelImport: (importId: string, actor: ProjectActor) => Promise<MigrationImportResponse>
  commitImport: (importId: string, input: unknown, actor: ProjectActor) => Promise<MigrationImportCommitResponse>
}
