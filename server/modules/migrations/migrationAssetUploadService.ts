...
export const MIGRATION_ASSET_MULTIPART_THRESHOLD =
...
function uploadNotFound(): never {
  throw new AuthServiceError({
    ...
    message: "Migration asset upload not found",
  });
}
...
function calculatePartCount(expectedByteSize: number) {
  const partCount =
    expectedByteSize > MIGRATION_ASSET_MULTIPART_THRESHOLD
...
function createStagingObjectKey(
  workspaceId: string,
  importId: string,
...
function findManifestAsset(
  importRow: MigrationImportRow,
  logicalAssetId: string,
) {
  const asset = importRow.asset_manifest_json.assets.find(
    (candidate) => candidate.logicalAssetId === logicalAssetId,
  );
  if (!asset) {
    uploadNotFound();
  }
...
function uploadResponseBase(
  row: MigrationAssetUploadRow,
): MigrationImportAssetUploadResponse["upload"] {
  return {
    id: row.id,
    ...
    completedParts: row.completed_parts_json
...
async function withStagingUrls(
  ...
  const completed = new Set(
    row.completed_parts_json.map((part) => part.partNumber),
  );
...
async function assertObjectMatches(
  ...
  let metadata: Awaited<
    ReturnType<MigrationImportObjectStorage["getObjectMetadata"]>
  >;
  try {
  ...
  const expectedByteSize = toSafeInteger(
  ...
  if (actualMimeType !== row.expected_mime_type) {
    uploadInvalid(
      "Migration staging object MIME type does not match the manifest",
    );
  }
  ...
  if (actualSha256 !== row.expected_sha256) {
    uploadInvalid(
      "Migration staging object SHA-256 does not match the manifest",
    );
  }
}

function validatePartInput(
  input: CompleteMigrationImportAssetUploadRequest,
  row: MigrationAssetUploadRow,
) {
  const parts = input.parts ?? {};
  const parsed = (
    Object.entries(parts) as Array<[string, { etag: string; byteSize: number }]>
  ).map(([rawNumber, part]) => {
    const partNumber = Number(rawNumber);
    if (
      !Number.isSafeInteger(partNumber) ||
      partNumber < 1 ||
      partNumber > row.part_count
    ) {
    ...
    if (part.byteSize !== expectedByteSize) {
      validationError(
        `Part ${partNumber} byteSize does not match the upload plan`,
      );
    }
...
export interface MigrationAssetUploadService {
  ...
  maintainStagingObjects: (options?: {
    graceHours?: number;
    batchSize?: number;
  }) => Promise<number>;
}
...
export function createPostgresMigrationAssetUploadService(
  ...
  async function prepareOrGet(
    ...
    await withTransaction(pool, async (client) => {
      const importRow = await findImport(
        client,
        importId,
        actor.workspaceId,
        true,
      ...
      if (
        new Date(importRow.expires_at).getTime() <= Date.now() &&
        activeImportStatus(importRow.status)
      ) {
        await client.query(
          `UPDATE migration_imports SET status = 'expired', updated_at = now() WHERE id = $1`,
          [importId],
        );
      ...
      if (!row) {
        ...
        const uploadMode: MigrationImportAssetUploadMode =
          partCount > 1 ? "multipart" : "single";
        ...
        if (uploadMode === "multipart") {
          providerUploadId = (
            await objectStorage.initiateMultipartUpload({
              objectKey,
    ...
    const objectToCleanup = staleObject as unknown as {
      objectKey: string;
      providerUploadId: string | null;
    } | null;
    if (objectToCleanup) {
      ...
      await objectStorage
        .deleteObject(objectToCleanup.objectKey)
        .catch(() => undefined);
    }
  ...
  async function getUpload(
    ...
    const row = await withTransaction(pool, async (client) => {
      ...
      const current = await findUpload(
        client,
        importId,
        actor.workspaceId,
        logicalAssetId,
      ...
      if (
        current &&
        ["pending", "uploading", "validating"].includes(current.status) &&
  ...
  async function recordPart(
    ...
    const importId = normalizeMigrationImportId(rawImportId);
    const logicalAssetId = normalizeLogicalAssetId(rawLogicalAssetId);
    if (!Number.isSafeInteger(rawPartNumber) || rawPartNumber < 1)
      validationError("partNumber must be a positive safe integer");
    const input = validateCompleteMigrationImportAssetUploadRequest({
      parts: { [rawPartNumber]: rawInput },
    });
    ...
    await withTransaction(pool, async (client) => {
      ...
      if (row.upload_mode !== "multipart")
        validationError("Single-part uploads do not accept part completion");
      if (row.status !== "uploading" && row.status !== "pending")
        importConflict("Migration asset upload is not active");
      ...
      if (part.byteSize !== expectedByteSize)
        validationError("Part byteSize does not match the upload plan");
      const merged = mergePart(row.completed_parts_json, part);
      const uploadedByteSize = sumParts(merged);
      if (
        uploadedByteSize >
        toSafeInteger(row.expected_byte_size, "expectedByteSize")
      ) {
  ...
  async function completeUpload(
    ...
    await withTransaction(pool, async (client) => {
      ...
      if (row.upload_mode === "multipart") {
        if (
          parts.length !== row.part_count ||
          parts.some((part, index) => part.partNumber !== index + 1)
        ) {
          importConflict(
            "All multipart upload parts must be completed before finalization",
          );
        }
      }
      const result = await client.query<MigrationAssetUploadRow>(
    ...
    try {
      if (validatingRow.upload_mode === "multipart") {
        if (!validatingRow.provider_upload_id)
          throw new Error("Multipart provider upload ID is missing");
      ...
      await withTransaction(pool, async (client) => {
        await client.query(
          ...
          [
            ...
            "Migration staging object validation failed",
            validatingRow.id,
            actor.workspaceId,
          ],
        );
      ...
      if (
        validatingRow.upload_mode === "multipart" &&
        validatingRow.provider_upload_id
      ) {
        await objectStorage
          .abortMultipartUpload({
            objectKey: validatingRow.object_key,
      ...
      await objectStorage
        .deleteObject(validatingRow.object_key)
        .catch(() => undefined);
  ...
  async function cancelUpload(
    ...
    const result = await withTransaction(pool, async (client) => {
      ...
      if (importRow.status === "completed")
        importConflict(
          "Completed migration import cannot cancel asset uploads",
        );
      const current = await findUpload(
        client,
        importId,
  ...
  async function maintainStagingObjects(
    options: { graceHours?: number; batchSize?: number } = {},
  ) {
    ...
    if (
      ...
      throw new AuthServiceError({
        statusCode: 400,
        apiCode: "VALIDATION_FAILED",
        message: "Invalid migration staging maintenance options",
      });
    }
    await pool.query(`
