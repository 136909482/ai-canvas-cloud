...
const PROJECT_WRITE_ROLES: readonly WorkspaceRole[] = ['owner', 'admin', 'editor']
...
function normalizeListStatus(status: ProjectListStatus | undefined): ProjectListStatus {
...
function normalizeListLimit(limit: number | undefined) {
  ...
  if (!Number.isInteger(normalized) || normalized < 1 || normalized > PROJECT_LIST_MAX_LIMIT) {
    throw new AuthServiceError({
      statusCode: 400,
      apiCode: 'VALIDATION_FAILED',
...
function encodeCursor(project: ProjectSummary) {
  return Buffer.from(JSON.stringify({
    ...
    id: project.id,
  } satisfies ProjectCursor)).toString('base64url')
}
...
export function createPostgresProjectService(
  ...
  return {
    async listProjects(input, actor) {
      ...
      return {
        projects,
        nextCursor: hasNextPage && projects.length > 0 ? encodeCursor(projects[projects.length - 1]!) : null,
      }
    },
    ...
    async renameProject(projectId, input, actor) {
      await authorize(authorizationService, actor, PROJECT_WRITE_ROLES)
      const name = normalizeProjectName(input.name)
      const row = await updateProject(projectId, actor, 'name = $3, updated_at = now()', [name])
