...
function createAuthorizationService() {
  const calls: Parameters<WorkspaceAuthorizationService['requireWorkspaceAccess']>[0][] = []
...
test('project list uses opaque keyset cursors and never returns workspace ids', async () => {
  ...
  const rows = [
    projectRow(),
    projectRow({ id: secondProjectId, name: '第二个项目', updated_at: new Date('2026-07-14T00:00:00.000Z') }),
  ]
...
test('project reads hide projects outside the actor workspace', async () => {
  ...
  await assert.rejects(
    () => service.getProject(PROJECT_ID, { userId: 'user-a', workspaceId: 'workspace-a' }),
...
test('project metadata mutations and soft delete remain workspace-scoped', async () => {
  ...
  assert.deepEqual(await service.deleteProject(PROJECT_ID, actor), { ok: true })
...
test('project service rejects invalid names, ids, filters, and cursors', async () => {
  ...
  await assert.rejects(() => service.createProject({ name: '   ' }, actor), AuthServiceError)
  await assert.rejects(() => service.createProject({ id: 'not-a-uuid', name: '项目' }, actor), AuthServiceError)
  await assert.rejects(() => service.getProject('not-a-uuid', actor), AuthServiceError)
  await assert.rejects(
    () => service.listProjects({ status: 'deleted' as never }, actor),
    AuthServiceError,
  )
  await assert.rejects(() => service.listProjects({ cursor: 'invalid' }, actor), AuthServiceError)
})
