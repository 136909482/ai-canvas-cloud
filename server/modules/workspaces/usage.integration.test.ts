...
test('PostgreSQL workspace usage is status-aware and isolates two accounts', {
  ...
  try {
    ...
    const rows = [
      [WORKSPACE_A, 'usage-user-a', 100, 'completed'],
      [WORKSPACE_A, 'usage-user-a', 50, 'pending'],
      [WORKSPACE_A, 'usage-user-a', 25, 'failed'],
      [WORKSPACE_A, 'usage-user-a', 10, 'quarantined'],
      [WORKSPACE_A, 'usage-user-a', 999, 'deleted'],
      [WORKSPACE_B, 'usage-user-b', 1000, 'completed'],
    ] as const
    for (const [index, [workspaceId, userId, byteSize, status]] of rows.entries()) {
      await pool.query(`
        INSERT INTO assets (
          workspace_id, created_by_user_id, object_key, original_file_name,
          mime_type, byte_size, asset_kind, status, deleted_at
        ) VALUES (
          $1, $2, $3, 'usage.png', 'image/png', $4, 'upload', $5,
          CASE WHEN $5 = 'deleted' THEN now() ELSE NULL END
        )
      `, [workspaceId, userId, `workspaces/${workspaceId}/usage/${index}.png`, byteSize, status])
    }

    const service = createPostgresWorkspaceUsageService(pool)
    const usageA = await service.getCurrentUsage({ userId: 'usage-user-a', workspaceId: WORKSPACE_A })
    ...
    const usageB = await service.getCurrentUsage({ userId: 'usage-user-b', workspaceId: WORKSPACE_B })
    assert.equal(usageB.storage.usedBytes, 1000)
    assert.equal(usageB.storage.reservedBytes, 0)

    await assert.rejects(
      () => service.getCurrentUsage({ userId: 'usage-user-a', workspaceId: WORKSPACE_B }),
