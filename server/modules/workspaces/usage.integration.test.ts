...
test('PostgreSQL workspace usage is status-aware and isolates two accounts', {
  ...
  try {
    ...
    for (const [index, [workspaceId, projectId, userId, byteSize, status]] of rows.entries()) {
      await pool.query(`
        INSERT INTO assets (
      ...
      `, [workspaceId, projectId, userId, `workspaces/${workspaceId}/usage/${index}.png`, byteSize, status])
    }
    ...
    assert.deepEqual(usageA.projects.map((project) => ({
    ...
    assert.deepEqual(usageB.projects.map((project) => project.projectId), [PROJECT_B])
