...
test("workspace authorization rejects insufficient role and disabled workspace", async () => {
  const viewerService = createWorkspaceAuthorizationService(
    createMockPool([createAccessRow("viewer")]).pool as never,
  );
  ...
  const disabledService = createWorkspaceAuthorizationService(
    createMockPool([createAccessRow("owner", "disabled")]).pool as never,
  );
