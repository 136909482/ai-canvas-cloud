...
test('PostgreSQL owner admin editor viewer authorization matrix is explicit and non-disclosing', {
  ...
  try {
    ...
    for (const fileName of ['0001_schema_migrations.sql', '0002_auth_workspaces.sql']) {
    ...
    const scenarios = [
      { name: 'read', allowedRoles: undefined, allowed: ['owner', 'admin', 'editor', 'viewer'] },
      { name: 'content write', allowedRoles: ['owner', 'admin', 'editor'], allowed: ['owner', 'admin', 'editor'] },
      { name: 'administration', allowedRoles: ['owner', 'admin'], allowed: ['owner', 'admin'] },
    ...
    for (const scenario of scenarios) {
      for (const role of ['owner', 'admin', 'editor', 'viewer'] as const) {
        const operation = service.requireWorkspaceAccess({
          ...
          ...(scenario.allowedRoles ? { allowedRoles: [...scenario.allowedRoles] } : {}),
        })
    ...
    await assert.rejects(
      () => service.requireWorkspaceAccess({ userId: 'role-outsider', workspaceId: WORKSPACE_ID }),
      (error: unknown) => error instanceof AuthServiceError
