...
test('administrator user ban, unban, and session revocation are transactional and audited', async () => {
  ...
  const summary = () => ({
    id: 'user_01', user_no: '10001', name: 'Artist', email: 'artist@example.com',
    email_verified: true, status: state.status, workspace_count: '0', storage_used_bytes: '0',
    active_session_count: String(state.sessions.length), last_active_at: state.sessions.length ? now : null,
  ...
  const database = {
    async query(sql: string, values: unknown[] = []) {
      ...
      if (sql.includes('UPDATE public."user"') && sql.includes("status = 'disabled'")) {
        state.status = 'disabled'
        return { rows: [], rowCount: 1 }
      }
      if (sql.includes('UPDATE public."user"') && sql.includes("status = 'active'")) {
        state.status = 'active'
        return { rows: [], rowCount: 1 }
      }
      ...
      if (sql.includes('WITH workspace_usage AS')) return { rows: [summary()], rowCount: 1 }
      if (sql.includes('SELECT') && sql.includes('w.id::text')) return { rows: [], rowCount: 0 }
      throw new Error(`Unexpected administrator user operations SQL: ${sql.replace(/\s+/g, ' ').trim().slice(0, 120)}`)
    },
    async connect() {
  ...
  const adminSession = {
    admin: { id: 'admin-01', username: 'support', role: 'support' as const, status: 'active' as const },
  ...
  const context = { requestId: 'request-01', ipAddress: '192.0.2.10', userAgent: 'Test Agent' }

  const banned = await service.banUser('user_01', { reason: '  风险复核  ' }, context)
  assert.equal(banned.user.status, 'disabled')
  assert.equal(banned.revokedSessionCount, 2)
  state.sessions.push('late-session')
  const bannedAgain = await service.banUser('user_01', { reason: '处理迟到登录' }, context)
  assert.equal(bannedAgain.revokedSessionCount, 1)
  assert.deepEqual(state.sessions, [])

  const unbanned = await service.unbanUser('user_01', { reason: '身份复核通过' }, context)
  assert.equal(unbanned.user.status, 'active')
  state.sessions.push('session-3')
  const revoked = await service.revokeUserSessions('user_01', { reason: '用户请求退出全部设备' }, context)
  assert.equal(revoked.revokedSessionCount, 1)
  assert.deepEqual(state.sessions, [])

  assert.deepEqual(state.audits.map((values) => values[2]), [
    'user.ban', 'user.ban', 'user.unban', 'user.sessions.revoke',
  ])
  const serializedAudit = JSON.stringify(state.audits)
  assert.match(serializedAudit, /风险复核|处理迟到登录|身份复核通过|用户请求退出全部设备/)
  assert.doesNotMatch(serializedAudit, /artist@example\.com|late-session|session-3/)
...
test('administrator user queries keep two accounts isolated across filters, cursor pages, and details', async () => {
  ...
  const row = (id: 'user_A' | 'user_B') => ({
    ...
    created_at: id === 'user_A' ? '2026-07-23T03:00:00.000Z' : '2026-07-23T02:00:00.000Z',
    updated_at: '2026-07-23T04:00:00.000Z',
  })
  const database = {
    async query(sql: string, values: unknown[] = []) {
      queryCalls.push({ sql, values })
      if (sql.includes('WITH workspace_usage AS') && !sql.includes('WHERE u.id = $1')) {
        listCalls += 1
        return listCalls === 1
      ...
      if (sql.includes('w.id::text')) {
        ...
        return {
          rows: [{
            id: `workspace_${userId.at(-1)}`, name: `${userId} workspace`, type: 'personal', role: 'owner',
            status: 'active', plan_key: 'free', storage_quota_bytes: '1000',
            storage_used_bytes: userId === 'user_A' ? '111' : '222', storage_reserved_bytes: '0',
            created_at: '2026-07-20T00:00:00.000Z', updated_at: '2026-07-23T00:00:00.000Z',
          }],
          rowCount: 1,
        }
      }
  ...
  assert.deepEqual(first.items.map((user) => user.id), ['user_A'])
  ...
  assert.deepEqual(second.items.map((user) => user.id), ['user_B'])
  assert.equal(second.nextCursor, null)
  assert.deepEqual(queryCalls[1]!.values.slice(2), ['disabled', false, 'beta', 2])
  ...
  assert.doesNotMatch(JSON.stringify(alpha), /user_B|workspace_B|beta@example\.com/)
  assert.doesNotMatch(JSON.stringify(beta), /user_A|workspace_A|alpha@example\.com/)
  assert.deepEqual(queryCalls.slice(-4).map((call) => call.values), [
