...
test("administrator access requires active status and matching role permission", () => {
  ...
  for (const role of [
    ...
    assert.equal(
      hasAdminPermission(role, "dashboard.read"),
      true,
      `${role} dashboard.read`,
    );
  }
  for (const role of ["super_admin", "support"] as const) {
    assert.equal(
      hasAdminPermission(role, "user.read"),
      true,
      `${role} user.read`,
    );
    assert.equal(
      hasAdminPermission(role, "user.write"),
      true,
      `${role} user.write`,
    );
  }
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof AdminAccessError && error.code === "ADMIN_ACCESS_DENIED",
  );
  assert.throws(
...
test("administrator audit redaction drops user content and removes every credential shape", () => {
  const redacted = redactAdminAuditPayload({
    ...
    nested: {
      ...
      endpoint:
        "https://user:password@example.com/v1/generate?token=raw#fragment",
  ...
  assert.equal(
    (redacted.nested as Record<string, unknown>).endpoint,
    "https://example.com",
  );
  assert.equal("prompt" in (redacted.nested as Record<string, unknown>), false);
  assert.equal(
    "providerResponse" in (redacted.nested as Record<string, unknown>),
    false,
  );
  assert.doesNotMatch(
    serialized,
    /sk-should|raw-token|private user|private provider|still-secret/,
  );
});
