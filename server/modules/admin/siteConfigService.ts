...
function validateAssetRequest(input: CreateSiteAssetRequest) {
  if (!input || typeof input !== "object")
    throw validationError("Site asset request is invalid");
  if (input.kind !== "logo" && input.kind !== "favicon")
    throw validationError("Site asset kind is invalid");
  ...
  if (!ALLOWED_MIME_TYPES.has(mimeType))
    throw validationError("Site asset MIME type is invalid");
  if (
    !Number.isInteger(input.byteSize) ||
    input.byteSize < 1 ||
    input.byteSize > MAX_SITE_ASSET_BYTES
  ) {
    throw validationError("Site asset byte size is invalid");
  }
  const sha256 = input.sha256?.trim().toLowerCase();
  if (!SHA256_PATTERN.test(sha256))
    throw validationError("Site asset SHA-256 is invalid");
  if (
  ...
  if (!IDEMPOTENCY_PATTERN.test(idempotencyKey))
    throw validationError("Site asset idempotency key is invalid");
...
function validatePublishRequest(input: PublishSiteConfigRequest) {
  ...
  try {
    config = validateSiteConfigDocument(input?.config);
  } catch {
    throw validationError("Site configuration is invalid");
  }
  const note = input.note?.trim() || null;
  if (note && note.length > 500)
    throw validationError("Site configuration note is too long");
  return { config, note };
}
...
export function inspectSiteImage(bytes: Uint8Array, mimeType: string) {
  if (mimeType === "image/png") {
    ...
    if (
      ...
      throw validationError("PNG signature is invalid");
  ...
  if (mimeType === "image/jpeg") {
    if (bytes.length < 4 || bytes[0] !== 0xff || bytes[1] !== 0xd8)
      throw validationError("JPEG signature is invalid");
    let offset = 2;
  ...
  if (
    ...
    throw validationError("WebP signature is invalid");
...
function toAssetSummary(
  row: SiteAssetRow,
  signed?: { url: string; expiresAt: string } | null,
): SiteAssetSummary {
  return {
    id: row.id,
    kind: row.asset_kind,
  ...
  return {
    ...
    assets: {
      ...
      favicon: row
        ? await signedPublicAsset(
            storage,
            row.favicon_asset_id,
            row.favicon_object_key,
            row.favicon_mime_type,
          )
        : null,
    },
  };
...
async function resolveConfigAsset(
  client: DbClient,
  assetId: string | null,
  kind: "logo" | "favicon",
) {
  if (!assetId) return null;
  const row = await findAsset(
    client,
    validateUuid(assetId, `${kind}AssetId`),
    false,
  );
  if (!row || row.status !== "completed" || row.asset_kind !== kind)
    throw validationError(`${kind} asset is not ready`);
  return row;
}
  ...
  async function signedSummary(row: SiteAssetRow) {
    const signed =
      row.status === "completed"
        ? await options.objectStorage.createPresignedDownload({
  ...
  return {
    ...
    async publish(input, context) {
      ...
      await withTransaction(pool, async (client) => {
        const logo = await resolveConfigAsset(
          client,
          request.config.logoAssetId,
          "logo",
        );
        const favicon = await resolveConfigAsset(
          client,
          request.config.faviconAssetId,
          "favicon",
        );
        await client.query(
          `
        ...
        `,
          [
            ...
            JSON.stringify(request.config),
            logo?.id ?? null,
            logo?.object_key ?? null,
            logo?.mime_type ?? null,
            favicon?.id ?? null,
            favicon?.object_key ?? null,
            favicon?.mime_type ?? null,
          ],
        );
        ...
        if (existing.rows[0]) {
          if (existing.rows[0].request_fingerprint !== fingerprint)
            throw new AdminAccessError(
              409,
              "VALIDATION_FAILED",
              "Site asset idempotency key was reused",
            );
    ...
    async completeAsset(assetId, context) {
      ...
      if (!candidate)
        throw new AdminAccessError(
          404,
          "VALIDATION_FAILED",
          "Site asset was not found",
        );
      if (candidate.status === "completed")
        return { asset: await signedSummary(candidate) };
      if (
        candidate.status !== "pending" ||
        new Date(candidate.upload_expires_at).getTime() <= Date.now()
      ) {
        throw new AdminAccessError(
          409,
          "VALIDATION_FAILED",
          "Site asset upload is not pending",
        );
      }
      const metadata = await options.objectStorage.getObjectMetadata(
        candidate.object_key,
      );
      if (
        metadata.byteSize !== Number(candidate.byte_size) ||
        metadata.mimeType?.toLowerCase() !== candidate.mime_type
      ) {
        throw new AdminAccessError(
          422,
          "VALIDATION_FAILED",
          "Site asset metadata does not match the upload request",
        );
      }
      const bytes = await options.objectStorage.getObjectBytes({
        objectKey: candidate.object_key,
        maxBytes: MAX_SITE_ASSET_BYTES,
      });
      ...
      if (
        ...
        throw new AdminAccessError(
          422,
          "VALIDATION_FAILED",
          "Site asset content does not match the upload request",
        );
      }
      const completed = await withTransaction(pool, async (client) => {
        ...
        await insertAdminAuditEvent(
          client,
          {
            ...
            userAgent: context.userAgent,
            after: {
              kind: locked.asset_kind,
              mimeType: locked.mime_type,
              byteSize: Number(locked.byte_size),
            },
          },
          options.auditSecret,
        );
...
export function createUnavailableAdminSiteConfigService(): AdminSiteConfigService {
  const unavailable = async (): Promise<never> => {
    throw new AdminAccessError(
      503,
      "SERVICE_UNAVAILABLE",
      "Site configuration service is unavailable",
    );
  };
  return {
...
export function createUnavailablePublicSiteConfigService(): PublicSiteConfigService {
  return {
    async getCurrent() {
      return publicResponse({} as SiteAssetObjectStorage, null);
    },
  };
}
