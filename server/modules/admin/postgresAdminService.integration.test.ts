...
test(
  "real Admin Better Auth supports password login, optional one-time CAPTCHA, credential changes, bans, and session revocation",
  {
    skip:
      databaseUrl && testOptions
        ? false
        : "ADMIN_DATABASE_URL and ADMIN_BETTER_AUTH_SECRET are not configured",
  },
  async () => {
    ...
    const service = createPostgresAdminService(pool, {
      ...testOptions!,
      authApi,
    });
    const id = randomUUID();
    ...
    try {
      ...
      await pool.query(
        `UPDATE "user" SET role = 'super_admin', email_verified = true WHERE id = $1`,
        [userId],
      );
      ...
      let sessionCookie = loggedIn.setCookieHeaders
      ...
      const renamed = await service.updateUsername(
        { username: nextUsername },
        { requestId: `username-${id}`, cookieHeader: sessionCookie },
      );
      ...
      const changed = await service.changePassword(
        ...
        { requestId: `password-${id}`, cookieHeader: sessionCookie },
      );
      assert.equal(changed.response.admin.username, username);
      sessionCookie = changed.setCookieHeaders
        .map((value) => value.split(";")[0])
        .join("; ");
      password = nextPassword;

      stage = "enable_captcha";
      const enabled = await service.updateLoginSecuritySettings(
        { captchaEnabled: true },
        { requestId: `captcha-on-${id}`, cookieHeader: sessionCookie },
      );
      ...
      assert.match(
        generated.challenge?.imageDataUrl ?? "",
          ...
          service.login(
            {
              username,
              password,
              captchaChallengeId: generated.challenge!.id,
              captchaCode: "00000",
            },
            { requestId: `captcha-bad-${id}` },
          ),
        (error) =>
    ...
    `,
        [
          challengeId,
          hashAdminCaptchaCode(testOptions!.secret, challengeId, challengeCode),
        ],
      );
      stage = "login_with_captcha";
      const captchaLogin = await service.login(
        {
          username,
          password,
          captchaChallengeId: challengeId,
          captchaCode: challengeCode,
        },
        { requestId: `captcha-login-${id}` },
      );
      assert.equal(captchaLogin.response.state, "authenticated");
      assert.equal(
        (
          await pool.query(
            "SELECT consumed_at IS NOT NULL AS consumed FROM login_captcha_challenges WHERE id = $1",
            [challengeId],
          )
        ).rows[0]?.consumed,
        true,
      ...
      sessionCookie = captchaLogin.setCookieHeaders
        .map((value) => value.split(";")[0])
        .join("; ");
      stage = "disable_captcha";
      const disabled = await service.updateLoginSecuritySettings(
        { captchaEnabled: false },
        { requestId: `captcha-off-${id}`, cookieHeader: sessionCookie },
      );
      ...
      assert.equal(
        (
          await pool.query(
            `SELECT count(*)::int AS count FROM audit_events WHERE action = 'admin.security.captcha_updated' AND admin_user_id = $1`,
            [userId],
          )
        ).rows[0]?.count >= 2,
        true,
      ...
      await assert.rejects(
        ...
        (error) =>
          error instanceof AdminAccessError && error.code === "AUTH_REQUIRED",
      );

      await pool.query(`UPDATE "user" SET status = 'banned' WHERE id = $1`, [
        userId,
      ]);
      await assert.rejects(
        () =>
      ...
      throw new Error(`Admin login integration failed at ${stage}`, {
        cause: error,
      });
    } finally {
      await pool
      ...
      if (userId)
        await pool
          .query(`UPDATE "user" SET status = 'active' WHERE id = $1`, [userId])
          .catch(() => undefined);
      if (userId)
        await pool
...
test(
  "Admin runtime role appends redacted audit events but cannot update or delete them",
  {
    skip:
      databaseUrl && testOptions
        ? false
  ...
  async () => {
    ...
    const service = createPostgresAdminService(client as unknown as DbPool, {
      ...testOptions!,
      authApi: unavailableAuth,
    });
    try {
    ...
    `,
      ...
      const serialized = JSON.stringify([
        stored.rows[0]?.before_json,
        stored.rows[0]?.after_json,
      ]);
      assert.doesNotMatch(
        serialized,
        /sk-before-secret|private prompt|after-secret|token=secret/,
      );
