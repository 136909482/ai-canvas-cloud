...
export interface AdminUserOperationsService {
  listUsers(query: unknown, context: AdminRequestContext): Promise<AdminUsersResponse>
  getUser(userId: unknown, context: AdminRequestContext): Promise<AdminUserResponse>
  banUser(userId: unknown, input: unknown, context: AdminRequestContext): Promise<AdminUserStatusActionResponse>
  unbanUser(userId: unknown, input: unknown, context: AdminRequestContext): Promise<AdminUserStatusActionResponse>
  revokeUserSessions(userId: unknown, input: unknown, context: AdminRequestContext): Promise<AdminUserSessionRevocationResponse>
}
...
function toIso(value: Date | string) {
  ...
  if (!Number.isFinite(parsed.getTime())) throw new Error('Administrator user timestamp is invalid')
...
function toUserSummary(row: AdminUserListRow): AdminManagedUserSummary {
  return {
    ...
    activeSessionCount: toSafeInteger(row.active_session_count, 'activeSessionCount'),
...
function toWorkspaceSummary(row: AdminWorkspaceRow): AdminManagedWorkspaceSummary {
  return {
    id: row.id,
    name: row.name,
    type: row.type,
    ...
    storageQuotaBytes: toSafeInteger(row.storage_quota_bytes, 'storageQuotaBytes'),
    storageUsedBytes: toSafeInteger(row.storage_used_bytes, 'storageUsedBytes'),
    storageReservedBytes: toSafeInteger(row.storage_reserved_bytes, 'storageReservedBytes'),
    createdAt: toIso(row.created_at),
    updatedAt: toIso(row.updated_at),
  }
}
...
function normalizeUserId(value: unknown) {
  try {
    ...
    throw validationError(error instanceof Error ? error.message : 'userId is invalid')
  }
}
...
function normalizeActionRequest(value: unknown) {
  try {
    ...
    throw validationError(error instanceof Error ? error.message : 'User action request is invalid')
  }
}

function encodeCursor(user: AdminManagedUserSummary) {
  return Buffer.from(JSON.stringify({ createdAt: user.createdAt, id: user.id } satisfies AdminUserCursor)).toString('base64url')
}

function decodeCursor(value: string): AdminUserCursor {
  try {
    const parsed = JSON.parse(Buffer.from(value, 'base64url').toString('utf8')) as Partial<AdminUserCursor>
    if (!parsed || Object.keys(parsed).sort().join(',') !== 'createdAt,id') throw new Error()
    if (typeof parsed.createdAt !== 'string' || !Number.isFinite(new Date(parsed.createdAt).getTime())) throw new Error()
    return { createdAt: new Date(parsed.createdAt).toISOString(), id: validateAdminManagedUserId(parsed.id) }
  } catch {
  ...
  const service: AdminUserOperationsService = {
    async listUsers(query, context) {
      ...
      try {
        ...
        throw validationError(error instanceof Error ? error.message : 'User list query is invalid')
      }
      ...
      if (validated.cursor) {
        ...
        clauses.push(`(u.created_at, u.id) < ($${values.length - 1}::timestamptz, $${values.length}::text)`)
      }
      if (validated.status) {
    ...
    async getUser(userId, context) {
      ...
      if (!userRow) throw new AdminAccessError(404, 'RESOURCE_NOT_FOUND', 'User was not found')
    ...
    async banUser(userId, input, context) {
      const session = await options.adminService.requirePermission(context, 'user.write')
      const normalizedUserId = normalizeUserId(userId)
      const request = normalizeActionRequest(input)
      const revokedSessionCount = await withTransaction(pool, async (client) => {
        const locked = await client.query<{ status: 'active' | 'disabled' | 'deleted' }>(`
          SELECT status
          FROM public."user"
          WHERE id = $1
          FOR UPDATE
        ...
        if (!current) throw new AdminAccessError(404, 'RESOURCE_NOT_FOUND', 'User was not found')
        if (current.status === 'deleted') {
          throw new AdminAccessError(409, 'VALIDATION_FAILED', 'Deleted users cannot be changed')
        }
        ...
        await insertAdminAuditEvent(client, {
          ...
          after: { status: 'disabled', reason: request.reason, revokedSessionCount: count },
        }, options.auditSecret)
        return count
      })
