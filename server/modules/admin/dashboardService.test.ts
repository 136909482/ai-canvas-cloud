...
function adminService() {
  return {
    async requirePermission(_context: unknown, permission: string) {
      ...
      return {
        admin: { id: 'admin-01', username: 'auditor', role: 'auditor' as const, status: 'active' as const },
...
test('administrator dashboard returns bounded aggregate metrics and dependency health', async () => {
  ...
  const service = createPostgresAdminDashboardService(pool, {
    ...
    async readInfrastructureHealth() {
      return {
        ...
        objectStorage: { ok: false, latencyMs: 11, error: 'bucket_unavailable' as const },
      }
    },
  })
  ...
  assert.deepEqual(result.registrations, { total: 120, past24Hours: 4, past7Days: 23 })
  assert.deepEqual(result.activity, { activeUsers24Hours: 19, activeUsers7Days: 58, activeSessions: 21 })
  assert.deepEqual(result.storage, { usedBytes: 4096, reservedBytes: 768, quotaBytes: 32768, assetCount: 42 })
  assert.deepEqual(result.authentication, { verifiedUsers: 101, unverifiedUsers: 18, disabledUsers: 3 })
  ...
  assert.doesNotMatch(aggregateSql, /object_key|data_json|presentation_json|\bs\.token\b/i)
})

test('administrator dashboard collapses infrastructure reader failures to unknown categories', async () => {
  const pool = {
    async query() { return { rows: [aggregateRow], rowCount: 1 } },
  } as unknown as DbPool
  const service = createPostgresAdminDashboardService(pool, {
    adminService: adminService(),
    async readInfrastructureHealth() { throw new Error('sensitive infrastructure failure') },
  })

  const result = await service.getDashboard({ requestId: 'dashboard-health-failure' })
  assert.deepEqual(result.infrastructure, {
    postgres: { ok: false, latencyMs: 0, error: 'unknown' },
    objectStorage: { ok: false, latencyMs: 0, error: 'unknown' },
  })
  assert.doesNotMatch(JSON.stringify(result), /sensitive infrastructure failure/)
})
