...
function normalizeUsername(value: string) {
  ...
  if (!/^[a-z0-9_.]{3,30}$/.test(normalized)) {
    throw new AdminAccessError(
      400,
      "VALIDATION_FAILED",
      "Administrator username must be 3 to 30 letters, numbers, underscores, or dots",
    );
  }
  return normalized;
}
...
function validateNewPassword(value: string) {
  if (value.length < 12 || value.length > 256) {
    throw new AdminAccessError(
      400,
      "VALIDATION_FAILED",
      "Administrator password must be 12 to 256 characters",
    );
  }
}
...
function validateBootstrapPassword(value: string, environment: string) {
  if (environment === "development" && value.length >= 5 && value.length <= 256)
    return;
  ...
  if (error instanceof APIError) {
    ...
    if (error.statusCode >= 400 && error.statusCode < 500) {
      return new AdminAccessError(
        400,
        "VALIDATION_FAILED",
        "Administrator authentication request was rejected",
      );
    }
  }
  return new AdminAccessError(
    503,
    "SERVICE_UNAVAILABLE",
    "Administrator authentication service failed",
  );
}

export function hashAdminCaptchaCode(
  secret: string,
  challengeId: string,
  code: string,
) {
    ...
    for (let row = 0; row < rows.length; row += 1) {
      for (let column = 0; column < rows[row]!.length; column += 1) {
        if (rows[row]![column] === "1") {
          pixels.push(
            `<rect x="${originX + column * 5}" y="${10 + row * 6}" width="4" height="5" rx="1"/>`,
          );
        }
      }
    }
    shapes.push(
      `<g fill="${colors[randomInt(0, colors.length)]}" transform="rotate(${angle} ${originX + 12} 31)">${pixels.join("")}</g>`,
    );
  }
...
async function readLoginSecuritySetting(
  ...
  if (!row)
    throw new AdminAccessError(
      503,
      "SERVICE_UNAVAILABLE",
      "Administrator login security settings are unavailable",
    );
  return row;
}
...
async function verifyLoginCaptcha(
  ...
  const valid = await withTransaction(pool, async (client) => {
    ...
    const matches = safeTokenEqual(
      row.code_hash,
      hashAdminCaptchaCode(secret, challengeId, code),
    );
    if (matches) {
      await client.query(
        "UPDATE login_captcha_challenges SET consumed_at = now() WHERE id = $1",
        [challengeId],
      );
      return true;
    }
  ...
  if (!valid)
    throw new AdminAccessError(
      400,
      "VALIDATION_FAILED",
      "Administrator CAPTCHA is invalid or expired",
    );
}
...
export function createAdminBetterAuthApi(
  pool: DbPool,
...
async function requireRawSession(
  pool: DbPool,
  authApi: AdminBetterAuthApi,
...
function encodeCursor(row: Pick<AdminAuditEvent, "createdAt" | "id">) {
  return Buffer.from(JSON.stringify([row.createdAt, row.id])).toString(
    "base64url",
  );
}
...
function decodeCursor(cursor: string | undefined) {
  ...
  try {
    ...
    if (Number.isNaN(date.getTime()) || !/^[0-9a-f-]{36}$/i.test(value[1]))
      throw new Error();
...
function validateAuditQuery(query: AdminAuditQuery) {
  ...
  if (query.action && !/^[a-z0-9_.:-]{1,96}$/i.test(query.action)) {
    throw new AdminAccessError(
      400,
      "VALIDATION_FAILED",
      "Audit action filter is invalid",
    );
  }
  return { limit, cursor: decodeCursor(query.cursor) };
}

export function createPostgresAdminService(
  pool: DbPool,
  options: PostgresAdminServiceOptions,
): AdminService {
  ...
  const service: AdminService = {
    async createLoginCaptcha() {
      ...
      return withTransaction(pool, async (client) => {
        ...
        return {
          ...
          challenge: {
            id,
            imageDataUrl: renderCaptchaSvg(code),
    ...
    async login(input, context) {
      try {
        ...
        if (!session)
          throw new Error("Created administrator session was not found");
        return {
    ...
    async getLoginSecuritySettings(context) {
      ...
      return {
        captchaEnabled: setting.captcha_enabled,
        updatedAt: new Date(setting.updated_at).toISOString(),
      };
    },
    ...
    async updateLoginSecuritySettings(input, context) {
      if (typeof input.captchaEnabled !== "boolean") {
        throw new AdminAccessError(
          400,
          "VALIDATION_FAILED",
          "captchaEnabled must be a boolean",
        );
      }
      const session = await service.requirePermission(
        context,
        "security.write",
      );
      return withTransaction(pool, async (client) => {
    ...
    async logout(context) {
      ...
      const clearHeaders = [
        ...
        ["", "__Secure-"].map(
          (prefix) =>
            `${prefix}${COOKIE_PREFIX}.${name}=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0${secureAttribute}`,
        ),
      );
      try {
        ...
        return {
          ...
          setCookieHeaders: [
            ...getSetCookieHeaders(result.headers),
            ...clearHeaders,
          ],
        };
      } catch {
    ...
    async appendAuditEvent(input) {
      try {
        ...
        if (
          ...
          error.message === "Audit event identifiers are invalid"
        ) {
    ...
    async updateUsername(input, context) {
      try {
        ...
        return {
          ...session,
          admin: { ...session.admin, username: usernameValue },
        };
      } catch (error) {
        if (
          error &&
    ...
    async changePassword(input, context) {
      try {
        ...
        if (!result.response.token)
          throw new Error(
            "Password change did not rotate the administrator session",
          );
        const updatedSession = await getSessionByToken(
          pool,
          result.response.token,
        );
        if (!updatedSession)
          throw new Error("Rotated administrator session was not found");
...
export async function bootstrapFirstSuperAdmin(
  ...
  try {
    ...
    await pool.query('DELETE FROM "session" WHERE user_id = $1', [
      result.response.user.id,
    ]);
    ...
    await lockClient
      .query("SELECT pg_advisory_unlock($1)", [8_002_500_001])
      .catch(() => undefined);
    lockClient.release();
  }
}
