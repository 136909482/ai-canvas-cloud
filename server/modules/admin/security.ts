...
const CONTENT_KEY_PATTERN = /(?:prompt|content|body|response|completion|message|project|node|asset)/i
const CREDENTIAL_VALUE_PATTERN = /(?:bearer\s+[a-z0-9._~+/=-]+|(?:api[_-]?key|token|password|secret)\s*[:=]\s*\S+)/gi
...
function redactString(value: string) {
  const withoutCredentials = value.replace(CREDENTIAL_VALUE_PATTERN, '[REDACTED]')
  try {
...
function redactValue(value: unknown, depth: number): unknown {
  ...
  if (Array.isArray(value)) return value.slice(0, MAX_AUDIT_KEYS).map((item) => redactValue(item, depth + 1))
  if (typeof value !== 'object') return String(value).slice(0, MAX_AUDIT_STRING)

  const output: Record<string, unknown> = {}
  for (const [key, item] of Object.entries(value).slice(0, MAX_AUDIT_KEYS)) {
    if (CONTENT_KEY_PATTERN.test(key)) continue
    output[key] = SECRET_KEY_PATTERN.test(key) ? '[REDACTED]' : redactValue(item, depth + 1)
  }
  return output
}

export function redactAdminAuditPayload(value: unknown): Record<string, unknown> {
...
export function hashAdminRequestIdentity(value: string | undefined, pepper: string) {
  const normalized = value?.trim()
  if (!normalized) return null
  return createHash('sha256').update(pepper).update('\0').update(normalized).digest('hex')
}
