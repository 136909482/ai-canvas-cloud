...
export interface AdminService {
  ...
  ): Promise<AdminAuthResult<AdminLoginResponse>>;
  createLoginCaptcha(): Promise<{
    enabled: boolean;
    challenge: { id: string; imageDataUrl: string; expiresAt: string } | null;
  }>;
  getLoginSecuritySettings(
    context: AdminRequestContext,
  ): Promise<{ captchaEnabled: boolean; updatedAt: string }>;
  updateLoginSecuritySettings(
    input: { captchaEnabled: boolean },
    context: AdminRequestContext,
  ): Promise<{ captchaEnabled: boolean; updatedAt: string }>;
  ...
  changePassword(
    ...
    context: AdminRequestContext,
  ): Promise<AdminAuthResult<AdminSession>>;
  logout(
    context: AdminRequestContext,
  ): Promise<AdminAuthResult<{ success: true }>>;
  requirePermission(
    context: AdminRequestContext,
