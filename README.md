...
平台不提供官方模型、积分、计费、Provider 代理或服务器代生成。用户 Provider、endpoint、模型 ID 和 API Key 只允许保存在当前浏览器的本地加密 Vault；生成结果通过既有资产上传和项目图 API 写入云端。P8-5 Vault 仍未完成，现有代码只作为后续实现草稿，不对外暴露空壳设置入口。
...
`db:roles:provision` 从 migration 连接创建相互隔离的普通 API 与 Admin API 角色，并将随机本机凭据写入未跟踪 `.env`，终端不打印凭据。它同时删除旧 Worker 角色和失效环境键。`db:roles:check` 验证角色非超级用户、Admin/public 权限隔离、P8-4 旧表已删除且旧 Worker 角色不存在。
...
Admin API 提供独立的 live/ready 路径。readiness 对 PostgreSQL 执行查询、对 Redis 执行 `PING`、对对象存储执行 `HeadBucket`；失败只返回稳定脱敏分类。平台 API 不接收用户 Provider Key、endpoint、真实模型 ID 或任意 target URL，日志、指标、错误、前端 bundle 和 Git 均不得包含密码、Token、对象存储密钥或 Provider Key。
