...
function parseCookies(header: string | undefined) {
  ...
  for (const item of (header ?? "").split(";")) {
    ...
    if (index > 0)
      output.set(item.slice(0, index).trim(), item.slice(index + 1).trim());
  }
  return output;
}
...
export function createCsrfCookie(
  token: string,
  config: Pick<AdminApiConfig, "env">,
) {
...
export function verifyCsrf(
  ...
  if (
    ...
    issuedAt > current + 30 ||
    current - issuedAt > CSRF_MAX_AGE_SECONDS
  )
    return false;
  return safeTokenEqual(
    suppliedSignature,
    signature(`${timestamp}.${nonce}`, secret),
  );
}
  ...
  if (!SAFE_METHODS.has(request.method ?? "GET")) {
    if (
      ...
      sendDenied(
        response,
        requestId,
        "Administrator writes require an allowed Origin",
      );
      return true;
    }
