...
test("Admin security boundary rejects cross Origin and forged CSRF writes", () => {
  ...
  assert.equal(
    handleAdminSecurityBoundary(
      requestDouble({ headers: { origin: "https://evil.example" } }),
      crossOrigin as unknown as http.ServerResponse,
      config,
      "request-1",
    ),
    true,
  ...
  assert.equal(
    handleAdminSecurityBoundary(
      ...
      forged as unknown as http.ServerResponse,
      config,
      "request-2",
    ),
    true,
...
test("Admin security boundary accepts only a signed, unexpired double-submit token", () => {
  ...
  assert.equal(
    handleAdminSecurityBoundary(
      request,
      responseDouble() as unknown as http.ServerResponse,
      config,
      "request-3",
    ),
    false,
