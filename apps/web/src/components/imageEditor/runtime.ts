...
export function getTextAnnotationMetrics(
  annotation: Pick<TextAnnotation, "value" | "fontSize">,
) {
  ...
  return {
    width: Math.max(
      MIN_TEXT_BOX_WIDTH,
      Math.ceil(measuredWidth + TEXT_BOX_PADDING_X * 2),
    ),
    height: Math.max(
      MIN_TEXT_BOX_HEIGHT,
      lines.length * lineHeight + TEXT_BOX_PADDING_Y * 2,
    ),
    lines,
  };
}
...
export function drawTextAnnotation(
  ...
  lines.forEach((line, index) => {
    context.fillText(
      line || " ",
      TEXT_BOX_PADDING_X,
      TEXT_BOX_PADDING_Y + index * lineHeight,
    );
  });
  context.restore();
}
