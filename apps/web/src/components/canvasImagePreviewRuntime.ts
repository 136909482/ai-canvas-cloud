...
export function cacheThumbnail(
  src: string,
  thumbnailSrc: string,
  revokeOnEvict = thumbnailSrc !== src && thumbnailSrc.startsWith("blob:"),
) {
...
function createThumbnail(src: string, signal: AbortSignal) {
  return new Promise<string>((resolve, reject) => {
    ...
    image.onload = () => {
      try {
        ...
        const scale = Math.min(
          1,
          LOW_QUALITY_PREVIEW_MAX_SIZE / Math.max(width, height),
        );
        if (scale >= 1) {
          finishResolve(src);
          return;
        ...
        void canvasToThumbnailUrl(canvas, signal)
          .then(finishResolve)
          .catch(finishReject);
      } catch (error) {
    ...
    image.onerror = () =>
      finishReject(new Error("Failed to create low quality preview"));
    image.src = src;
  });
}
