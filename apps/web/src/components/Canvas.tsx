...
import {
  getNodeConnectionInputs,
  getNodeConnectionOutput,
  getQuickCreateTargetHandle,
  type NodeConnectionKind,
...
const VISIBLE_ELEMENT_CULLING_OVERRIDE_STORAGE_KEY =
  "ai-canvas.visibleElementCulling";
...
export function Canvas() {
  ...
  const imagePerformanceStats = useMemo(
    () => getCanvasImagePerformanceStats(nodes),
    [nodes],
  );
  ...
  const shouldFitView =
    !shouldUsePerformanceRendering && nodes.length <= LARGE_CANVAS_NODE_LIMIT;
  const shouldShowMiniMap =
    !shouldUseLiteRendering && nodes.length <= LARGE_CANVAS_NODE_LIMIT;
  const shouldShowBackground = canvasGridEnabled && !shouldUseLiteRendering;
  const shouldShowAlignmentGuides =
    alignmentGuidesEnabled && !shouldUseLiteRendering;
  const visibleElementCullingOverride =
    typeof window !== "undefined"
      ? window.localStorage.getItem(
          VISIBLE_ELEMENT_CULLING_OVERRIDE_STORAGE_KEY,
        )
      : null;
  const shouldStabilizeViewportElements =
    visibleElementCullingOverride !== "on" &&
    nodes.length <= IMAGE_HEAVY_CULLING_DISABLE_NODE_LIMIT &&
    imagePerformanceStats.imageNodeCount >=
      IMAGE_HEAVY_CULLING_DISABLE_IMAGE_LIMIT;
  const shouldCullOffscreenElements =
    visibleElementCullingOverride === "on"
      ? true
      : visibleElementCullingOverride === "off"
        ? false
        : true;
  const internalDragEnabled =
    typeof window !== "undefined" &&
    window.localStorage.getItem(INTERNAL_DRAG_ENABLE_STORAGE_KEY) === "1";
  const shouldUseInternalDrag =
    shouldUsePerformanceRendering &&
  ...
  const [quickCreateMenu, setQuickCreateMenu] =
    useState<QuickCreateMenuState | null>(null);
  ...
  const handleConnect = useCallback(
    (connection: Connection) => {
      ...
      const isReferenceImageSource = isImageSourceNodeType(sourceNode?.type);

      if (targetNode?.type === "compareNode") {
        if (
          !isReferenceImageSource ||
          !connection.targetHandle ||
          !["image1", "image2"].includes(connection.targetHandle)
        ) {
          return;
        }
      }
      ...
      if (targetNode?.type === "videoGenerateNode") {
        ...
        if (
          (connection.targetHandle === "image" ||
            connection.targetHandle === "firstFrame" ||
            connection.targetHandle === "lastFrame") &&
          !isReferenceImageSource
        ) {
          return;
        }
      ...
      if (targetNode?.type === "generateNode" && isReferenceImageSource) {
        const currentReferenceCount = edges.filter(
          (edge) =>
      ...
      if (targetNode?.type === "imageEditNode") {
        if (
          !isReferenceImageSource ||
          !connection.targetHandle ||
          !["base", "reference"].includes(connection.targetHandle)
        ) {
          return;
        }
        ...
        if (connection.targetHandle === "reference") {
          ...
          if (currentReferenceCount >= MAX_GENERATE_REFERENCE_IMAGES - 1) {
            notify({
              tone: "warning",
              title: "参考图已达上限",
              message: UI_TEXT.maxImageEditReferences,
            });
            return;
          }
        }
  ...
  const handleConnectEnd = useCallback(
    (
      ...
      const pendingNode = nodes.find(
        (node) => node.id === pendingConnection.nodeId,
      );
  ...
  const quickCreateActions = useMemo<QuickCreateAction[]>(() => {
    ...
    const pendingNode = nodes.find(
      (node) => node.id === quickCreateMenu.nodeId,
    );
    if (!pendingNode) {
      return [];
    }
    ...
    if (quickCreateMenu.handleType === "source") {
      ...
      const actions: Array<
        QuickCreateAction & { sourceKinds: NodeConnectionKind[] }
      > = [
        {
          id: "generate",
          label: "AI 绘图",
      ...
      return actions.filter((action) =>
        action.sourceKinds.includes(sourceKind),
      );
    }

    const targetKinds = getNodeConnectionInputs(
      pendingNode.type,
      quickCreateMenu.handleId,
    );
    ...
    const sourceActions: Array<
      QuickCreateAction & { outputKind: NodeConnectionKind }
    > = [
      {
        id: "text",
        label: "文本节点",
    ...
    return sourceActions.filter((action) =>
      targetKinds.includes(action.outputKind),
    );
  }, [
    addCompareNode,
    addGenerateNode,
  ...
  const quickCreateMenuPosition = useMemo(() => {
    ...
    return getQuickCreateMenuPosition(
      quickCreateMenu.clientX,
      quickCreateMenu.clientY,
    );
  }, [quickCreateMenu]);
  ...
  const handleQuickCreate = useCallback(
    (action: QuickCreateAction) => {
      ...
      const pendingNodeExists = nodes.some(
        (node) => node.id === quickCreateMenu.nodeId,
      );
      ...
      const preferredPosition = screenToFlowPosition({
        x:
          quickCreateMenu.clientX +
          (quickCreateMenu.handleType === "target"
            ? QUICK_CREATE_PREPEND_NODE_OFFSET_X
            : QUICK_CREATE_APPEND_NODE_OFFSET_X),
  ...
  const handleCanvasContextCreate = useCallback(
    (tool: CanvasNodeTool) => {
      if (!canvasContextMenu) {
        return;
      }
  ...
  const handleCanvasDragOver = useCallback(
    (event: DragEvent<HTMLDivElement>) => {
      if (
        !isCanvasEmptyDropTarget(event.target) ||
        !hasImageFileTransfer(event.dataTransfer)
      ) {
        return;
      }
    ...
    (event: DragEvent<HTMLDivElement>) => {
      ...
      if (!file.type.startsWith("image/")) {
        notify({
          tone: "warning",
          title: "无法导入文件",
          message: UI_TEXT.invalidDroppedImage,
        });
        return;
      }
  ...
  const miniMapNodeColor = isLightTheme
    ? "rgba(82,82,91,0.42)"
    : "rgba(113,113,122,0.58)";
  const miniMapSelectedNodeColor = isLightTheme
    ? "rgba(14,165,233,0.72)"
    : "rgba(196,181,253,0.82)";
  const miniMapMaskColor = isLightTheme
    ? "rgba(24, 24, 27, 0.08)"
    ...
    >
      ...
      quickCreateMenuPosition ? (
        <div
          ...
          style={{
            left: quickCreateMenuPosition.left,
            top: quickCreateMenuPosition.top,
          }}
        >
          <div
            className={`flex h-8 items-center border-b border-[var(--border-subtle)] px-2 text-[0px] font-semibold leading-none ${themeClasses.textMuted}`}
          >
      ...
      {canvasContextMenu && canvasContextMenuPosition ? (
        <div
          ...
          style={{
            left: canvasContextMenuPosition.left,
            top: canvasContextMenuPosition.top,
          }}
          onContextMenu={(event) => event.preventDefault()}
        >
          <div
            className={`flex h-8 items-center border-b border-[var(--border-subtle)] px-2 text-[10px] font-semibold leading-none ${themeClasses.textMuted}`}
          >
            创建节点
          </div>
          ...
          <div className="node-scrollbar max-h-[min(62vh,30rem)] overflow-y-auto py-1">
            {canvasContextMenuCategories.map((category) => (
              <section key={category.id} className="py-1">
                <div
                  className={`px-2 pb-1 text-[9px] font-semibold uppercase tracking-[0.14em] ${themeClasses.textMuted}`}
                >
                  {category.label}
                </div>
