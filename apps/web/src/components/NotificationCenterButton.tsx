...
function NotificationRow({
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`h-full min-h-11 w-0.5 rounded-full ${kindTone[notification.kind]}`}
          aria-hidden="true"
        />
        <span className="min-w-0">
          <span className="flex min-w-0 items-start justify-between gap-3">
            <span className="flex min-w-0 items-center gap-1.5">
              <span
                className={`shrink-0 ${notification.kind === "error" ? "text-red-500 dark:text-red-300" : notification.kind === "broadcast" ? "text-sky-500 dark:text-sky-300" : "text-amber-600 dark:text-amber-300"}`}
              >
...
export function NotificationCenterButton() {
  ...
  const unreadCount = items.reduce(
    (count, item) => count + Number(item.readAt === null),
    0,
  );
            ...
            >
              <header className="flex items-center justify-between gap-3 border-b border-[var(--border-subtle)] px-3.5 py-3">
                <span className="min-w-0">
                  ...
                  <span
                    className={`mt-0.5 block text-[10px] ${themeClasses.textMuted}`}
                  >
              ...
              <div className="min-h-0 flex-1 overflow-y-auto [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[var(--border-subtle)] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar]:w-1.5">
                {visibleItems.length > 0 ? (
                  ...
                  <div className="flex min-h-44 flex-col items-center justify-center px-6 text-center">
                    ...
                    <span
                      className={`mt-3 text-xs font-medium ${themeClasses.textPrimary}`}
                    >
                      {filter === "unread" ? "没有未读消息" : "暂无通知"}
                    </span>
                    <span
                      className={`mt-1 text-[10px] leading-4 ${themeClasses.textMuted}`}
                    >
                      广播、错误和系统消息会出现在这里
                    </span>
                  </div>
