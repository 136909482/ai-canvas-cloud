...
function decodeCanvasAssetItem(value: string): CanvasImageAssetItem | null {
  try {
    ...
    return {
      ...
      imageAsset:
        parsed.imageAsset && typeof parsed.imageAsset === "object"
...
function getCanvasAssetKindLabel(
...
export function FloatingToolbar() {
  ...
  const addTextSplitterNode = useCanvasStore(
    (state) => state.addTextSplitterNode,
  );
  const addImageNode = useCanvasStore((state) => state.addImageNode);
  const addVideoNode = useCanvasStore((state) => state.addVideoNode);
  const addVideoGenerateNode = useCanvasStore(
    (state) => state.addVideoGenerateNode,
  );
  ...
  const addGeneratedPreviewNode = useCanvasStore(
    (state) => state.addGeneratedPreviewNode,
  );
  ...
  const insertWorkflowTemplate = useCanvasStore(
    (state) => state.insertWorkflowTemplate,
  );
  ...
  const templateOpenRequestVersion = useWorkflowTemplateStore(
    (state) => state.openRequestVersion,
  );
  const hydrateTemplates = useWorkflowTemplateStore((state) => state.hydrate);
  const saveSelectionAsTemplate = useWorkflowTemplateStore(
    (state) => state.saveSelection,
  );
  const renameTemplate = useWorkflowTemplateStore(
    (state) => state.renameTemplate,
  );
  const deleteTemplate = useWorkflowTemplateStore(
  ...
  const { screenToFlowPosition, setCenter, viewportInitialized, getViewport } =
    useReactFlow();
  ...
  const createNodeFromToolbar = (
    ...
    const toolbarRect =
      toolbarRef.current?.getBoundingClientRect() ?? buttonRect;
    ...
    const preferredPosition = screenToFlowPosition({
      ...
      y: Math.min(
        viewportHeight - 80,
        Math.max(80, buttonRect.top + buttonRect.height / 2),
      ),
    });
    ...
    setCenter(
      createdNode.position.x + width / 2,
      createdNode.position.y + height / 2,
      {
        duration: 180,
        zoom,
      },
  ...
  const commonTools =
    nodeLibraryCategories.find((category) => category.id === "common")?.tools ??
    [];
  ...
  const hasLibraryResults = filteredNodeLibraryCategories.some(
    (category) => category.tools.length > 0,
  );
  const canvasImageAssetKeys = useCanvasStore(
    useShallow((state) =>
      isAssetLibraryOpen
        ? state.nodes.flatMap((node) => {
            ...
            const imageWidth =
              getNumberValue(data.imageWidth) || nodeSize.width;
            const imageHeight =
              getNumberValue(data.imageHeight) || nodeSize.height;
            const resolution = getStringValue(data.resolution);
            const imageAsset =
              data.imageAsset && typeof data.imageAsset === "object"
                ? (data.imageAsset as WorkspaceImageAsset)
                : null;

            return [
              encodeCanvasAssetItem({
                nodeId: node.id,
  ...
  useEffect(() => {
    ...
    if (isAssetLibraryOpen)
      window.requestAnimationFrame(() => assetPanelRef.current?.focus());
  ...
  const insertTemplateFromLibrary = (
    event: MouseEvent<HTMLButtonElement>,
    templateId: string,
  ) => {
    ...
    const preferredPosition = screenToFlowPosition({
      x: Math.max(
        toolbarRect.right + TOOLBAR_SAFE_OFFSET_X,
        window.innerWidth * TOOLBAR_SPAWN_VIEWPORT_X,
      ),
      y: Math.min(
    ...
    const ids = runTracked(() =>
      insertWorkflowTemplate(template, preferredPosition),
    );
    const firstNode = useCanvasStore
      .getState()
      .nodes.find((node) => node.id === ids[0]);
    if (firstNode) {
      const { width, height } = getNodeSize(firstNode);
      void setCenter(
        firstNode.position.x + width / 2,
        firstNode.position.y + height / 2,
        { duration: 220, zoom: getViewport().zoom || 1 },
      );
    }
    closeFloatingPanels();
  };
  ...
  const locateCanvasAsset = (asset: CanvasImageAssetItem) => {
    const sourceNode = useCanvasStore
      .getState()
      .nodes.find((node) => node.id === asset.nodeId);
    selectNode(asset.nodeId);

    if (sourceNode) {
      const { width, height } = getNodeSize(sourceNode);
      void setCenter(
        sourceNode.position.x + width / 2,
        sourceNode.position.y + height / 2,
        {
          duration: 280,
          zoom: 0.9,
        },
  ...
  const reuseCanvasAsset = (
    asset: CanvasImageAssetItem,
  ...
  const handleAssetDragStart = (
    ...
    event.dataTransfer.setData(
      "application/x-ai-canvas-image-asset",
      asset.nodeId,
    );
  ...
  return (
    <div ref={rootRef} className="fixed left-3 top-1/2 z-20 -translate-y-1/2">
          ...
          return (
            <div key={tool.id} className="relative">
              <button
                ...
                aria-controls={
                  tool.id === "more" && isActive
                    ? "node-library-panel"
                    : tool.id === "asset-library" && isActive
                      ? "asset-library-panel"
                      : tool.id === "workflow-templates" && isActive
                        ? "workflow-template-panel"
                        : undefined
                }
                data-testid={
                  tool.id === "project-manager"
      ...
      {isTemplateLibraryOpen && (
        <div
          id="workflow-template-panel"
          role="dialog"
          aria-label={UI_TEXT.templateLibrary}
          className={`absolute left-full top-1/2 ml-3 flex max-h-[min(640px,calc(100vh-32px))] w-[360px] -translate-y-1/2 flex-col overflow-hidden rounded-xl ${themeClasses.strongPanel}`}
        >
            ...
            <div className="flex items-center gap-2 text-[var(--text-primary)]">
              ...
              <div>
                <div className="text-sm font-semibold leading-tight">
                  {UI_TEXT.templateLibrary}
                </div>
                <div className="mt-0.5 text-[11px] text-[var(--text-muted)]">
                  {UI_TEXT.templateLibraryHint}
                </div>
              </div>
            </div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-2.5 scrollbar-hidden">
            {!templatesHydrated && templatesBusy ? (
              <div className="flex h-32 items-center justify-center text-[11px] text-[var(--text-muted)]">
                正在加载模板...
              </div>
            ) : templates.length === 0 ? (
              ...
              <div className="divide-y divide-[var(--border-subtle)] border-y border-[var(--border-subtle)]">
                {templates.map((template) => (
                  <div
                    key={template.id}
                    className="flex min-h-14 items-center gap-2 bg-[var(--control-bg)] px-2.5 py-2"
                  >
                      ...
                      {renamingTemplateId === template.id ? (
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            onKeyDown={(event) => {
                              if (event.key === "Enter")
                                void commitTemplateRename(template.id);
                              if (event.key === "Escape")
                                setRenamingTemplateId(null);
                            }}
                            aria-label="模板名称"
                    ...
                    {renamingTemplateId !== template.id && (
                      <>
                        <button
                          type="button"
                          onClick={(event) =>
                            insertTemplateFromLibrary(event, template.id)
                          }
                        ...
                        <button
                          ...
                          disabled={!workspaceConfigured || templatesBusy}
                          onClick={() =>
                            void removeTemplate(template.id, template.name)
                          }
      ...
      {isAssetLibraryOpen && (
        <div
          ...
          ref={assetPanelRef}
          role="dialog"
          aria-label={UI_TEXT.assetLibrary}
          tabIndex={-1}
          className={`absolute left-full top-1/2 ml-3 flex max-h-[min(680px,calc(100vh-32px))] w-[420px] -translate-y-1/2 flex-col overflow-hidden rounded-2xl ${themeClasses.strongPanel}`}
        >
          <div className="border-b border-[var(--border-subtle)] bg-[var(--control-bg)] px-3.5 py-3">
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-2 text-[var(--text-primary)]">
                ...
                <div className="min-w-0">
                  <div className="text-sm font-semibold leading-tight">
                    {UI_TEXT.assetLibrary}
                  </div>
                  <div className="mt-0.5 truncate text-[11px] text-[var(--text-muted)]">
                    {UI_TEXT.assetLibraryHint}
                  </div>
                </div>
              </div>
                ...
                {canvasImageAssets.map((asset, index) => (
                  ...
                  >
                    ...
                    <span className="block min-w-0 px-1.5 py-1.5">
                      <span className="block truncate text-[10px] font-semibold leading-4 text-[var(--text-primary)]">
                        {asset.title}
                      </span>
                      <span className="mt-0.5 block truncate text-[9px] leading-3 text-[var(--text-muted)]">
                        {asset.metaLabel}
                      </span>
                    </span>
                  </button>
