...
function getGroupColorOption(color: unknown) {
  return (
    GROUP_COLOR_OPTIONS.find((option) => option.id === color) ??
...
const DANGER_TOOLBAR_BUTTON_CLASS_NAME =
  "inline-flex h-7 w-7 items-center justify-center rounded-lg border border-transparent bg-transparent text-[var(--text-muted)] transition hover:border-red-400/25 hover:bg-red-500/10 hover:text-red-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400/20 dark:hover:text-red-200";
...
export function SelectionActionsToolbar() {
  ...
  const selectedTopLevelNodes = useCanvasStore(
    useShallow(selectSelectedTopLevelNodes),
  );
  const selectedGroupNodes = useCanvasStore(
    useShallow(selectSelectedGroupNodes),
  );
  ...
  const selectedNodeCount =
    selectedTopLevelNodes.length + selectedGroupNodes.length;
  const isMultiSelection = selectedNodeCount >= 2;
  const isSingleGroupSelection =
    selectedNodeCount === 1 && selectedGroupNodes.length === 1;
  const selectedGroupNode = isSingleGroupSelection
    ? selectedGroupNodes[0]
    : null;
  const selectedGroupColorOption = getGroupColorOption(
    selectedGroupNode?.data.color ?? DEFAULT_GROUP_COLOR,
  );
  const isColorMenuOpen = Boolean(
    selectedGroupNode && colorMenuGroupId === selectedGroupNode.id,
  );
  ...
  const toolbarNodeIds = useMemo(
    () => toolbarNodes.map((node) => node.id),
    [toolbarNodes],
  );
          ...
          <>
            <div className="relative">
              <button
                ...
                onClick={() =>
                  setColorMenuGroupId(
                    isColorMenuOpen ? null : (selectedGroupNode?.id ?? null),
                  )
                }
                className={TOOLBAR_BUTTON_CLASS_NAME}
              >
                <span
                  className={`h-3.5 w-3.5 rounded-full border border-[var(--border-subtle)] shadow-[0_0_0_2px_var(--control-bg-hover),0_0_12px_rgba(139,92,246,0.16)] ${selectedGroupColorOption.swatchClassName}`}
                />
              </button>
              ...
              {isColorMenuOpen ? (
                <div
                  role="menu"
                  aria-label={UI_TEXT.changeColor}
                  className={`absolute left-1/2 top-[calc(100%+8px)] z-40 flex -translate-x-1/2 flex-col gap-1.5 p-2 ${themeClasses.nodeToolbarPanel}`}
                >
            ...
            <ToolbarIconButton
              ...
              onClick={() =>
                runTracked(() => ungroupNode(selectedGroupNodes[0].id))
              }
              className={TOOLBAR_BUTTON_CLASS_NAME}
            ...
            selectedTopLevelNodes.length >= 2 ? (
              <>
                ...
                <ToolbarIconButton
                  ...
                  onClick={() =>
                    runTracked(() => arrangeSelectedNodes("horizontal"))
                  }
                  className={TOOLBAR_BUTTON_CLASS_NAME}
                ...
                <ToolbarIconButton
                  ...
                  onClick={() =>
                    runTracked(() => arrangeSelectedNodes("vertical"))
                  }
                  className={TOOLBAR_BUTTON_CLASS_NAME}
