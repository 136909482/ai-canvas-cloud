...
function FeedbackToastItem({ toast }: { toast: FeedbackToast }) {
  ...
  return (
    <div
      className={`flex w-[min(24rem,calc(100vw-2rem))] items-start gap-3 rounded-xl border p-3 shadow-[var(--shadow-panel)] backdrop-blur-2xl ${themeClasses.strongPanel}`}
    >
      <div
        className={`mt-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border ${toastToneClassName[toast.tone]}`}
      >
        <ToastIcon tone={toast.tone} />
      </div>
      <div className="min-w-0 flex-1">
        <div
          className={`text-sm font-semibold leading-5 ${themeClasses.textPrimary}`}
        >
          {toast.title}
        </div>
        {toast.message ? (
...
export function AppFeedbackHost() {
  ...
  const confirmDialogRef = useDialogFocus<HTMLDivElement>(
        ...
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/32 px-4 py-6 backdrop-blur-sm">
          ...
          >
            <div
              ...
              id={`${confirmRequest.id}-title`}
            >
              {confirmRequest.title}
            </div>
            <div className={`mt-2 text-sm leading-6 ${themeClasses.textMuted}`}>
