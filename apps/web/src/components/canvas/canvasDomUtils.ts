...
export function getSelectedReactFlowNodeIdFromDom() {
  const selectedNode = document.querySelector(
    '.react-flow__node.selected [data-testid^="node-"]',
  );
...
export function isCanvasEmptyDropTarget(
  target: DragEvent<HTMLDivElement>["target"] | EventTarget | null,
) {
  if (!(target instanceof Element)) {
    return false;
  ...
  return !target.closest(
    '.react-flow__node, .react-flow__edge, .react-flow__panel, input, textarea, select, [contenteditable="true"], .ProseMirror',
  );
}
