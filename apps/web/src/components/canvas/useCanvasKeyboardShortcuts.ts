...
function syncFallbackSelection(
  lastPointerDownNodeIdRef: MutableRefObject<string | null>,
) {
  const canvasState = useCanvasStore.getState();
  const selectedNodeCount = canvasState.nodes.reduce(
  ...
  const selectedNodeId =
    selectedNodeCount === 0
      ? (getSelectedReactFlowNodeIdFromDom() ??
        lastPointerDownNodeIdRef.current)
      : null;
...
export function useCanvasKeyboardShortcuts({
  ...
  useEffect(() => {
    const handlePointerDownCapture = (event: PointerEvent) => {
      lastPointerDownNodeIdRef.current = getCanvasNodeIdFromTarget(
        event.target,
      );
    };
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (key === "c" && !isModifierPressed && !isEditable) {
        ...
        const arranged = historyState.runTracked(() =>
          useCanvasStore.getState().arrangeConnectedGraphFromSelection(),
        );
        if (arranged) {
      ...
      if (key === "d") {
        flushPendingNodeDragRef.current?.();
        const duplicatedNodeId = historyState.runTracked(() =>
          useCanvasStore.getState().duplicateSelectedNode(),
        );
        if (duplicatedNodeId) {
          event.preventDefault();
        }
    ...
    const handlePaste = (event: ClipboardEvent) => {
      ...
      const clipboardHasImage = Array.from(
        event.clipboardData?.items ?? [],
      ).some((item) => item.type.startsWith("image/"));
      const shouldPreserveImagePaste =
      ...
      const pastedNodeId = useHistoryStore.getState().runTracked(() => {
        ...
        return (
          latestCanvasState.pasteCopiedNode() ??
          latestCanvasState.duplicateSelectedNode()
        );
      });
      if (!pastedNodeId) {
        return;
