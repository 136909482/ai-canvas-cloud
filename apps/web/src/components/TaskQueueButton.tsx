...
export function TaskQueueButton() {
  ...
  const modelEntries = useSettingsStore((s) => s.config.modelEntries);
  ...
  const activeTaskCount = visibleTasks.filter(
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {visibleTasks.length === 0 && (
                    <span
                      className={`truncate text-xs ${themeClasses.textMuted}`}
                    >
                      {UI_TEXT.panelEmpty}
                    </span>
                  )}
