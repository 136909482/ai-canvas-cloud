  ...
  return (
    ...
    >
      ...
      <span className="flex h-5 min-w-0 items-center overflow-visible">
        <span className="block min-w-0 truncate text-[11px] font-medium leading-tight text-[var(--text-primary)]">
          {modelDisplayName}
        </span>
      </span>
      ...
      >
        {task.status === "error" && task.errorMsg ? (
          <TaskQueueIconButton
            label={`${TASK_QUEUE_COPY.errorDetail}: ${task.errorMsg}`}
            onClick={() => undefined}
            testId={`error-task-${task.id}`}
            tooltipPlacement="top"
            className="h-5 w-5 rounded-full text-red-400 hover:border-red-400/20 hover:bg-red-500/10 dark:text-red-300"
      ...
      <span className="inline-flex w-11 items-center justify-center text-[11px] font-semibold leading-none tabular-nums text-[var(--text-secondary)]">
        {formatDuration(task, now)}
      </span>

      <span
        className={`inline-flex h-5 min-w-12 items-center justify-center gap-1 whitespace-nowrap rounded border px-1 text-[10px] font-semibold leading-none ${statusMeta.pillClassName}`}
      >
        {statusMeta.label}
      </span>
      ...
      {!isCurrentProject ? (
        <span className="col-span-7 -mt-0.5 block min-w-0 truncate pl-2 text-[10px] leading-none text-[var(--text-muted)]">
