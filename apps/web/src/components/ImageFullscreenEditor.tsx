...
export function ImageFullscreenEditor() {
  ...
  const undoHistoryRef = useRef<Record<DrawMode, UndoSnapshot[]>>({
    annotation: [],
    mask: [],
  });
  const [undoCounts, setUndoCounts] = useState<Record<DrawMode, number>>({
    annotation: 0,
    mask: 0,
  });
  ...
  const imageSize = activeImage
    ...
    : { width: 0, height: 0 };
  const errorMsg =
    session && loadError?.imageUrl === session.imageUrl
      ? loadError.message
      : "";
  ...
  const undoDrawingForMode = useCallback(
    (mode: DrawMode) => {
      const canvas =
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      const nextZoom = Math.min(
        4,
        Math.max(0.25, Number((zoom + direction * 0.1).toFixed(2))),
      );
      ...
      pendingZoomAnchorRef.current = {
        ...
        anchorX:
          stageRect.width > 0
            ? Math.min(
                1,
                Math.max(0, (event.clientX - stageRect.left) / stageRect.width),
              )
            : 0.5,
        anchorY:
          stageRect.height > 0
            ? Math.min(
                1,
                Math.max(0, (event.clientY - stageRect.top) / stageRect.height),
              )
            : 0.5,
      };
      setZoom(nextZoom);
  ...
  useLayoutEffect(() => {
    ...
    const stageRect = stage.getBoundingClientRect();
    viewport.scrollLeft +=
      stageRect.left + stageRect.width * anchor.anchorX - anchor.pointerX;
    viewport.scrollTop +=
      stageRect.top + stageRect.height * anchor.anchorY - anchor.pointerY;
    scheduleBrushCursor(
  ...
  const drawShape = (
    from: CanvasPoint,
    to: CanvasPoint,
    mode: Extract<ToolMode, "line" | "rect" | "ellipse">,
  ) => {
    const canvas = getActiveDrawCanvas();
  ...
  const handlePointerDown = (event: PointerEvent<HTMLCanvasElement>) => {
    ...
    if (
      drawMode === "annotation" &&
      (toolMode === "line" || toolMode === "rect" || toolMode === "ellipse")
    ) {
      const canvas = getActiveDrawCanvas();
      shapeStartPointRef.current = point;
      shapePreviewSnapshotRef.current = canvas
  ...
  const handleViewportPointerMove = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const startScrollLeft = Number(
      viewport.dataset.panScrollLeft || viewport.scrollLeft,
    );
    const startScrollTop = Number(
      viewport.dataset.panScrollTop || viewport.scrollTop,
    );
  ...
  const handleTextAnnotationPointerDown = (
  ...
  const handleTextAnnotationPointerMove = (
    event: PointerEvent<HTMLDivElement>,
  ) => {
        ...
        annotation.id === drag.id
          ? {
              ...
              x: Math.max(
                0,
                Math.min(
                  imageSize.width - getTextAnnotationMetrics(annotation).width,
              ...
              y: Math.max(
                0,
                Math.min(
                  imageSize.height - MIN_TEXT_BOX_HEIGHT,
  ...
  const handleTextAnnotationPointerUp = (
    event: PointerEvent<HTMLDivElement>,
  ) => {
    const drag = textDragRef.current;
  ...
  const persistEditorOutputImage = async (
    imageUrl: string,
    fileName: string,
  ) => {
    ...
    return {
      imageUrl: await platformBridge.resolveWorkspaceAssetUrl(
    ...
    try {
      ...
      const createdNodeId = runTracked(() =>
        createImageEditorOutputNode(session.nodeId, {
          ...
          imageWidth: imageSize.width,
          imageHeight: imageSize.height,
          sourceImageNodeId: isMaskOutput
            ? null
            : session.nodeType === "imageNode"
              ? session.nodeId
  ...
  const downloadImage = () => {
    ...
    const imageUrl =
      downloadMode === "mask" ? createMaskImage() : createCompositedImage();
    if (imageUrl) {
      downloadDataUrl(
        imageUrl,
        getDownloadFileName(
  ...
  const brushCursorSize = Math.max(
    18,
    Math.min(44, brushSize * displayScale + 12),
  );
  const brushCursorDotSize = Math.max(
    6,
    Math.min(16, brushSize * displayScale * 0.5),
  );
  const selectedTextAnnotation = selectedTextId
    ? (textAnnotations.find((annotation) => annotation.id === selectedTextId) ??
      null)
    : null;
  const activeSize =
    toolMode === "text"
      ...
      : selectedTextAnnotation
        ? selectedTextAnnotation.fontSize
        : brushSize;
  const activeSizeMin =
    toolMode === "text" || selectedTextAnnotation
      ? MIN_TEXT_SIZE
      : MIN_BRUSH_SIZE;
  const activeSizeMax =
    toolMode === "text" || selectedTextAnnotation
  ...
  const annotationCanvasCursorClassName =
    toolMode === "text"
      ? "cursor-text"
      : toolMode === "select"
        ? "cursor-default"
  ...
  return (
    <div
      ...
      aria-label="图片全屏编辑器"
      tabIndex={-1}
      className="fixed inset-0 z-[10000] bg-[var(--canvas-bg)] text-[var(--text-primary)]"
    >
      <div
        className={`absolute left-1/2 top-2 z-20 flex -translate-x-1/2 items-center gap-1 p-[5px] ${themeClasses.strongPanel}`}
      >
        ...
        {drawMode === "annotation" && selectedTextId ? (
          <>
            ...
            <TooltipIconButton
              ...
              onClick={() => {
                setTextAnnotations((annotations) =>
                  annotations.filter((item) => item.id !== selectedTextId),
                );
                setSelectedTextId(null);
              }}
              className={TOOLBAR_ICON_BUTTON_CLASS}
        ...
        <div className="relative">
          ...
          {colorPaletteOpen ? (
            <div
              className={`absolute left-1/2 top-full z-30 mt-2 grid w-[8.5rem] -translate-x-1/2 grid-cols-4 justify-items-center gap-2 p-2 ${themeClasses.strongPanel}`}
            >
                ...
                ? textAnnotations.map((annotation) => {
                    ...
                    return (
                      <div
                        ...
                        onPointerDown={(event) =>
                          handleTextAnnotationPointerDown(event, annotation)
                        }
