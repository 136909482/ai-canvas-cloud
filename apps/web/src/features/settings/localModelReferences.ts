...
const LOCAL_MODEL_REFERENCE_PATTERN =
...
export function normalizeLocalModelBindings(
  value: unknown,
...
export function resolveLocalModelReference(
...
export function findLocalModelReference(
  bindings: Record<string, string>,
  modelId: string,
) {
  ...
  return (
    Object.entries(bindings).find(
      ([, boundModelId]) => boundModelId === normalizedModelId,
    )?.[0] ?? null
  );
}

export function createLocalModelReference(
