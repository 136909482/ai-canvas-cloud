...
export function LocalVaultSettingsPanel() {
  ...
  const filteredProviderProfiles = useMemo(() => {
    ...
    return config.providerProfiles.filter(
      (profile) =>
        ...
        profile.baseUrl.toLowerCase().includes(query),
    );
  ...
  const closeImportDialog = (hasPendingSelection: boolean) => {
    ...
    void confirm({
      title: "取消模型导入",
      message: "未确认的模型选择不会写入本地 Vault。",
      confirmLabel: "取消导入",
      tone: "danger",
    }).then((confirmed) => {
      if (confirmed) setImportDialogOpen(false);
    });
  };
  ...
  return (
    <>
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex items-center gap-2 border-b border-[var(--border-subtle)] px-5 py-3 text-xs font-medium text-[var(--text-secondary)]">
        ...
        <div className="grid min-h-0 flex-1 md:grid-cols-[15rem_minmax(0,1fr)]">
              ...
              {filteredProviderProfiles.map((profile) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`mt-0.5 block truncate text-[10px] ${themeClasses.textMuted}`}
                    >
                      {profile.baseUrl}
                    </span>
                  </button>
          ...
          <section className="min-h-0 overflow-y-auto px-5 py-4">
              ...
              <div className="mx-auto max-w-3xl space-y-4">
                ...
                <section className="space-y-3 border-t border-[var(--border-subtle)] pt-4">
                  ...
                  {providerModelEntries.length > 0 ? (
                    <div className="overflow-hidden border border-[var(--border-subtle)] bg-[var(--control-bg)]">
            ...
            {providerDraft && modelDraft && isModelEditorOpen ? (
              <div className="mx-auto max-w-3xl space-y-4 border-t border-[var(--border-subtle)] pt-4">
                <div className="flex items-center justify-between gap-3">
                  <span className="text-xs font-medium text-[var(--text-primary)]">
                ...
                <div className="flex justify-end border-t border-[var(--border-subtle)] pt-4">
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => {
                        setDefaultModel(modelDraft.id);
                        void persistLocalVault();
                      }}
                      disabled={
                        modelDraft.id.startsWith("draft-model-") ||
                        modelDraft.category !== "image"
                      }
                      className={`${themeClasses.secondaryButton} h-8 rounded-[7px] px-3 text-xs disabled:opacity-50`}
                    >
                      设为默认
                    </button>
                    <button
      ...
      <ProviderModelImportDialog
        ...
        onConfirm={importDiscoveredModels}
      />
    </>
  );
}
