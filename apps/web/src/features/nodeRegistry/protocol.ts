...
export type NodeOutputLayoutKind =
  | "none"
  | "generated-preview"
  | "generated-video"
  | "llm-output"
...
const registrations = {
  imageNode: {
    ...
    idPrefix: "img",
    manual: {
      size: size(DEFAULT_IMAGE_NODE_WIDTH, DEFAULT_IMAGE_NODE_HEIGHT),
      build: buildManualImageNode,
    },
    connection: { output: "image" },
    outputLayout: "none",
    library: {
      id: "image",
      category: "common",
      ...
      icon: "image",
      label: "图片组件",
      description: "导入图片并作为参考或素材。",
      keywords: ["image", "图片", "素材", "参考图"],
    },
  },
  videoNode: {
    type: "videoNode",
    idPrefix: "video",
    manual: {
      size: size(DEFAULT_VIDEO_NODE_WIDTH, DEFAULT_VIDEO_NODE_HEIGHT),
      build: buildManualVideoNode,
    },
    connection: {},
    outputLayout: "none",
    library: {
  ...
  videoGenerateNode: {
    ...
    manual: {
      ...
      build: buildManualVideoGenerateNode,
    },
    connection: {
      inputs: {
        input: ["text", "image"],
      ...
      quickCreateTargetHandle: "input",
    },
    outputLayout: "generated-video",
    library: {
      id: "ai-video",
      category: "common",
      order: 50,
      icon: "video",
      label: "AI 视频",
      description: "配置视频模型、提示词、比例、时长和分辨率。",
      keywords: ["video", "ai", "视频", "生成视频"],
    },
  },
  imageCropNode: {
    type: "imageCropNode",
    idPrefix: "crop",
    manual: {
      size: size(DEFAULT_IMAGE_CROP_NODE_WIDTH, DEFAULT_IMAGE_CROP_NODE_HEIGHT),
      build: buildManualImageCropNode,
    },
    connection: {
      inputs: { input: ["image"] },
      quickCreateTargetHandle: "input",
    },
    outputLayout: "crop-output",
    library: {
      id: "image-crop",
      ...
      icon: "crop",
      label: "图像裁切",
      description: "裁切参考图或提取画面区域。",
      keywords: ["crop", "裁切", "图片处理"],
    },
  },
  textNode: {
    type: "textNode",
    idPrefix: "text",
    manual: {
      size: size(DEFAULT_TEXT_NODE_WIDTH, DEFAULT_TEXT_NODE_HEIGHT),
      build: buildManualTextNode,
    },
    ...
    library: {
      ...
      icon: "text",
      label: "文本组件",
      description: "输入提示词、备注和中间文本。",
      keywords: ["text", "prompt", "提示词", "文字"],
    },
  },
  textSplitterNode: {
    type: "textSplitterNode",
    idPrefix: "split",
    rendererType: "textSplitterNode",
    connection: { output: "text", inputs: { input: ["text"] } },
    outputLayout: "splitter-output",
  },
  inlineTextSplitterNode: {
    type: "inlineTextSplitterNode",
    idPrefix: "inlinesplit",
    manual: {
      size: size(
        DEFAULT_INLINE_TEXT_SPLITTER_NODE_WIDTH,
        DEFAULT_INLINE_TEXT_SPLITTER_NODE_HEIGHT,
      ),
      build: buildManualInlineTextSplitterNode,
    },
    connection: {
      output: "text",
      inputs: { input: ["text"] },
    ...
    library: {
      ...
      icon: "split",
      label: "文本分割",
      description: "把长文本切分成可连接的片段。",
      keywords: ["split", "文本分割", "分镜", "拆分"],
    },
  },
  generateNode: {
    type: "generateNode",
    idPrefix: "gen",
    manual: {
      size: size(DEFAULT_GENERATE_NODE_WIDTH, DEFAULT_GENERATE_NODE_HEIGHT),
      build: buildManualGenerateNode,
    },
    connection: {
      inputs: { prompt: ["text", "image"] },
      quickCreateTargetHandle: "prompt",
    },
  ...
  interiorDesignNode: {
    ...
    library: {
      ...
      icon: "house",
      label: "室内设计",
      description: "配置专业室内参数并输出结构化 JSON 提示词。",
  ...
  imageEditNode: {
    ...
    idPrefix: "edit",
    manual: {
      size: size(DEFAULT_IMAGE_EDIT_NODE_WIDTH, DEFAULT_IMAGE_EDIT_NODE_HEIGHT),
      build: buildManualImageEditNode,
    },
    connection: {
      inputs: { base: ["image"], reference: ["image"] },
  ...
  generatedPreviewNode: {
    ...
    idPrefix: "preview",
    manual: {
      size: size(DEFAULT_PREVIEW_NODE_WIDTH, DEFAULT_PREVIEW_NODE_HEIGHT),
      build: buildManualGeneratedPreviewNode,
    },
    connection: { output: "image" },
    outputLayout: "none",
    library: {
      ...
      icon: "preview",
      label: "生成预览",
      description: "展示生成结果并承接后续编辑。",
      keywords: ["preview", "预览", "结果"],
    },
  },
  compareNode: {
    type: "compareNode",
    idPrefix: "compare",
    manual: {
      size: size(DEFAULT_COMPARE_NODE_WIDTH, DEFAULT_COMPARE_NODE_HEIGHT),
      build: buildManualCompareNode,
    },
    connection: {
      inputs: { image1: ["image"], image2: ["image"] },
      quickCreateTargetHandle: "image1",
    },
    outputLayout: "none",
    library: {
      id: "compare",
      ...
      icon: "compare",
      label: "图片对比",
      description: "并排或滑动比较两张图片。",
      keywords: ["compare", "对比", "图片对比"],
    },
  },
  groupNode: {
    type: "groupNode",
  ...
  llmNode: {
    type: "llmNode",
    idPrefix: "llm",
    rendererType: "llmFileNode",
    connection: {
      inputs: { input: ["text", "image"] },
      quickCreateTargetHandle: "input",
    },
    outputLayout: "llm-output",
  },
  llmFileNode: {
    type: "llmFileNode",
    idPrefix: "llmfile",
    manual: {
      size: size(DEFAULT_LLM_NODE_WIDTH, DEFAULT_LLM_NODE_HEIGHT),
      build: buildManualLLMFileNode,
    },
    connection: {
      inputs: { input: ["text", "image"] },
      quickCreateTargetHandle: "input",
    ...
    library: {
      ...
      icon: "bot",
      label: "大模型节点",
      description: "运行多模态或文本大模型任务。",
      keywords: ["llm", "大模型", "对话", "分析"],
    },
  },
  llmOutputTextNode: {
  ...
  testImageNode: {
    ...
    manual: {
      size: size(DEFAULT_IMAGE_NODE_WIDTH, DEFAULT_IMAGE_NODE_HEIGHT),
      build: buildManualTestImageNode,
    },
    connection: { output: "image" },
    outputLayout: "none",
  },
  panoramaNode: {
    ...
    idPrefix: "pano",
    manual: {
      size: size(DEFAULT_PANORAMA_NODE_WIDTH, DEFAULT_PANORAMA_NODE_HEIGHT),
      build: buildManualPanoramaNode,
    },
    connection: {
      inputs: { input: ["image"] },
      quickCreateTargetHandle: "input",
    ...
    library: {
      ...
      icon: "panorama",
      label: "全景图片",
      description: "展示 360° 全景图片。",
      keywords: ["panorama", "全景", "360"],
    },
  },
} satisfies Record<AppNodeType, CanvasNodeRegistration>;

export const canvasNodeRegistrations: Record<
  AppNodeType,
  CanvasNodeRegistration
> = registrations;
...
export function getNodeConnectionOutput(
  type: string | undefined | null,
): NodeConnectionKind | null {
  return getCanvasNodeRegistration(type)?.connection.output ?? null;
}

export function getNodeConnectionInputs(
  type: string | undefined | null,
...
export function getQuickCreateTargetHandle(type: string | undefined | null) {
  return (
    getCanvasNodeRegistration(type)?.connection.quickCreateTargetHandle ?? null
  );
}

export const nodeLibraryRegistrations = Object.values(canvasNodeRegistrations)
  .flatMap((registration) =>
    registration.library && registration.manual
      ? [
          registration as CanvasNodeRegistration & {
            library: NodeLibraryRegistration;
            manual: ManualNodeRegistration;
          },
        ]
  ...
  .sort(
    (left, right) =>
      left.library.category.localeCompare(right.library.category) ||
      left.library.order - right.library.order,
  );
