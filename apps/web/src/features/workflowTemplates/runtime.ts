...
function getNodeWidth(node: Node) {
  return (
    Number(node.measured?.width ?? node.width ?? node.style?.width ?? 0) || 0
  );
}

function getNodeHeight(node: Node) {
  return (
    Number(node.measured?.height ?? node.height ?? node.style?.height ?? 0) || 0
  );
}
...
function isInsideGroup(node: Node, group: Node) {
  return (
    ...
    node.position.x + getNodeWidth(node) <=
      group.position.x + getNodeWidth(group) &&
    node.position.y + getNodeHeight(node) <=
      group.position.y + getNodeHeight(group)
  );
}
...
export function captureSelectedWorkflowTemplate(
  nodes: Node[],
  edges: Edge[],
): WorkflowTemplateDraft | null {
...
function remapNodeReferences(
  ...
  if (key && NODE_ID_ARRAY_FIELDS.has(key)) {
    return Array.isArray(value)
      ? value.flatMap((item) =>
          typeof item === "string" && nodeIdMap.has(item)
            ? [nodeIdMap.get(item)]
            : [],
        )
      : [];
...
export function instantiateWorkflowTemplate(
  ...
  const nodes = template.nodes.map((node) => ({
    ...
    selected: true,
    data: remapNodeReferences(node.data, null, nodeIdMap) as Record<
      string,
      unknown
    >,
  }));
...
export function normalizeWorkflowTemplateLibrary(
  value: unknown,
): WorkflowTemplateLibrary {
  const candidate = value as Partial<WorkflowTemplateLibrary> | null;
  const templates =
    candidate?.type === "ai-canvas-workflow-templates" &&
    candidate.version === 1 &&
    Array.isArray(candidate.templates)
      ? candidate.templates.filter((template): template is WorkflowTemplate =>
          Boolean(
            template &&
