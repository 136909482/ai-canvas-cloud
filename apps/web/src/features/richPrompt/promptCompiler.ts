...
function isImageMentionNode(
  node: RichPromptInlineNode,
): node is RichPromptImageMentionNode {
...
export function hydrateRichPromptImageMentionUrls(
  ...
  const thumbnailRelativePathBySourceId = new Map(
    references.map((reference) => [
      reference.sourceId,
      reference.thumbnailRelativePath ?? "",
    ]),
  );
  ...
  const content = document.content.map((paragraph) => {
    ...
    const nextContent = paragraph.content?.map((node) => {
      ...
      const nextThumbnailRelativePath = thumbnailRelativePathBySourceId.get(
        node.attrs.sourceNodeId,
      );
      ...
      if (
        ...
        (!nextThumbnailRelativePath ||
          nextThumbnailRelativePath === node.attrs.thumbnailRelativePath) &&
...
export function richPromptToPlainText(
  document: RichPromptDocument | null | undefined,
) {
  ...
  return document.content
    .map((paragraph) =>
      (paragraph.content ?? [])
        .map((node) =>
          isImageMentionNode(node) ? getMentionToken(node) : node.text,
        )
        .join(""),
    )
    .join("\n")
...
export function createRichPromptDocumentFromText(
  text: string,
): RichPromptDocument {
...
export function compileImageMentionPrompt(input: {
  ...
  const userPrompt =
    richPromptToPlainText(input.richPrompt) || input.fallbackPrompt.trim();
  ...
  const mappingLines = input.references
    ...
    .map(
      (reference) =>
        `- @${reference.label} = reference image ${reference.order} = image_urls[${reference.order - 1}]`,
    );
