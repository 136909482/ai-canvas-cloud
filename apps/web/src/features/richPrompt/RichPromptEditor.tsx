...
import {
  hydrateRichPromptImageMentionUrls,
...
export function RichPromptEditor({
  ...
  const content = useMemo(
    () =>
      createInitialContent(
        hydrateRichPromptImageMentionUrls(value, references),
        fallbackText,
      ),
  ...
  const clearAbandonedMentionTrigger = useCallback(
    (menu = mentionMenu) => {
      if (!editor || !menu) {
        setMentionMenu(null);
        return;
      ...
      const abandonedRange = getAbandonedMentionTriggerRange({
        ...
        textInRange: editor.state.doc.textBetween(
          menu.range.from,
          menu.range.to,
          "\n",
          "\0",
  ...
  return (
    <div
      ref={editorFrameRef}
      className="relative flex min-h-0 flex-1 cursor-text"
    >
      <EditorContent
        editor={editor}
        ...
        ? createPortal(
            <div
              ...
              style={{
                ...
                transform:
                  mentionMenu.position.placement === "above"
                    ? "translateY(-100%)"
                    : undefined,
              }}
