...
export function getImageMentionTriggerRange({
  cursorPosition,
  textBeforeCursor,
  readOnly,
}: TriggerRangeInput): MentionTriggerRange | null {
...
export function getAbandonedMentionTriggerRange({
  triggerRange,
  textInRange,
}: AbandonedTriggerInput): MentionTriggerRange | null {
...
export function shouldDismissMentionMenuPointer({
  hasActiveMenu,
  clickedInsideMenu,
}: PointerDismissInput) {
...
export function getMenuPositionFromCaret({
  caret,
  viewport,
}: {
  caret: CaretRect;
  ...
  const left = Math.min(
    ...
    Math.max(
      ...
      viewport.width - MENU_ESTIMATED_WIDTH_PX - MENU_EDGE_PADDING_PX,
    ),
  );
  ...
  return {
    ...
    top: hasRoomAbove
      ...
      : Math.min(
          viewport.height - MENU_EDGE_PADDING_PX,
          caret.bottom + MENU_BLOCK_GAP_PX,
        ),
