...
export const CANVAS_IMAGE_LOD_EXIT_ZOOM = 0.3;
...
export function getCanvasImagePerformanceStats(
  nodes: CanvasPerformanceNode[],
): CanvasImagePerformanceStats {
  return nodes.reduce<CanvasImagePerformanceStats>(
    (stats, node) => {
      ...
      if (
        !imageUrl &&
        !hasWorkspaceAsset(data.imageAsset) &&
        !hasWorkspaceAsset(data.videoAsset)
      ) {
        return stats;
      }
      ...
      return {
        ...
        largeImageNodeCount:
          stats.largeImageNodeCount +
          (isLargeImage(data, embeddedBytes) ? 1 : 0),
