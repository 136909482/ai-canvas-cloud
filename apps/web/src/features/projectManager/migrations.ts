...
export type PersistedProjectSnapshot = Omit<
...
function getPersistedSchemaVersion(snapshot: PersistedProjectSnapshot) {
  return typeof snapshot.schemaVersion === "number" &&
...
function normalizeTaskQueueSnapshot(
  snapshot: PersistedTaskQueueSnapshot,
): TaskQueueSnapshot {
  return {
...
export function migrateProjectRecordSnapshots(
...
export function migrateWorkspaceDataSnapshots(
  data: WorkspaceData,
): WorkspaceData {
  return {
    ...data,
    projects: data.projects.map((project) =>
      migrateProjectRecordSnapshots(project),
    ),
  };
}
