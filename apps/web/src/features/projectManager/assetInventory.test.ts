...
function createAsset(
  relativePath: string,
  extras: Partial<WorkspaceImageAsset> = {},
): WorkspaceImageAsset {
  return {
    relativePath,
...
function runAssetInventoryTests() {
  ...
  assert(
    references.some((reference) => reference.assetField === "resultImageAsset"),
    "task result image assets should be inventoried",
  );
  assert(
    references.some((reference) => reference.assetField === "videoAsset"),
    "video node assets should be inventoried",
  );
  assert(
    references.some((reference) => reference.kind === "thumbnail"),
    "thumbnail assets should be inventoried",
  );
  assert(
    references.some((reference) => reference.kind === "preview"),
    "preview assets should be inventoried",
  );

  const summary = summarizeWorkspaceAssetReferences(workspaceData);
  assert(
    summary.totalUniquePathCount === 6,
    "workspace summary should count unique original, thumbnail, and preview paths",
  );
  assert(
    summary.originalCount === 3,
    "workspace summary should dedupe original paths across saved and working snapshots",
  );
  assert(
    summary.thumbnailCount === 2,
    "workspace summary should dedupe thumbnail paths",
  );
  assert(
    summary.previewCount === 1,
    "workspace summary should count preview paths",
  );
  assert(
    summary.nodeReferenceCount === 8,
    "workspace summary should preserve node reference counts across snapshots",
  );
  assert(
    summary.taskReferenceCount === 2,
    "workspace summary should preserve task asset reference counts",
  );
  assert(
    summary.activeProjectSummary?.uniquePathCount === 5,
    "active project summary should be available",
  );
}
