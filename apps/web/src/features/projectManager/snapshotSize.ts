...
function getSnapshotSizeStatus(
  byteSize: number,
): ProjectSnapshotSizeReport["status"] {
...
function sortByByteSizeDescending<T extends { byteSize: number }>(
  entries: T[],
) {
  return [...entries]
...
function describeLargeString(
  snapshot: ProjectSnapshot,
  path: string,
  byteSize: number,
): SnapshotLargeStringEntry {
  ...
  if (nodeMatch) {
    ...
    if (attachmentMatch) {
      const inputFiles = Array.isArray(node?.data?.inputFiles)
...
function sanitizeNodeErrorMessage(
  node: ProjectSnapshot["canvas"]["nodes"][number],
) {
  const errorMsg = node.data?.errorMsg;
  if (
    typeof errorMsg !== "string" ||
    errorMsg.length <= SNAPSHOT_ERROR_MESSAGE_CHAR_LIMIT
  ) {
    return node;
  }
...
function sanitizeTaskErrorMessage(
  task: ProjectSnapshot["taskQueue"]["tasks"][number],
) {
  if (
    typeof task.errorMsg !== "string" ||
    task.errorMsg.length <= SNAPSHOT_ERROR_MESSAGE_CHAR_LIMIT
  ) {
    return task;
  }
...
export function sanitizeProjectSnapshotForPersistence(
  snapshot: ProjectSnapshot,
): ProjectSnapshot {
  return {
    ...snapshot,
    canvas: {
      ...
      nodes: snapshot.canvas.nodes.map((node) =>
        sanitizeNodeErrorMessage(node),
      ),
    },
    taskQueue: {
      ...snapshot.taskQueue,
      tasks: snapshot.taskQueue.tasks.map((task) =>
        sanitizeTaskErrorMessage(task),
      ),
    },
  };
}

export function stripLocalTaskQueueFromProjectSnapshot(
  snapshot: ProjectSnapshot,
): ProjectSnapshot {
  return {
...
export function sanitizeProjectRecordForPersistence(
  project: ProjectRecord,
): ProjectRecord {
  return {
    ...project,
    savedSnapshot: stripLocalTaskQueueFromProjectSnapshot(
      project.savedSnapshot,
    ),
    workingSnapshot: stripLocalTaskQueueFromProjectSnapshot(
      project.workingSnapshot,
    ),
  };
}

export function sanitizeWorkspaceDataForPersistence(
  data: WorkspaceData,
): WorkspaceData {
  return {
    ...data,
    projects: data.projects.map((project) =>
      sanitizeProjectRecordForPersistence(project),
    ),
  };
}

export function analyzeProjectSnapshotSize(
  snapshot: ProjectSnapshot,
): ProjectSnapshotSizeReport {
  ...
  return {
    ...
    embeddedMediaByteSize: embeddedMedia.reduce(
