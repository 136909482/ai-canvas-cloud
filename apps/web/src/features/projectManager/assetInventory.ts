...
export interface WorkspaceAssetReference {
  ...
  sourceId: string;
  assetField:
    "imageAsset" | "videoAsset" | "resultImageAsset" | "resultVideoAsset";
}
...
export function collectWorkspaceAssetReferences(data: WorkspaceData) {
  ...
  for (const project of data.projects) {
    ...
    collectSnapshotAssetReferences(
...
function finalizeProjectSummary(
  summary: MutableProjectSummary,
): ProjectAssetInventorySummary {
  return {
...
export function summarizeWorkspaceAssetReferences(
  data: WorkspaceData,
  ...
  return {
    ...
    activeProjectSummary: data.activeProjectId
      ? (projectSummaries.find(
          (summary) => summary.projectId === data.activeProjectId,
        ) ?? null)
      : null,
  };
}
