...
export function hasGraphDeletion(current: CanvasSnapshot, previous: CanvasSnapshot) {
  if (current.nodes.length < previous.nodes.length || current.edges.length < previous.edges.length) {
    return true
  }
