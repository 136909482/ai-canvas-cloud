      ...
      nodes: [
        ...
        },
        {
          ...
          data: {
            ...
            imageAsset: { relativePath: "images/city.png" },
          },
        },
      ],
      edges: [],
...
test("extracts project, text, and asset documents without embedded data urls", () => {
  ...
  assert.deepEqual(
    ...
    ["project", "text", "text", "asset"],
  );
  assert.equal(
    documents.some((document) => document.content.includes("data:image")),
    false,
  );
  assert.equal(
    documents.find((document) => document.kind === "asset")?.assetRelativePath,
    "images/city.png",
  );
});
...
test("searches Chinese substrings and combined keywords with stable relevance", () => {
  const documents = extractProjectSearchDocuments(project());
  assert.equal(
    searchWorkspaceDocuments(documents, { text: "霓虹街" })[0]?.nodeId,
    "text-1",
  );
  assert.equal(
    searchWorkspaceDocuments(documents, {
      text: "夜景 2048",
      kinds: ["asset"],
    })[0]?.nodeId,
    "image-2",
  ...
  assert.equal(
    searchWorkspaceDocuments(documents, { text: "不存在" }).length,
    0,
  );
});
