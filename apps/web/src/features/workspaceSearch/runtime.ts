...
function getAssetRelativePath(node: Node) {
  ...
  return asset && typeof asset === "object"
    ? stringValue((asset as Record<string, unknown>).relativePath)
    : "";
}
...
function isAssetNode(node: Node) {
  return (
    Boolean(getAssetRelativePath(node)) ||
    [
      "imageNode",
      "videoNode",
...
export function extractProjectSearchDocuments(
...
export function searchWorkspaceDocuments(
  ...
  return documents
    .flatMap((document) => {
      ...
      if (
        nodeTypes.size > 0 &&
        (!document.nodeType || !nodeTypes.has(document.nodeType))
      )
        return [];
