...
function blobToDataUrl(blob: Blob) {
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = () =>
      reject(reader.error ?? new Error("裁切结果转换失败"));
...
function loadImageElement(imageUrl: string) {
  return new Promise<HTMLImageElement>((resolve, reject) => {
    ...
    image.onerror = () =>
      reject(new Error("图片加载失败，请检查输入图片是否可用"));
    image.src = imageUrl;
  });
}
...
export function clampCropSegmentCount(
  value: number | undefined,
  fallback = DEFAULT_IMAGE_CROP_ROWS,
) {
  ...
  return clamp(
    ...
    MIN_IMAGE_CROP_SEGMENTS,
    MAX_IMAGE_CROP_SEGMENTS,
  );
}
...
export function buildEvenlyDistributedCuts(segmentCount: number) {
  ...
  return Array.from(
    { length: Math.max(0, safeSegmentCount - 1) },
    (_, index) => (index + 1) / safeSegmentCount,
  );
}
...
export function normalizeCropCuts(cuts: unknown, segmentCount: number) {
  ...
  for (let index = 0; index < targetLength; index += 1) {
    ...
    const nextValue = clamp(
      sortedCuts[index] ?? fallback[index] ?? lowerBound,
      lowerBound,
      upperBound,
    );
...
export async function cropImageIntoTiles(input: {
  ...
  const horizontalBoundaries = buildCropBoundaries(
    rowCount,
    input.horizontalCuts,
  );
  const verticalBoundaries = buildCropBoundaries(
    columnCount,
    input.verticalCuts,
  );
  ...
  for (let row = 0; row < rowCount; row += 1) {
    const sourceTop = Math.round(
      horizontalBoundaries[row] * image.naturalHeight,
    );
    const sourceBottom =
      row === rowCount - 1
        ? image.naturalHeight
    ...
    for (let column = 0; column < columnCount; column += 1) {
      const sourceLeft = Math.round(
        verticalBoundaries[column] * image.naturalWidth,
      );
      const sourceRight =
      ...
      const persisted = await persistCropTile(
        blob,
        input.projectId,
        input.cropNodeId,
        row,
