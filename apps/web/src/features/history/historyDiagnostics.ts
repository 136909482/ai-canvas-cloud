...
export function getHistorySizeStatus(
  byteSize: number,
): HistorySizeReport["status"] {
...
export function analyzeHistorySize(input: HistorySizeInput): HistorySizeReport {
  const entries: HistorySnapshotSizeEntry[] = [
    ...
    ...(input.pendingBaseline
      ? [
          {
            stack: "pending" as const,
            index: 0,
            byteSize: getSnapshotByteSize(input.pendingBaseline),
          },
        ]
      : []),
  ];
  ...
  const largestSnapshot = entries.reduce<HistorySnapshotSizeEntry | null>(
    (largest, entry) =>
      !largest || entry.byteSize > largest.byteSize ? entry : largest,
    null,
  );
