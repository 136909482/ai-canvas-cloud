...
test("classifies network, permission, missing resource, and persistence failures", () => {
  assert.equal(
    classifyDiagnosticKind(new TypeError("fetch failed"), "model"),
    "network",
  );
  assert.equal(
    classifyDiagnosticKind(new Error("HTTP 429 quota exhausted"), "model"),
    "network",
  );
  assert.equal(
    classifyDiagnosticKind(new Error("Permission denied"), "persistence"),
    "permission",
  );
  assert.equal(
    classifyDiagnosticKind(new Error("ENOENT asset missing"), "resource"),
    "not-found",
  );
  assert.equal(
    classifyDiagnosticKind(new Error("disk write failed"), "persistence"),
    "storage",
  );
});
...
test("private Provider diagnostics never retain endpoint, model, key, or upstream response details", () => {
  const privateDetails =
    "https://private.example/v1 model=private-model Authorization=Bearer private-key";
  const diagnostic = createAppDiagnostic(
    {
      area: "model",
      title: "图片生成失败",
