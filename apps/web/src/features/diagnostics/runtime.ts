...
const PRIVATE_PROVIDER_ERROR_MESSAGE =
  "浏览器 Provider 请求失败；私有 Provider 详情不写入诊断。";

const NETWORK_ERROR_PATTERN =
  /(?:\bHTTP\s*[45]\d\d\b|\b(?:429|5\d\d)\b|fetch|network|timeout|timed out|unable to reach|connection|quota|rate limit|网络|调用失败|请求失败|连接失败|超时)/i;
const PERMISSION_ERROR_PATTERN =
  /(?:permission|denied|not allowed|unauthorized|forbidden|权限|拒绝访问|未授权)/i;
const NOT_FOUND_ERROR_PATTERN =
  /(?:\bENOENT\b|not found|missing|不存在|找不到|缺少)/i;
const VALIDATION_ERROR_PATTERN =
  /(?:invalid|unsupported|requires?|must |格式不正确|不支持|请先|必须|无效)/i;
...
export function classifyDiagnosticKind(
  error: unknown,
  area: DiagnosticArea,
): DiagnosticKind {
...
function normalizeContext(context: CreateDiagnosticInput["context"]) {
  return Object.fromEntries(
    Object.entries(context ?? {}).filter(
      (entry): entry is [string, DiagnosticContextValue] =>
        entry[1] !== undefined,
    ),
  );
}

export function createAppDiagnostic(
