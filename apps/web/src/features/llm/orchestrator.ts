...
function getSelectedModel(nodeData: Pick<LLMNodeData, "model">) {
  ...
  const effectiveModel = nodeData.model.trim();
  const modelEntryId = resolveNodeModelEntryId(settings.config, effectiveModel);
  const isUnboundLocalModel =
    isLocalModelReference(effectiveModel) && !modelEntryId;
  const resolution = modelEntryId
  ...
  const diagnostic: ProviderConfigDiagnostic | null =
    resolution && !resolution.ok ? resolution.diagnostic : null;
...
function getLLMInputImageSources(
  nodeId: string,
  ...
  edges
    ...
    .forEach((edge) => {
      ...
      const sourceNode = nodes.find(
        (node) => node.id === edge.source && isImageSourceNodeType(node.type),
      );
      const imageUrl =
  ...
  const orderedSourceIds = [
    ...inputImageSourceOrder.filter((sourceId) =>
      imageBySourceId.has(sourceId),
    ),
    ...Array.from(imageBySourceId.keys()).filter(
      (sourceId) => !inputImageSourceOrder.includes(sourceId),
    ),
  ];
  ...
  return orderedSourceIds
    ...
    .filter((item): item is { sourceId: string; imageUrl: string } =>
      Boolean(item),
    )
    .slice(0, MAX_GENERATE_REFERENCE_IMAGES);
}

function buildRichPromptReferences(
  inputImages: Array<{ sourceId: string; imageUrl: string }>,
...
function getConnectedTextInput(nodeId: string) {
  ...
  const incomingTextEdge = edges.find((edge) => {
    ...
    return (
      ...
      sourceNode?.type === "llmOutputTextNode" ||
      sourceNode?.type === "inlineTextSplitterNode"
    );
  });
  ...
  if (sourceNode.type === "inlineTextSplitterNode") {
    const match = /^part-(\d+)$/.exec(
      typeof incomingTextEdge?.sourceHandle === "string"
        ? incomingTextEdge.sourceHandle
        : "",
    );
    const partIndex = match ? Number(match[1]) : -1;
    const parts = Array.isArray(sourceNode.data?.parts)
      ? sourceNode.data.parts.filter(
...
function getLLMInputFiles(nodeData: LLMSourceNodeData) {
  ...
  return nodeData.inputFiles
    .filter(
      (file): file is LLMFileNodeData["inputFiles"][number] =>
        Boolean(file) &&
        typeof file === "object" &&
...
async function runLLMExecution(
  ...
  const { effectiveModel, selectedModel, diagnostic, isUnboundLocalModel } =
    getSelectedModel(nodeData);
  if (!selectedModel) {
    canvasStore.updateNodeData(nodeId, {
      status: "error",
      errorMsg: isUnboundLocalModel
        ? UI_TEXT.unboundModel
        : (diagnostic?.message ?? UI_TEXT.noModel),
    });
    return null;
  }

  const inputImageSourceOrder = Array.isArray(nodeData.inputImageSourceOrder)
    ? nodeData.inputImageSourceOrder.filter(
  ...
  const compiledInputText = compileImageMentionPrompt({
    ...
    fallbackPrompt:
      connectedTextInput?.text ??
      (typeof nodeData.inputText === "string" ? nodeData.inputText : ""),
    references: richPromptReferences,
  });
...
export async function runLLMFileNode(
  nodeId: string,
  input: {
    instructionPrompt: string;
    presetSystemPrompt?: string;
