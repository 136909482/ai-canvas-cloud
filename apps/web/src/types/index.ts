...
export type GenerateStatus =
  "idle" | "queued" | "generating" | "done" | "error";
...
export const IMAGE_SOURCE_NODE_TYPES = [
  "imageNode",
  "generatedPreviewNode",
  "testImageNode",
] as const;
...
export function isImageSourceNodeType(
  type: string | undefined | null,
): type is (typeof IMAGE_SOURCE_NODE_TYPES)[number] {
  return (
    typeof type === "string" &&
    IMAGE_SOURCE_NODE_TYPES.includes(
      type as (typeof IMAGE_SOURCE_NODE_TYPES)[number],
    )
  );
}

export function isWorkspaceAssetNodeType(
  type: string | undefined | null,
): type is (typeof WORKSPACE_ASSET_NODE_TYPES)[number] {
  return (
    typeof type === "string" &&
    WORKSPACE_ASSET_NODE_TYPES.includes(
      type as (typeof WORKSPACE_ASSET_NODE_TYPES)[number],
    )
  );
}
...
export type AppNode<T extends AppNodeType = AppNodeType> = Node<
  AppNodeDataMap[T],
  T
>;
