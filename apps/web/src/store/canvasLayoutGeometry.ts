...
export const DEFAULT_ENTOURAGE_NODE_HEIGHT = 300;
...
export function getAbsoluteNodePosition(
  ...
  while (currentParentId) {
    ...
    currentParentId =
      typeof parentNode.parentId === "string" ? parentNode.parentId : null;
  }
...
function isNodeFullyInsideGroup(nodes: Node[], node: Node, groupNode: Node) {
  ...
  return (
    ...
    nodeAbsolutePosition.x + nodeWidth <=
      groupAbsolutePosition.x + groupWidth &&
  ...
  return new Set(
    nodes
      .filter(
        (node) =>
          node.type !== "groupNode" &&
          isNodeFullyInsideGroup(nodes, node, groupNode),
      )
      .map((node) => node.id),
  );
}
...
function getSmallestContainingGroup(nodes: Node[], node: Node) {
  const containingGroups = nodes.filter(
    (candidate) =>
      candidate.type === "groupNode" &&
      isNodeFullyInsideGroup(nodes, node, candidate),
  );
  ...
  return containingGroups.reduce((smallest, groupNode) => {
    ...
    return groupSize.width * groupSize.height <
...
export function buildGroupAwareLayoutTargets(
  nodes: Node[],
  edges: Edge[],
  shouldIncludeUnit: (node: Node) => boolean,
) {
  const normalizedNodes = normalizeVisualGroupNodes(nodes);
  const groupNodes = normalizedNodes.filter(
  ...
  for (const node of normalizedNodes) {
    ...
    const unitNode = normalizedNodes.find(
  ...
  const layoutEdges = edges
    .map((edge) => {
      ...
      if (
        ...
        !unitIds.has(sourceUnitId) ||
        !unitIds.has(targetUnitId)
      ) {
        return null;
      }
...
export function moveVisualGroupMembers(
  previousNodes: Node[],
  nextNodes: Node[],
  changes: NodeChange[],
) {
  const resizingNodeIds = new Set(
    changes
      .filter(
        (change): change is NodeChange & { type: "dimensions"; id: string } =>
          "id" in change && change.type === "dimensions",
      )
      .map((change) => change.id),
  );
  const movedGroupChanges = changes.filter(
    (
      change,
  ...
  for (const change of movedGroupChanges) {
    const previousGroupNode = previousNodes.find(
      (node) => node.id === change.id && node.type === "groupNode",
    );
    const nextGroupNode = movedNodes.find(
      (node) => node.id === change.id && node.type === "groupNode",
    );
...
export function getDescendantNodeIds(nodes: Node[], rootIds: Set<string>) {
  ...
  while (foundNewNode) {
    ...
    for (const node of nodes) {
      ...
      if (
        (rootIds.has(parentId) || descendantIds.has(parentId)) &&
...
export function expandGroupToFitDescendants(nodes: Node[], groupId: string) {
  ...
  const directChildIds = new Set(
    nodes.filter((node) => node.parentId === groupId).map((node) => node.id),
  );
  const groupAbsolutePosition = getAbsoluteNodePosition(nodes, groupNode);
  ...
  const shiftX =
    bounds.minX < minimumContentX ? minimumContentX - bounds.minX : 0;
  const shiftY =
    bounds.minY < minimumContentY ? minimumContentY - bounds.minY : 0;
  const requiredWidth = Math.max(
    currentWidth,
    bounds.maxX + shiftX + GROUP_PADDING_X,
  );
  const requiredHeight = Math.max(
    currentHeight,
    bounds.maxY + shiftY + GROUP_PADDING_Y,
  );
...
export function findManualSpawnPosition(
  ...
  for (
    let column = 1;
    column <= MANUAL_NODE_SPAWN_SEARCH_COLUMNS;
    column += 1
  ) {
    ...
    for (let row = 1; row <= MANUAL_NODE_SPAWN_SEARCH_ROWS; row += 1) {
      candidates.push({
        x: basePosition.x + column * stepX,
        y: basePosition.y + row * stepY,
      });
      candidates.push({
        x: basePosition.x + column * stepX,
        y: basePosition.y - row * stepY,
  ...
  for (const candidate of candidates) {
    ...
    if (
      !occupiedRectangles.some((rectangle) =>
        rectanglesOverlap(nextRectangle, rectangle, MANUAL_NODE_SPAWN_GAP),
      )
    ) {
      return candidate;
    }
