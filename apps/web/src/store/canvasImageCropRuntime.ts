...
export function buildImageCropOutputState({
  ...
  const existingPreviewById = new Map(
    existingPreviewIds
      .map((previewId) =>
        nodes.find(
          (node) =>
            node.id === previewId && node.type === "generatedPreviewNode",
        ),
      )
  ...
  for (let index = 0; index < previewResults.length; index += 1) {
    ...
    const previewSize = getPreviewNodeSize(
      previewResult.width,
    ...
    if (existingPreviewId && previewNodeById.has(existingPreviewId)) {
      ...
      previewNodeById.set(existingPreviewId, {
        ...buildCropPreviewNode(
          existingPreviewId,
          cropNodeId,
          {
            ...previewDraft,
            createdAt:
              typeof existingPreviewNode.data?.createdAt === "number"
    ...
    previewNodeById.set(
      previewId,
      buildCropPreviewNode(previewId, cropNodeId, previewDraft, previewSize),
    );
  }

  const removedPreviewIds = new Set(
    existingPreviewIds.filter(
      (previewId) => !nextPreviewIds.includes(previewId),
    ),
  );
  const nextNodes = nodes
    .filter(
      (node) =>
        !removedPreviewIds.has(node.id) && !previewNodeById.has(node.id),
    )
    .map((node) => {
  ...
  const existingEdgeKeys = new Set(
    edges
      .filter(
        (edge) =>
          !removedPreviewIds.has(edge.source) &&
          !removedPreviewIds.has(edge.target),
      )
  ...
  return {
    nodes: [
      ...
      ...nextPreviewIds
        ...
        .filter((node): node is Node<GeneratedPreviewNodeData> =>
          Boolean(node),
        ),
    ],
    edges: [
