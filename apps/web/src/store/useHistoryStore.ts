...
function scheduleDeferredCommit(commit: () => void) {
  ...
  if (idleScheduler.requestIdleCallback && idleScheduler.cancelIdleCallback) {
    const idleId = idleScheduler.requestIdleCallback(commit, {
      timeout: HISTORY_COMMIT_IDLE_TIMEOUT_MS,
    });
...
export const useHistoryStore = create<HistoryStore>((set, get) => ({
  ...
  beginTransaction: () => {
    ...
    set({
      pendingBaseline: cloneSnapshot(
        useCanvasStore.getState().getHistorySnapshot(),
      ),
    });
  },
