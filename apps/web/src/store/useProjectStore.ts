...
interface ProjectStore {
  ...
  ) => Promise<CommitProjectBundleImportResult>;
...
async function loadWorkspaceProjectForStore(projectId: string) {
  ...
  const project = await platformBridge
...
function setProjectPersistenceError(
  error: unknown,
  operation = "background-save",
) {
  if (error === null || error === undefined) {
    ...
    useProjectStore.setState((state) => ({
      ...
      persistenceConflictByProjectId: withoutProjectConflict(
        state.persistenceConflictByProjectId,
        projectId,
      ),
    }));
    return;
...
export const useProjectStore = create<ProjectStore>()((set, get) => {
  const hydrateProjectsFromWorkspace = async (allowLegacyFallback: boolean) => {
    ...
    if (workspaceProjectIndex?.projects.length) {
      const fallbackProjectSummary = getFallbackProjectSummary(
      ...
      if (loadedFallbackProject) {
        ...
        set({
          ...
          persistedSnapshotByProjectId: {
            [openedProject.id]: serializeProjectSnapshot(
              openedProject.workingSnapshot,
            ),
          },
    ...
    set({
      ...
      persistedSnapshotByProjectId: workspaceData
        ? buildPersistedSnapshotMap(initialData.projects)
        : {},
      persistenceMetaByProjectId: {},
      persistenceConflictByProjectId: {},
      isPersisting: false,
    ...
    await restoreProjectWorkspace(
      fallbackProject.workingSnapshot,
      fallbackProject.id,
    );
    set({ isReady: true });
  };
  ...
  return {
    ...
    persistWorkspaceFile: async () => {
      ...
      try {
        ...
        void platformBridge
          .createProjectCheckpoint(projectId, {
            checkpointType: "periodic",
          })
          .catch((error) =>
            setProjectPersistenceError(error, "periodic-checkpoint"),
          );
      ...
      const save = (async () => {
        ...
        try {
          ...
          const nextProjects = state.projects.map((project) =>
            project.id === state.activeProjectId ? nextActiveProject : project,
          );
    ...
    duplicateProject: async (projectId) => {
      ...
      const project = createProjectRecord(
        `${sourceProject.name} 副本`,
        sourceProject.workingSnapshot,
      );
    ...
    loadProject: async (projectId) => {
      ...
      set((state) => ({
        projects: state.projects.map((item) =>
          item.id === projectId ? openedProject : item,
        ),
    ...
    archiveProject: async (projectId) => {
      ...
      const loadedFallbackProject = await loadWorkspaceProjectForStore(
        fallbackProject.id,
      );
      ...
      set({
        ...
        persistedSnapshotByProjectId: {
          [openedFallbackProject.id]: serializeProjectSnapshot(
            openedFallbackProject.workingSnapshot,
          ),
        },
        persistenceMetaByProjectId: {},
        isReady: false,
    ...
    deleteProject: async (projectId) => {
      ...
      const fallbackProject =
        availableProjects.find(
          (item) => item.id === state.lastOpenedProjectId,
        ) ??
        availableProjects.find(
          (item) =>
      ...
      if (state.activeProjectId === projectId) {
        ...
        set({
          projects: remainingProjects.map((item) =>
            item.id === fallbackProject.id ? nextFallbackProject : item,
          ),
          activeProjectId: fallbackProject.id,
          lastOpenedProjectId: fallbackProject.id,
          persistedSnapshotByProjectId: Object.fromEntries([
            ...Object.entries(state.persistedSnapshotByProjectId).filter(
              ([id]) => id !== projectId,
            ),
            [
              fallbackProject.id,
              serializeProjectSnapshot(nextFallbackProject.workingSnapshot),
            ] as const,
          ]),
          persistenceMetaByProjectId: Object.fromEntries(
            Object.entries(state.persistenceMetaByProjectId).filter(
              ([id]) => id !== projectId,
            ),
          ),
          isReady: false,
        ...
        await restoreProjectWorkspace(
          nextFallbackProject.workingSnapshot,
          nextFallbackProject.id,
        );
        set({ isReady: true });
        if (isStorageConfigured()) {
          await platformBridge
    ...
    commitProjectImport: async (candidateId, resolution) => {
      const result = await platformBridge.commitProjectBundleImport({
        candidateId,
        resolution,
      });
    ...
    hasUnsavedChanges: () => {
      ...
      return (
        serializeProjectSnapshot(takeWorkspaceSnapshot()) !==
        serializeProjectSnapshot(activeProject.savedSnapshot)
      );
    },
    ...
    hasPersistedChanges: () => {
      ...
      return (
        serializeProjectSnapshot(takeWorkspaceSnapshot()) !== persistedSnapshot
      );
    },
    ...
    getActivePersistenceConflict: () => {
      ...
      return (
        state.persistenceConflictByProjectId[state.activeProjectId] ?? null
      );
    },
