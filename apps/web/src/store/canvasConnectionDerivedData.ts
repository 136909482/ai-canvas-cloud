...
export function syncConnectionDerivedNodeData(nodes: Node[], edges: Edge[]) {
  ...
  const generateNodes = nextNodes.filter(
    (node) => node.type === "generateNode",
  );
  ...
  for (const node of generateNodes) {
    ...
    const incomingMaskEdge = maskInputEnabled
      ? incomingImageEdges.find(
          (edge) =>
            edge.targetHandle === "mask" &&
            isConnectedImageSourceNode(
              getCanvasNodeById(nextNodes, edge.source),
            ),
        )
      : undefined;
    const nextMaskSourceNodeId =
      typeof incomingMaskEdge?.source === "string"
        ? incomingMaskEdge.source
        : null;
    const currentReferenceSourceOrder = Array.isArray(
      node.data?.referenceSourceOrder,
    )
      ? node.data.referenceSourceOrder.filter(
          (sourceId): sourceId is string => typeof sourceId === "string",
        )
      : [];
    const nextReferenceSourceOrder = [
      ...currentReferenceSourceOrder.filter((sourceId) =>
        connectedReferenceSourceIds.includes(sourceId),
      ),
      ...connectedReferenceSourceIds.filter(
        (sourceId) => !currentReferenceSourceOrder.includes(sourceId),
      ),
    ];
    const incomingTextEdge = edges.find(
      (edge) =>
        edge.target === node.id &&
        isTextSourceNode(getCanvasNodeById(nextNodes, edge.source)),
    );
    const {
      sourceId: connectedTextNodeId,
      text: connectedText,
      richPrompt: connectedRichPrompt,
    } = getTextFromSourceEdge(nextNodes, incomingTextEdge);

    const currentPrompt =
      typeof node.data?.prompt === "string" ? node.data.prompt : "";
    const currentConnectedId =
      typeof node.data?.connectedTextNode === "string"
        ? node.data.connectedTextNode
        : null;
    const currentMaskSourceNodeId =
      typeof node.data?.maskSourceNodeId === "string"
        ? node.data.maskSourceNodeId
        : null;
    const nextPatch: Record<string, unknown> = {};

    if (
      currentReferenceSourceOrder.length !== nextReferenceSourceOrder.length ||
      currentReferenceSourceOrder.some(
        (sourceId, index) => sourceId !== nextReferenceSourceOrder[index],
      )
    ) {
    ...
    if (connectedTextNodeId) {
      if (
        currentConnectedId !== connectedTextNodeId ||
        currentPrompt !== connectedText
      ) {
        nextPatch.connectedTextNode = connectedTextNodeId;
        nextPatch.prompt = connectedText;
        nextPatch.richPrompt =
          connectedRichPrompt ??
          createRichPromptDocumentFromText(connectedText);
      }
  ...
  for (const node of videoGenerateNodes) {
    ...
    const incomingTextEdge = incomingEdges.find(
      (edge) =>
        (edge.targetHandle === "input" || edge.targetHandle === "prompt") &&
        isTextSourceNode(getCanvasNodeById(nextNodes, edge.source)),
    );
    ...
    const nextReferenceSourceOrder = [
      ...currentReferenceSourceOrder.filter((sourceId) =>
        connectedImageSourceIds.includes(sourceId),
      ),
      ...connectedImageSourceIds.filter(
        (sourceId) => !currentReferenceSourceOrder.includes(sourceId),
      ),
    ];
    const { sourceId: connectedTextNodeId, text: connectedText } =
      getTextFromSourceEdge(nextNodes, incomingTextEdge);
    const currentPrompt =
      typeof node.data?.prompt === "string" ? node.data.prompt : "";
    const currentConnectedId =
      typeof node.data?.connectedTextNode === "string"
        ? node.data.connectedTextNode
        : null;
    const currentFirstFrameSourceNodeId =
      typeof node.data?.firstFrameSourceNodeId === "string"
        ? node.data.firstFrameSourceNodeId
        : null;
    const currentLastFrameSourceNodeId =
      typeof node.data?.lastFrameSourceNodeId === "string"
        ? node.data.lastFrameSourceNodeId
        : null;
  ...
  const imageEditNodes = nextNodes.filter(
    (node) => node.type === "imageEditNode",
  );
  ...
  for (const node of imageEditNodes) {
    ...
    const incomingBaseEdge = incomingEdges.find(
      (edge) =>
  ...
  const llmNodes = nextNodes.filter(
    (node) => node.type === "llmNode" || node.type === "llmFileNode",
  );
  ...
  for (const node of llmNodes) {
    ...
    const currentInputImageSourceOrder = Array.isArray(
      node.data?.inputImageSourceOrder,
    )
      ? node.data.inputImageSourceOrder.filter(
          (sourceId): sourceId is string => typeof sourceId === "string",
        )
      : [];
    const nextInputImageSourceOrder = [
      ...currentInputImageSourceOrder.filter((sourceId) =>
        connectedImageSourceIds.includes(sourceId),
      ),
      ...connectedImageSourceIds.filter(
        (sourceId) => !currentInputImageSourceOrder.includes(sourceId),
      ),
    ];
    const incomingTextEdge = incomingEdges.find((edge) =>
      isTextSourceNode(getCanvasNodeById(nextNodes, edge.source)),
    );
    const {
      sourceId: connectedTextNodeId,
    ...
    const currentInputText =
      typeof node.data?.inputText === "string" ? node.data.inputText : "";
    const currentConnectedId =
      typeof node.data?.connectedTextNode === "string"
        ? node.data.connectedTextNode
        : null;
    ...
    if (
      currentInputImageSourceOrder.length !==
        nextInputImageSourceOrder.length ||
      currentInputImageSourceOrder.some(
        (sourceId, index) => sourceId !== nextInputImageSourceOrder[index],
      )
    ) {
    ...
    if (connectedTextNodeId) {
      const nextInputRichPrompt =
        connectedRichPrompt ?? createRichPromptDocumentFromText(connectedText);
      const currentInputRichPrompt = sanitizeRichPrompt({
        richPrompt: node.data?.inputRichPrompt,
      });
      if (
        currentConnectedId !== connectedTextNodeId ||
        currentInputText !== connectedText ||
        currentInputRichPrompt !== nextInputRichPrompt
      ) {
        nextPatch.connectedTextNode = connectedTextNodeId;
  ...
  const textSplitterNodes = nextNodes.filter(
    (node) =>
  ...
  const imageCropNodes = nextNodes.filter(
    (node) => node.type === "imageCropNode",
  );
  ...
  for (const node of imageCropNodes) {
    ...
    const nextSourceImageNodeId =
      typeof incomingImageEdge?.source === "string"
