...
import { canDuplicateNode, cloneNodeForDuplicate } from "./canvasNodeClipboard";
import {
...
interface CanvasStore {
  ...
  addVideoGenerateNode: (preferredPosition?: {
    x: number;
    y: number;
  }) => string;
  ...
  addInlineTextSplitterNode: (preferredPosition?: {
    x: number;
    y: number;
  }) => string;
  ...
  deleteEdgesBySourceTargetHandle: (
    sourceId: string,
    targetId: string,
    targetHandle: string,
  ) => void;
  deleteEdgesBySourceTargetExceptHandle: (
    sourceId: string,
    targetId: string,
  ...
  updateGenerateNodeData: (
    id: string,
    patch: Partial<GenerateNodeData>,
  ) => void;
    ...
    changes
      .map((change) => {
        ...
        return changedNode?.type === "generatedPreviewNode"
          ? typeof changedNode.data?.sourceGenerateNodeId === "string"
            ? changedNode.data.sourceGenerateNodeId
            : null
          : null;
      })
  ...
  if (movedPreviewSourceIds.size > 0) {
    nextNodes = nextNodes.map((node) => {
      if (
        ...
        !movedPreviewSourceIds.has(
          typeof node.data?.sourceGenerateNodeId === "string"
            ? node.data.sourceGenerateNodeId
            : "",
        )
      ) {
        return node;
    ...
    changes
      .map((change) => {
        ...
        return changedNode?.type === "llmOutputTextNode"
          ? typeof changedNode.data?.sourceLLMNodeId === "string"
            ? changedNode.data.sourceLLMNodeId
            : null
          : null;
      })
      .filter((id): id is string => Boolean(id)),
  ...
  if (movedLLMOutputSourceIds.size > 0) {
    nextNodes = nextNodes.map((node) => {
      if (
        ...
        !movedLLMOutputSourceIds.has(
          typeof node.data?.sourceLLMNodeId === "string"
            ? node.data.sourceLLMNodeId
            : "",
        )
      ) {
        return node;
...
export const useCanvasStore = create<CanvasStore>((set, get) => ({
  ...
  replaceSnapshot: (snapshot) =>
    set(() => {
      const sanitized = sanitizeCanvasSnapshotForHistory(
        snapshot,
        normalizeVisualGroupNodes,
      );
    ...
    set((s) => {
      ...
      const shouldReplaceExistingTargetHandle =
        ...
        (targetNode?.type === "compareNode" ||
          (targetNode?.type === "generateNode" && isTextSource) ||
          (targetNode?.type === "videoGenerateNode" && isTextSource) ||
      ...
      const nextEdges = addEdge(
        ...
        shouldReplaceExistingTargetHandle
          ? s.edges.filter((edge) => {
              ...
              const existingSourceNode = getCanvasNodeById(
                s.nodes,
                edge.source,
              );
  ...
  addNodeByType: (type, preferredPosition) => {
    ...
    set((s) => {
      ...
      const position = findManualSpawnPosition(
        s.nodes,
        preferredPosition,
        registration.size,
      );
  ...
  addImageNode: (preferredPosition) =>
    get().addNodeByType("imageNode", preferredPosition),
  addVideoNode: (preferredPosition) =>
    get().addNodeByType("videoNode", preferredPosition),
  addVideoGenerateNode: (preferredPosition) =>
    get().addNodeByType("videoGenerateNode", preferredPosition),
  addImageCropNode: (preferredPosition) =>
    get().addNodeByType("imageCropNode", preferredPosition),
  addTextNode: (preferredPosition) =>
    get().addNodeByType("textNode", preferredPosition),
  ...
  addLLMFileNode: (preferredPosition) =>
    get().addNodeByType("llmFileNode", preferredPosition),
  addGeneratedPreviewNode: (preferredPosition) =>
    get().addNodeByType("generatedPreviewNode", preferredPosition),
  addCompareNode: (preferredPosition) =>
    get().addNodeByType("compareNode", preferredPosition),
  addTestImageNode: (preferredPosition) =>
    get().addNodeByType("testImageNode", preferredPosition),
  addPanoramaNode: (preferredPosition) =>
    get().addNodeByType("panoramaNode", preferredPosition),

  insertWorkflowTemplate: (template, preferredPosition) => {
    let insertedNodeIds: string[] = [];
    set((s) => {
      const anchor = findManualSpawnPosition(s.nodes, preferredPosition, {
        width: 320,
        height: 220,
      });
  ...
  groupSelectedNodes: () => {
    ...
    set((s) => {
      const groupedState = buildGroupedSelectionState(s.nodes, s.edges, () =>
        takeNextNodeId("groupNode"),
      );
      if (!groupedState) {
        return s;
      }
  ...
  attachNodeToGroup: (nodeId, _groupId, absolutePosition) =>
    set((s) => {
      ...
      const nextAbsolutePosition =
        absolutePosition ?? getAbsoluteNodePosition(s.nodes, targetNode);
  ...
  ungroupNode: (groupId) =>
    set((s) => {
      const ungroupedState = buildUngroupedSelectionState(
        s.nodes,
        s.edges,
        groupId,
      );
  ...
  deleteSelectedElements: () =>
    set((s) => {
      const deletedState = buildSelectedElementsDeletedGraphState(
        s.nodes,
        s.edges,
      );
      if (!deletedState) {
  ...
  duplicateSelectedNode: () => {
    ...
    set((s) => {
      ...
      const duplicatedNode = cloneNodeForDuplicate(
        sourceNode,
        s.nodes,
        takeNextNodeId,
      );
  ...
  syncTextSplitterOutputs: (id) =>
    set((s) => {
      const splitterNode = s.nodes.find(
        (node) => node.id === id && node.type === "textSplitterNode",
      );
      ...
      const inputText =
        typeof splitterNode.data?.inputText === "string"
          ? splitterNode.data.inputText
          : "";
      const rawSeparator =
        typeof splitterNode.data?.separator === "string"
          ? splitterNode.data.separator
          : "";
      const separator = rawSeparator
        .replaceAll("\\n", "\n")
        .replaceAll("\\t", "\t");
      const outputNodeIds = getOrderedStringIds(
        splitterNode.data?.outputNodeIds,
      );
      ...
      const nextNodes = layoutTextSplitterOutputNodesInContext(
        outputState.nodes,
        id,
      );
  ...
  syncInlineTextSplitterParts: (id) =>
    set((s) => {
      const splitterNode = s.nodes.find(
        (node) => node.id === id && node.type === "inlineTextSplitterNode",
      );
      ...
      const separator = rawSeparator
        ...
        .replaceAll("\\t", "\t");
      const parts = (
        separator ? inputText.split(separator) : [inputText]
  ...
  createGeneratedPreviewNode: (sourceGenerateNodeId, preview) => {
    let previewId = "";
    set((s) => {
      const sourceNode = s.nodes.find(
        (node) => node.id === sourceGenerateNodeId,
      );
      if (
        getCanvasNodeRegistration(sourceNode?.type)?.outputLayout !==
        "generated-preview"
      ) {
        return s;
      }
  ...
  createGeneratedVideoNode: (sourceVideoGenerateNodeId, video) => {
    ...
    set((s) => {
      const sourceNode = s.nodes.find(
        (node) => node.id === sourceVideoGenerateNodeId,
      );
      ...
      const position = {
        ...
        y:
          ...
          Math.max((sourceHeight - DEFAULT_VIDEO_NODE_HEIGHT) / 2, 0),
      };
  ...
  createLLMOutputTextNode: (sourceLLMNodeId, outputNode) => {
    ...
    set((s) => {
      ...
      const hasExistingEdge = s.edges.some(
        (edge) =>
          edge.source === sourceLLMNodeId && edge.target === outputNodeId,
      );
  ...
  runImageCropNode: async (id, projectId) => {
    const cropNode = get().nodes.find(
      (node) => node.id === id && node.type === "imageCropNode",
    );
    if (!cropNode) {
      return;
    }
    ...
    const sourceImageNodeId =
      typeof cropNode.data?.sourceImageNodeId === "string"
        ? cropNode.data.sourceImageNodeId
        : null;
    ...
    try {
      const rowCount = clampCropSegmentCount(
        typeof cropNode.data?.rowCount === "number"
          ? cropNode.data.rowCount
          : DEFAULT_IMAGE_CROP_ROWS,
        DEFAULT_IMAGE_CROP_ROWS,
      );
      const columnCount = clampCropSegmentCount(
        typeof cropNode.data?.columnCount === "number"
          ? cropNode.data.columnCount
          : DEFAULT_IMAGE_CROP_COLUMNS,
        DEFAULT_IMAGE_CROP_COLUMNS,
      );
      const horizontalCuts = normalizeCropCuts(
        cropNode.data?.horizontalCuts,
        rowCount,
      );
      const verticalCuts = normalizeCropCuts(
        cropNode.data?.verticalCuts,
        columnCount,
      );
      ...
      set((s) => {
        const latestCropNode = s.nodes.find(
          (node) => node.id === id && node.type === "imageCropNode",
        );
        if (!latestCropNode) {
          return s;
        ...
        const outputState = buildImageCropOutputState({
          ...
          edges: s.edges,
          cropNodeId: id,
          existingPreviewIds: getOrderedStringIds(
            latestCropNode.data?.outputPreviewNodeIds,
          ),
          previewResults,
          rowCount,
          columnCount,
      ...
      const errorMessage =
        error instanceof Error ? error.message : "图片裁切失败，请稍后重试";
      set((s) => ({
        nodes: s.nodes.map((node) =>
          node.id === id && node.type === "imageCropNode"
            ? {
  ...
  arrangeSelectedNodes: (direction) =>
    set((s) => {
      const { normalizedNodes, targets, layoutEdges, memberIdsByGroupId } =
        buildGroupAwareLayoutTargets(
          s.nodes,
          s.edges,
      ...
      return {
        nodes: applyGroupAwareLayoutPositions(
          s.nodes,
          normalizedNodes,
          memberIdsByGroupId,
          positions,
  ...
  arrangeAllNodes: (direction) =>
    set((s) => {
      const { normalizedNodes, targets, layoutEdges, memberIdsByGroupId } =
        buildGroupAwareLayoutTargets(s.nodes, s.edges, () => true);
      if (targets.length < 2) return {};
      const positions = computeAutoLayout(targets, layoutEdges, direction);
      return {
        nodes: applyGroupAwareLayoutPositions(
  ...
  arrangeConnectedGraphFromSelection: () => {
    ...
    set((s) => {
      ...
      const connectedNodeIds = buildConnectedComponentNodeIds(
        topLevelNodes,
        s.edges,
        selectedNode.id,
      );
      ...
      const targets = topLevelNodes.filter((node) =>
        connectedNodeIds.has(node.id),
      );
      const positions = computeFocusedAutoLayout(
        targets,
        s.edges,
        selectedNode.id,
      );
