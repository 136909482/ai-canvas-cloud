...
export function sanitizeRichPrompt(
  data: Record<string, unknown> | undefined,
): RichPromptDocument | null {
  const richPrompt = data?.richPrompt;
  if (
    !richPrompt ||
    typeof richPrompt !== "object" ||
    Array.isArray(richPrompt)
  ) {
    return null;
  }
...
export function createImageNodeData(data?: Node["data"]): ImageNodeData {
  return {
    ...
    imageAsset: cloneWorkspaceImageAsset(
      data?.imageAsset,
    ) as ImageNodeData["imageAsset"],
    name: typeof data?.name === "string" ? data.name : undefined,
    imageNaturalWidth:
      typeof data?.imageNaturalWidth === "number" &&
      Number.isFinite(data.imageNaturalWidth)
        ? data.imageNaturalWidth
        : undefined,
    imageNaturalHeight:
      typeof data?.imageNaturalHeight === "number" &&
      Number.isFinite(data.imageNaturalHeight)
...
export function createTestImageNodeData(
  data?: Node["data"],
): TestImageNodeData {
  return {
    imageUrl: typeof data?.imageUrl === "string" ? data.imageUrl : null,
    imageAsset: cloneWorkspaceImageAsset(
      data?.imageAsset,
    ) as TestImageNodeData["imageAsset"],
...
export function createVideoNodeData(data?: Node["data"]): VideoNodeData {
  return {
    ...
    videoAsset: cloneWorkspaceImageAsset(
      data?.videoAsset,
    ) as VideoNodeData["videoAsset"],
    name: typeof data?.name === "string" ? data.name : undefined,
    duration:
      typeof data?.duration === "number" && Number.isFinite(data.duration)
        ...
        : 0,
    videoWidth:
      typeof data?.videoWidth === "number" && Number.isFinite(data.videoWidth)
        ? data.videoWidth
        : 0,
    videoHeight:
      typeof data?.videoHeight === "number" && Number.isFinite(data.videoHeight)
        ? data.videoHeight
        : 0,
    status:
      data?.status === "queued" ||
...
export function createVideoGenerateNodeData(
  data?: Node["data"],
): VideoGenerateNodeData {
  const ratio = data?.ratio === "9:16" ? "9:16" : "16:9";
  const duration = data?.duration === "10s" ? "10s" : "5s";
  const resolution =
    data?.resolution === "480p" || data?.resolution === "1080p"
      ? data.resolution
      : "720p";
  ...
  return {
    ...
    mode:
      data?.mode === "keyframes" || data?.mode === "reference"
        ? data.mode
        : "text",
    ratio,
    duration,
    resolution,
    connectedTextNode:
      typeof data?.connectedTextNode === "string"
        ? data.connectedTextNode
        : null,
    referenceSourceOrder: getOrderedStringIds(data?.referenceSourceOrder),
    firstFrameSourceNodeId:
      typeof data?.firstFrameSourceNodeId === "string"
        ? data.firstFrameSourceNodeId
        : null,
    lastFrameSourceNodeId:
      typeof data?.lastFrameSourceNodeId === "string"
        ? data.lastFrameSourceNodeId
        : null,
    status:
      data?.status === "queued" ||
      data?.status === "generating" ||
...
export function createImageEditNodeData(
  data?: Node["data"],
): ImageEditNodeData {
  return {
    sourceImageNodeId:
    ...
    maskDataUrl:
      typeof data?.maskDataUrl === "string" ? data.maskDataUrl : null,
    maskUpdatedAt:
      typeof data?.maskUpdatedAt === "number" ? data.maskUpdatedAt : null,
    brushSize:
      typeof data?.brushSize === "number"
        ? Math.max(4, Math.min(96, data.brushSize))
        : 28,
...
export function createImageCropNodeData(
  data?: Node["data"],
): ImageCropNodeData {
