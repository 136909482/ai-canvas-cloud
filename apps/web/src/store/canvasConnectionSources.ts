...
export function getGenerateReferenceSourceNodes(nodes: Node[], nodeId: string) {
  ...
  const directReferenceSourceIds = getOrderedStringIds(
    targetNode?.data?.referenceSourceOrder,
  );
  const mentionedReferenceSourceIds = Array.from(
    collectMentionedSourceIds(sanitizeRichPrompt(targetNode?.data)),
  );
  return getOrderedImageSourceNodes(nodes, [
    ...directReferenceSourceIds,
    ...mentionedReferenceSourceIds.filter(
      (sourceId) => !directReferenceSourceIds.includes(sourceId),
    ),
  ]);
}
...
export function getGenerateMaskSourceNode(nodes: Node[], nodeId: string) {
  ...
  const maskSourceNodeId =
    typeof targetNode?.data?.maskSourceNodeId === "string"
      ? targetNode.data.maskSourceNodeId
      : "";
...
export function getImageEditReferenceSourceNodes(
  ...
  return getOrderedImageSourceNodes(
    nodes,
    getOrderedStringIds(targetNode?.data?.referenceSourceOrder),
  );
}
...
export function getLLMInputImageSourceNodes(nodes: Node[], nodeId: string) {
  ...
  return getOrderedImageSourceNodes(
    nodes,
    getOrderedStringIds(targetNode?.data?.inputImageSourceOrder),
  );
}
...
export function getTextFromSourceEdge(nodes: Node[], edge: Edge | undefined) {
  if (!edge?.source) {
    return {
      ...
      text: "",
      richPrompt: null as RichPromptDocument | null,
    };
  }
  ...
  return {
    ...
    richPrompt:
      sourceNode.type === "textNode"
        ? sanitizeRichPrompt(sourceNode.data)
        : null,
  };
}
