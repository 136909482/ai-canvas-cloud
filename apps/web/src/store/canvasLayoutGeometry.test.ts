...
test("moves visual group members during every active drag frame", () => {
  ...
  const firstFrame = applyVisualNodeChanges(nodes, [
    moveGroup(groupId, 40, 30, true),
  ]);
  assert.deepEqual(firstFrame.find((node) => node.id === "member")?.position, {
    x: 140,
    y: 130,
  });
  assert.deepEqual(firstFrame.find((node) => node.id === "outside")?.position, {
    x: 600,
    y: 100,
  });

  const secondFrame = applyVisualNodeChanges(firstFrame, [
    moveGroup(groupId, 75, 55, true),
  ]);
  assert.deepEqual(secondFrame.find((node) => node.id === "member")?.position, {
    x: 175,
    y: 155,
  });

  const settled = applyVisualNodeChanges(secondFrame, [
    moveGroup(groupId, 75, 55, false),
  ]);
  assert.deepEqual(settled.find((node) => node.id === "member")?.position, {
    x: 175,
    y: 155,
  });
});
...
test("applies queued group drag frames incrementally instead of duplicating the final delta", () => {
  ...
  assert.deepEqual(moved.find((node) => node.id === groupId)?.position, {
    x: 60,
    y: 40,
  });
  assert.deepEqual(
    moved.find((node) => node.id === "queued-member")?.position,
    { x: 150, y: 130 },
  );
