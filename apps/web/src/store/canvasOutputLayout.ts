...
function layoutGeneratedPreviewNodes(
  ...
  const relatedPreviewNodes = nodes
    .filter(
      (node): node is Node<GeneratedPreviewNodeData> =>
        ...
        node.data?.sourceGenerateNodeId === sourceGenerateNodeId,
    )
    .sort((left, right) => {
      const leftCreatedAt =
  ...
  const autoPreviewNodes = relatedPreviewNodes.filter(
    (node) => node.data?.layoutMode !== "manual",
  );
  const manualPreviewNodes = relatedPreviewNodes.filter(
    (node) => node.data?.layoutMode === "manual",
  );
  ...
  const slotWidth = Math.max(
    ...
    ...relatedPreviewNodes.map((node) =>
      typeof node.width === "number" ? node.width : DEFAULT_PREVIEW_NODE_WIDTH,
    ),
  );
  ...
  const cropColumns =
    sourceNode.type === "imageCropNode"
      ? clampCropSegmentCount(
          typeof sourceNode.data?.columnCount === "number"
            ? sourceNode.data.columnCount
            : DEFAULT_IMAGE_CROP_COLUMNS,
          DEFAULT_IMAGE_CROP_COLUMNS,
        )
  ...
  const totalGridHeight =
    cropRows * slotHeight + Math.max(0, cropRows - 1) * PREVIEW_LAYOUT_GAP_X;
  ...
  const nextPositionById = new Map(
    autoPreviewNodes.map((previewNode, index) => {
      const columnIndex =
        sourceNode.type === "imageCropNode" ? index % cropColumns : index;
      const rowIndex =
        sourceNode.type === "imageCropNode"
          ? Math.floor(index / cropColumns)
          : 0;
...
export function layoutGeneratedPreviewNodesInContext(
  nodes: Node[],
  sourceGenerateNodeId: string,
) {
  const laidOutNodes = layoutGeneratedPreviewNodes(nodes, sourceGenerateNodeId);
  const sourceNode = laidOutNodes.find(
...
function layoutLLMOutputTextNodes(nodes: Node[], sourceLLMNodeId: string) {
  ...
  const relatedOutputNodes = nodes
    .filter(
      (node): node is Node<LLMOutputTextNodeData> =>
        ...
        node.data?.sourceLLMNodeId === sourceLLMNodeId,
    )
    .sort((left, right) => {
      const leftCreatedAt =
        typeof left.data?.createdAt === "number" ? left.data.createdAt : 0;
  ...
  const autoOutputNodes = relatedOutputNodes.filter(
    (node) => node.data?.layoutMode !== "manual",
  );
  const manualOutputNodes = relatedOutputNodes.filter(
    (node) => node.data?.layoutMode === "manual",
  );
...
export function layoutLLMOutputTextNodesInContext(
  nodes: Node[],
...
export function layoutTextSplitterOutputNodesInContext(
  nodes: Node[],
  sourceSplitterNodeId: string,
) {
  ...
  const outputNodes = outputNodeIds
    .map((outputNodeId) =>
      nodes.find(
        (node) => node.id === outputNodeId && node.type === "textNode",
      ),
    )
  ...
  const slotHeight = Math.max(
    ...
    ...outputNodes.map((node) =>
      typeof node.height === "number" ? node.height : 140,
    ),
  );
  const totalHeight =
    outputNodes.length * slotHeight + Math.max(0, outputNodes.length - 1) * 20;
  const startX = sourceNode.position.x + sourceWidth + 72;
  const startY =
    sourceNode.position.y + Math.min(0, (sourceHeight - totalHeight) / 2);
  const nextPositionById = new Map(
...
export function applyDragStopSideEffects(
  ...
  for (const nodeId of completedDraggedNodeIds) {
    ...
    if (draggedNode.type === "generatedPreviewNode") {
      const sourceGenerateNodeId =
        typeof draggedNode.data?.sourceGenerateNodeId === "string"
    ...
    if (draggedNode.type === "llmOutputTextNode") {
      const sourceLLMNodeId =
        typeof draggedNode.data?.sourceLLMNodeId === "string"
  ...
  for (const generateNodeId of affectedGenerateNodeIds) {
    laidOutNodes = layoutGeneratedPreviewNodesInContext(
      laidOutNodes,
      generateNodeId,
    );
  }
