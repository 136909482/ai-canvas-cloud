...
export function buildTextSplitterOutputState({
  ...
  const parts = (separator ? inputText.split(separator) : [inputText]).filter(
  ...
  const removedOutputIds = new Set(
    outputNodeIds.filter(
      (outputNodeId) => !keptOutputIds.includes(outputNodeId),
    ),
  );
  const outputTextById = new Map(
    keptOutputIds.map((outputNodeId, index) => [outputNodeId, parts[index]]),
  );
  const existingNodes = nodes
    ...
    .map((node) => {
      ...
      return buildTextSplitterOutputTextNode(
        node.id,
        outputText,
        `片段 ${outputIndex + 1}`,
      );
  ...
  const keptEdges = edges.filter(
    (edge) =>
      !removedOutputIds.has(edge.source) && !removedOutputIds.has(edge.target),
  );
  ...
  const outputEdges = keptOutputIds
    .filter(
      (outputNodeId) =>
        !existingEdgeKeys.has(`${splitterNodeId}->${outputNodeId}`),
    )
    .map((outputNodeId) => ({
