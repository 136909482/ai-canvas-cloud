...
import { hydrateCanvasLocalModelReferences, prepareCanvasForCloud } from './cloudModelReferences.ts'
...
test('Cloud graph replaces private model identity and removes local task/provider state', () => {
  ...
  assert.equal(JSON.stringify(cloud).includes('private video upstream response'), false)
  assert.equal(JSON.stringify(cloud).includes('private chat upstream response'), false)
})

test('same-device hydration resolves aliases while another device keeps them unavailable', () => {
  const reference = createLocalModelReference()
  const cloud: CanvasSnapshot = {
    nodes: [{ id: 'llm-1', type: 'llmNode', position: { x: 0, y: 0 }, data: { model: reference } }],
    edges: [],
  }

  const sameDevice = hydrateCanvasLocalModelReferences(cloud, (value) => value === reference ? 'private-chat-model' : null)
