...
test('cloud graph operation batching respects the server operation limit', () => {
  ...
  const batches = buildProjectGraphOperationBatches({ nodes: [], edges: [] }, operations)
...
test('cloud graph operation batching preserves node topology between chunks', () => {
  ...
  assert.deepEqual(batches.map((batch) => batch[0]), [
    { type: 'deleteNode', nodeId: 'child' },
    { type: 'deleteNode', nodeId: 'parent' },
    upsertParent,
    upsertChild,
...
test('cloud graph operation batches can be applied to advance the local baseline', () => {
  const baselineEdge: Edge = { id: 'edge-a-b', source: 'node-a', target: 'node-b', data: {} }
  const baseline: { nodes: Node[]; edges: Edge[] } = {
    nodes: [
  ...
  assert.deepEqual(next.nodes.find((node) => node.id === 'node-c')?.data, { text: 'new' })
})

test('cloud graph change overlap detection is conservative around nodes and edges', () => {
  assert.equal(
    doProjectGraphChangesOverlap(
      [{ type: 'upsertNode', node: { id: 'local', nodeType: 'textNode', position: { x: 0, y: 0 }, dataSchemaVersion: 1, data: {} } }],
      [{
        sequence: 2,
        baseVersion: 1,
        resultVersion: 2,
        ...
        batchId: 'remote_1',
        source: 'user',
        operations: [{ type: 'upsertNode', node: { id: 'remote', nodeType: 'textNode', position: { x: 1, y: 1 }, dataSchemaVersion: 1, data: {} } }],
  ...
  assert.equal(
    doProjectGraphChangesOverlap(
      [{ type: 'upsertEdge', edge: { id: 'edge-local', source: 'shared-node', target: 'local' } }],
      [{
        sequence: 3,
        baseVersion: 2,
        resultVersion: 3,
...
test('cloud graph changes can advance a baseline before retrying local operations', () => {
  const baseline: { nodes: Node[]; edges: Edge[] } = {
    nodes: [{ id: 'node-a', type: 'textNode', position: { x: 0, y: 0 }, data: {} }],
    edges: [],
  }
  ...
  assert.deepEqual(advanced.nodes.map((node) => node.id), ['node-a', 'node-b'])
  assert.deepEqual(advanced.nodes.find((node) => node.id === 'node-b')?.data, { remote: true })
})
