...
export function createCloudAssetUrlCache(options: CloudAssetUrlCacheOptions) {
  ...
  const refreshSkewMs = options.refreshSkewMs ?? CLOUD_ASSET_URL_REFRESH_SKEW_MS
  ...
  function resolve(assetId: string) {
    ...
    const request = options.loadAssetUrl(normalizedAssetId)
      .then((response) => {
        if (requestGeneration !== generation) {
          throw new Error('Cloud asset URL cache was cleared while the request was in flight')
        }

        if (typeof response.assetId !== 'string' || response.assetId.toLowerCase() !== normalizedAssetId) {
          throw new Error('Cloud asset URL response does not match the requested asset')
        }
        ...
        if (
          ...
          throw new Error('Cloud asset URL response is already expired or invalid')
        }
