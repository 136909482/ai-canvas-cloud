...
export function describeCloudAssetWrite(
  ...
  return {
    projectId:
      input.projectId === undefined ? inferredProjectId : input.projectId,
    assetKind,
...
export function createCloudAssetUploader(options: CloudAssetUploaderOptions) {
  const fetchDirectUpload =
    options.fetchDirectUpload ?? ((url, init) => fetch(url, init));
