...
function sanitizeRuntimeState(data: Record<string, unknown>) {
  ...
  if (sanitized.status === 'queued' || sanitized.status === 'generating' || sanitized.status === 'running') {
...
export function prepareCanvasForCloud(
  ...
  return {
    nodes: snapshot.nodes.map((node) => {
      ...
      if (modelId) data.model = isLocalModelReference(modelId) ? modelId : ensureReference(modelId)
