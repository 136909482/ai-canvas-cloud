...
export const VideoNode = memo(function VideoNode({
  ...
  const handleResizeEnd: OnResizeEnd = useCallback(
    (_event, params) => {
      if (!data.videoUrl || videoInfo.width === 0 || videoInfo.height === 0)
        return;
      ...
      const contentW = Math.max(
        ...
        MIN_VIDEO_NODE_WIDTH - PADDING_X,
      );
  ...
  return (
    <>
      {selected ? (
        <StableNodeToolbar
          isVisible={!dragging && Boolean(data.videoUrl) ? undefined : false}
          position={Position.Top}
          offset={10}
        >
          <div
            className={`nodrag nopan flex items-center gap-1 p-[5px] ${themeClasses.nodeToolbarPanel}`}
          >
            <button
              type="button"
