...
export const TextSplitterNode = memo(function TextSplitterNode({
  id,
  data,
  selected,
}: TextSplitterNodeProps) {
  const updateNodeData = useCanvasStore((s) => s.updateNodeData);
  const deleteNode = useCanvasStore((s) => s.deleteNode);
  const syncTextSplitterOutputs = useCanvasStore(
    (s) => s.syncTextSplitterOutputs,
  );
  const runTracked = useHistoryStore((s) => s.runTracked);
  const inputText = typeof data.inputText === "string" ? data.inputText : "";
  const separator =
    typeof data.separator === "string" ? data.separator : "\\n\\n";
  const outputCount = Array.isArray(data.outputNodeIds)
    ? data.outputNodeIds.length
    : 0;
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2.5 p-3">
        ...
        <div className="grid grid-cols-[1fr_auto] items-end gap-2">
          <label className="min-w-0">
            <span
              className={`mb-1 block text-[10px] font-medium ${themeClasses.nodeHint}`}
            >
              {UI_TEXT.separator}
            </span>
            <input
        ...
        <div className="flex min-h-4 items-center justify-between gap-2 px-0.5 text-[10px] text-[var(--text-muted)]">
          ...
          {data.errorMsg ? (
            <span className="truncate text-red-300">{data.errorMsg}</span>
          ) : null}
        </div>
      </div>
