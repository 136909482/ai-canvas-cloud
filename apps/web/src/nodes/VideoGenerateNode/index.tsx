...
function buildReferenceImageAsset(
...
export const VideoGenerateNode = memo(function VideoGenerateNode({
  id,
  data,
  selected,
  ...
  const deleteEdgesBySourceTarget = useCanvasStore(
    (s) => s.deleteEdgesBySourceTarget,
  );
  ...
  const mode =
    data.mode === "keyframes" || data.mode === "reference" ? data.mode : "text";
  const ratio = data.ratio || "16:9";
  const duration = data.duration || "5s";
  const resolution = data.resolution || "720p";
  const modeLabel =
    MODES.find((option) => option.value === mode)?.label ?? MODES[0].label;
  ...
  const referenceSourceOrderKey = Array.isArray(data.referenceSourceOrder)
    ? data.referenceSourceOrder
        .filter((sourceId): sourceId is string => typeof sourceId === "string")
        .join(REFERENCE_IMAGE_KEY_SEPARATOR)
    : "";
  const selectReferenceImageKeys = useMemo(() => {
    const orderedIds = referenceSourceOrderKey
      ? referenceSourceOrderKey.split(REFERENCE_IMAGE_KEY_SEPARATOR)
      : [];
    return (state: {
  ...
  const updateVideoSettings = (patch: Record<string, unknown>) => {
    runTracked(() =>
      updateNodeData(id, { ...patch, errorMsg: "", status: "idle" }),
    );
  };
  ...
  return (
    ...
    >
      ...
      <NodeHeader
        ...
        right={
          <span
            className={`ml-auto ${themeClasses.nodeBadge} ${themeClasses.nodeBadgeViolet} font-semibold`}
          >
            {mode === "text" ? (
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-1.5 p-2.5">
              ...
              <div className={themeClasses.nodeAssetStrip}>
                <div className="flex items-center gap-1">
                  ...
                  {mode === "keyframes" && referenceImages.length > 2 && (
                    <span
                      className={`${themeClasses.nodeBadge} border-[var(--border-subtle)] bg-[var(--control-bg)] text-[var(--text-muted)] normal-case tracking-normal`}
                    >
        ...
        <textarea
          ...
          placeholder={
            mode === "text" || visibleImages.length > 0 || isConnected
              ? UI_TEXT.placeholder
              : `${imageHint}，或直接输入提示词...`
          }
          rows={5}
                  ...
                  <div className="space-y-3">
                    <SettingsSection title="生成方式">
                      <SettingsSegment
                        ...
                        onChange={(value) =>
                          updateVideoSettings({ mode: value })
                        }
                        ...
                        renderOption={(value) => {
                          const optionLabel =
                            MODES.find((option) => option.value === value)
                              ?.label ?? value;
                          return (
                            <>
                              {value === "text" ? (
                    ...
                    <SettingsSection title="比例">
                      <SettingsSegment
                        ...
                        onChange={(value) =>
                          updateVideoSettings({ ratio: value })
                        }
                        groupClassName="h-10 rounded-[9px] border-[color-mix(in_srgb,var(--text-primary)_11%,transparent)] bg-[color-mix(in_srgb,var(--control-bg)_62%,transparent)] p-1 shadow-none"
                    ...
                    <SettingsSection title="清晰度">
                      <SettingsSegment
                        ...
                        onChange={(value) =>
                          updateVideoSettings({ resolution: value })
                        }
                        groupClassName="h-10 rounded-[9px] border-[color-mix(in_srgb,var(--text-primary)_11%,transparent)] bg-[color-mix(in_srgb,var(--control-bg)_62%,transparent)] p-1 shadow-none"
                        buttonClassName="rounded-[7px] text-[11px] hover:bg-[color-mix(in_srgb,var(--text-primary)_5%,transparent)]"
                        slider
                    ...
                    <SettingsSection title="生成时长">
                      <SettingsSegment
                        ...
                        onChange={(value) =>
                          updateVideoSettings({ duration: value })
                        }
                        groupClassName="h-10 rounded-[9px] border-[color-mix(in_srgb,var(--text-primary)_11%,transparent)] bg-[color-mix(in_srgb,var(--control-bg)_62%,transparent)] p-1 shadow-none"
                        buttonClassName="rounded-[7px] text-[11px] hover:bg-[color-mix(in_srgb,var(--text-primary)_5%,transparent)]"
                        slider
