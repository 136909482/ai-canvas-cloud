...
import {
  getMaxInputFiles,
  LLM_INPUT_FILE_ACCEPT,
  readLLMInputFiles,
} from "@/features/llm/inputFiles";
...
function ModelOptionIcon({
  ...
  const normalized = `${model.displayName} ${model.modelId}`.toLowerCase();
  const isOpenAIModel =
    normalized.includes("gpt") || model.provider === "openai";
...
function normalizeInputFiles(input: unknown) {
  return Array.isArray(input)
    ? input.filter((file): file is LLMInputFileData =>
        Boolean(
          file &&
          typeof file === "object" &&
          typeof (file as LLMInputFileData).id === "string",
        ),
      )
    : [];
}
...
export const LLMFileNode = memo(function LLMFileNode({
  id,
  data,
  selected,
}: LLMFileNodeProps) {
  ...
  const inputImageSourceOrderKey = Array.isArray(data.inputImageSourceOrder)
    ? data.inputImageSourceOrder
        .filter((sourceId): sourceId is string => typeof sourceId === "string")
        .join(INPUT_IMAGE_KEY_SEPARATOR)
    : "";
  const selectInputImageKeys = useMemo(() => {
    const orderedIds = inputImageSourceOrderKey
      ? inputImageSourceOrderKey.split(INPUT_IMAGE_KEY_SEPARATOR)
      : [];
    return (state: {
  ...
  const { updateNodeData, deleteNode, deleteEdgesBySourceTarget } =
    useCanvasStore(
      useShallow((s) => ({
  ...
  const inputImages = useMemo<InputImageItem[]>(
    () =>
      inputImageKeys
        .map(decodeInputImageKey)
        .filter((item): item is InputImageItem => Boolean(item)),
    [inputImageKeys],
  );
  ...
  const inputFiles = useMemo(
    () => normalizeInputFiles(data.inputFiles),
    [data.inputFiles],
  );
  ...
  const syncRichPromptToStore = (
    ...
    if (
      nextInstruction !== (data.instructionPrompt || "") ||
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const nextFiles = await readLLMInputFiles(
        selectedFiles,
        inputFiles.length,
      );
      runTracked(() =>
  ...
  const handleRun = async () => {
    ...
    const nextInstruction = (
      latestInstructionDraftRef.current ||
      data.instructionPrompt ||
      ""
    ).trim();
    if (!nextInstruction) {
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 p-2.5">
                ...
                >
                  ...
                  <button
                    ...
                    onClick={(event) => {
                      ...
                      runTracked(() =>
                        deleteEdgesBySourceTarget(inputImage.sourceId, id),
                      );
                    }}
                  >
        ...
        <div className="relative flex min-h-[120px] flex-1">
          <RichPromptEditor
            value={
              richPromptDraft ??
              createRichPromptDocumentFromText(instructionDraft)
            }
        ...
        <div className={themeClasses.nodeFooter}>
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              title={
                inputFileCount > 0
                  ? `${UI_TEXT.uploadMoreFiles}（${inputFileCount}/${maxInputFiles}）`
                  : UI_TEXT.uploadFiles
              }
              aria-label={
                inputFileCount > 0
          ...
          {data.errorMsg && (
            <p
              className={`${themeClasses.nodeInlineNotice} ${data.status === "success" ? themeClasses.nodeWarningText : themeClasses.nodeErrorText}`}
            >
              {data.errorMsg}
            </p>
          )}
