...
function replaceCut(cuts: number[], index: number, nextValue: number) {
  return cuts.map((value, cutIndex) =>
    cutIndex === index ? nextValue : value,
  );
}

export const ImageCropNode = memo(function ImageCropNode({
  id,
  data,
  selected,
}: ImageCropNodeProps) {
  ...
  const [stageSize, setStageSize] = useState({
    width: 0,
    height: MIN_PREVIEW_STAGE_HEIGHT,
  });

  const rowCount = clampCropSegmentCount(
    typeof data.rowCount === "number" ? data.rowCount : undefined,
  );
  const columnCount = clampCropSegmentCount(
    typeof data.columnCount === "number" ? data.columnCount : undefined,
  );
  const menuRowCount = clamp(
    rowCount,
    MIN_IMAGE_CROP_MENU_SEGMENTS,
    MAX_IMAGE_CROP_MENU_SEGMENTS,
  );
  const menuColumnCount = clamp(
    columnCount,
    MIN_IMAGE_CROP_MENU_SEGMENTS,
    MAX_IMAGE_CROP_MENU_SEGMENTS,
  );
  const horizontalCuts = useMemo(
    () => normalizeCropCuts(data.horizontalCuts, rowCount),
    [data.horizontalCuts, rowCount],
  );
  const verticalCuts = useMemo(
    () => normalizeCropCuts(data.verticalCuts, columnCount),
    [columnCount, data.verticalCuts],
  );
  ...
  const setRowCount = (nextValue: number | string) => {
    const safeRowCount = clamp(
      Math.round(Number(nextValue)),
      MIN_IMAGE_CROP_MENU_SEGMENTS,
      MAX_IMAGE_CROP_MENU_SEGMENTS,
    );
  ...
  const updateDraggedLine = useCallback(
    (clientX: number, clientY: number) => {
      ...
      if (dragState.axis === "vertical") {
        ...
        const previousCut =
          dragState.index > 0 ? currentCuts[dragState.index - 1] : 0;
        const nextCut =
          dragState.index < currentCuts.length - 1
            ? currentCuts[dragState.index + 1]
            : 1;
  ...
  return (
    ...
    >
      ...
      <NodeHeader
        icon={
          <ScissorsLineDashed className="h-3.5 w-3.5 text-[var(--text-muted)]" />
        }
        title={UI_TEXT.title}
        right={
              ...
              <div className="pointer-events-none absolute inset-0">
                {verticalCuts.map((cut, index) => (
                  <button
                    ...
                    type="button"
                    onPointerDown={(event) =>
                      startLineDrag("vertical", index, event)
                    }
                ...
                {horizontalCuts.map((cut, index) => (
                  <button
                    ...
                    type="button"
                    onPointerDown={(event) =>
                      startLineDrag("horizontal", index, event)
                    }
