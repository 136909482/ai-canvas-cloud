...
const BASE_WRAPPER_CLASS_NAME =
  "min-h-0 h-full w-full overflow-hidden rounded-xl border border-transparent bg-transparent transition-[border-color,box-shadow,background-color] duration-150 focus-within:border-violet-400/85 focus-within:shadow-[0_0_0_1px_rgba(167,139,250,0.26)]";
const BASE_TEXTAREA_CLASS_NAME =
  "nowheel nodrag nopan min-h-0 h-full w-full resize-none overflow-y-auto overscroll-contain bg-transparent px-3 py-2.5 text-sm leading-6 text-[var(--text-primary)] outline-none placeholder:text-[var(--text-muted)]";
...
export function SharedTextEditor({
  ...
  return (
    <div
      className={
        wrapperClassName
          ? `${BASE_WRAPPER_CLASS_NAME} ${wrapperClassName}`
          : BASE_WRAPPER_CLASS_NAME
      }
    >
      <textarea
        ref={textRef}
        ...
        className={
          className
            ? `${BASE_TEXTAREA_CLASS_NAME} ${className}`
