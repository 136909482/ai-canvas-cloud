...
const PanoramaViewer = lazy(() =>
  import("./PanoramaViewer").then((module) => ({
    default: module.PanoramaViewer,
  })),
);

export const PanoramaNode = memo(function PanoramaNode({
  id,
  data,
  selected,
  dragging,
  ...
  const sourceImage = useCanvasStore(
    useShallow((state) => {
      ...
      const node =
        candidate && isImageSourceNodeType(candidate.type) ? candidate : null;
  ...
  const labelText = hasSourceImage
    ? `${UI_TEXT.title} · 已连接`
    : UI_TEXT.title;
  ...
  return (
    <>
      {selected ? (
        <StableNodeToolbar
          isVisible={
            !dragging && Boolean(effectiveImageUrl) ? undefined : false
          }
