...
const NODE_RESIZER_HANDLE_CLASS =
  "!w-3 !h-3 !bg-[var(--accent-violet)] !border-2 !border-[var(--node-bg)] !rounded-full";
...
const DELETE_BUTTON_VISIBLE_CLASS =
...
const NODE_STATE_TONE_CLASS: Record<
  NodeStateTone,
  { frame: string; icon: string; title: string }
> = {
  neutral: {
...
export function NodeResizerPreset({
  ...
  const lineClassName = hideVisuals
    ? `${NODE_RESIZER_LINE_CLASS} !opacity-0`
    : NODE_RESIZER_LINE_CLASS;
...
export function NodeEmptyState({
  ...
  return (
    ...
    >
      <div
        className={`mb-3 flex h-14 w-14 items-center justify-center rounded-full border shadow-[0_0_0_1px_rgba(255,255,255,0.03)] ${toneClass.icon}`}
      >
        {icon}
      </div>
