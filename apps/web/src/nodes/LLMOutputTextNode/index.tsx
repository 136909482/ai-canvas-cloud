...
import {
  canEditLLMOutput,
  getLLMOutputModeLabel,
} from "@/features/llm/outputEditMode";
...
export const LLMOutputTextNode = memo(function LLMOutputTextNode({
  id,
  data,
  selected,
}: LLMOutputTextNodeProps) {
  ...
  return (
    <>
      {selected ? (
        <StableNodeToolbar
          isVisible={hasText && !isError ? undefined : false}
        ...
        <div className="flex min-h-0 flex-1 flex-col p-2.5">
            ...
            {isGenerating && !hasText ? (
              <>
                ...
                <div className="relative z-10 flex flex-1 items-center justify-center px-6 text-center">
                  <div
                    className={`${themeClasses.nodeBadge} ${themeClasses.nodeBadgeViolet} normal-case tracking-normal backdrop-blur`}
                  >
                    {UI_TEXT.generating}
                  </div>
                </div>
              ...
              <div className="flex flex-1 items-center justify-center px-6 py-8 text-center">
                <div className="max-w-[260px]">
                  ...
                  <p className="mt-2 text-xs leading-5 text-[var(--text-muted)]">
                    {data.errorMsg || UI_TEXT.placeholder}
                  </p>
                </div>
              </div>
          ...
          {!isError && hasText ? (
            <p className="mt-2 px-0.5 text-[10px] leading-relaxed text-[var(--text-muted)]">
              {effectiveIsEditing
                ? `${getLLMOutputModeLabel(data.outputFormat)} · 编辑中`
                : getLLMOutputModeLabel(data.outputFormat)}
            </p>
          ) : null}
        </div>
      </div>
