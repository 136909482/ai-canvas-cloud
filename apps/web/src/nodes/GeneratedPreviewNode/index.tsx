...
function getPreviewTimestamp(data: GeneratedPreviewNodeProps["data"]) {
  return typeof data.createdAt === "number" && Number.isFinite(data.createdAt)
    ? data.createdAt
    : Date.now();
}
...
export const GeneratedPreviewNode = memo(function GeneratedPreviewNode({
  id,
  data,
  selected,
  dragging,
  ...
  const modelName = useSettingsStore(
    (s) => s.getModelConfig(data.model)?.displayName,
  );
  ...
  const handleApplyToSourceImage = useCallback(() => {
    ...
    runTracked(() => {
      const sourceNode = useCanvasStore
        .getState()
        .nodes.find((node) => node.id === data.sourceImageNodeId);
  ...
  return (
    <>
      {selected ? (
        <StableNodeToolbar
          isVisible={!dragging && hasImage ? undefined : false}
