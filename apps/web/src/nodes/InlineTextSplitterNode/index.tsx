...
export const InlineTextSplitterNode = memo(function InlineTextSplitterNode({
  id,
  data,
  selected,
}: InlineTextSplitterNodeProps) {
  const updateNodeData = useCanvasStore((s) => s.updateNodeData);
  const deleteNode = useCanvasStore((s) => s.deleteNode);
  const syncInlineTextSplitterParts = useCanvasStore(
    (s) => s.syncInlineTextSplitterParts,
  );
  ...
  const parts = useMemo(
    () =>
      Array.isArray(data.parts)
        ? data.parts.filter((part): part is string => typeof part === "string")
        : [],
    [data.parts],
  );
                ...
                <div key={`part-${index}`} className="relative">
                  <div className="flex min-h-10 items-center gap-3 rounded-lg border border-[var(--border-subtle)] bg-[var(--control-bg)] px-3 py-2 text-xs text-[var(--text-secondary)] focus-within:border-violet-400/45 focus-within:bg-[var(--control-bg-hover)]">
                    ...
                    <input
                      ...
                      onClick={stopCanvasGesture}
                      onChange={(event) =>
                        updatePart(index, event.currentTarget.value)
                      }
