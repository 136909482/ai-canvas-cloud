...
const TITLE_TEXT_CLASS_NAME =
  "block h-5 min-w-0 truncate p-0 text-xs font-medium leading-5 normal-case tracking-normal text-[var(--text-secondary)]";
...
function encodeRichPromptReferenceKey(
  item: Pick<
    RichPromptReferenceItem,
    "sourceId" | "imageUrl" | "thumbnailRelativePath"
  >,
) {
  return [item.sourceId, item.imageUrl, item.thumbnailRelativePath ?? ""].join(
    RICH_PROMPT_REFERENCE_KEY_SEPARATOR,
  );
}

function decodeRichPromptReferenceKey(
  key: string,
  index: number,
): RichPromptReferenceItem | null {
  ...
  const imageUrl = key.slice(
    separatorIndex + RICH_PROMPT_REFERENCE_KEY_SEPARATOR.length,
  );
  const thumbnailSeparatorIndex = imageUrl.indexOf(
    RICH_PROMPT_REFERENCE_KEY_SEPARATOR,
  );
  const resolvedImageUrl =
    thumbnailSeparatorIndex >= 0
      ? imageUrl.slice(0, thumbnailSeparatorIndex)
      : imageUrl;
  const thumbnailRelativePath =
    thumbnailSeparatorIndex >= 0
      ? imageUrl.slice(
          thumbnailSeparatorIndex + RICH_PROMPT_REFERENCE_KEY_SEPARATOR.length,
        )
      : "";
...
function buildTextNodeRichPromptReferences({
  ...
  for (const generateNode of connectedGenerateNodes) {
    const referenceSourceOrder = getOrderedStringIds(
      generateNode?.data?.referenceSourceOrder,
    );
    const imageEdgeSourceIds = edges
      .filter(
        (edge) =>
          edge.target === generateNode?.id && edge.targetHandle !== "mask",
      )
  ...
  return referenceSourceIds
    ...
    .filter((reference): reference is RichPromptReferenceItem =>
      Boolean(reference),
    );
}
...
export const TextNode = memo(function TextNode({
  ...
  const selectRichPromptReferenceKeys = useMemo(
    () => (state: { nodes: Node[]; edges: Edge[] }) =>
      buildTextNodeRichPromptReferences({
        textNodeId: id,
        nodes: state.nodes,
        edges: state.edges,
  ...
  const richPromptReferenceKeys = useCanvasStore(
    useShallow(selectRichPromptReferenceKeys),
  );
        ...
        >
          <div className={NODE_TOOLBAR_CLASS_NAME}>
            <button
              ...
              aria-label={
                copyState === "copied"
                  ? QUICK_TOOLBAR_TEXT.bodyCopied
                  : QUICK_TOOLBAR_TEXT.copyBodyText
              }
              title={
                copyState === "copied"
                  ? QUICK_TOOLBAR_TEXT.bodyCopied
                  : copyState === "error"
                    ? QUICK_TOOLBAR_TEXT.copyBodyFailed
                    : QUICK_TOOLBAR_TEXT.copyBodyText
              }
            >
              {copyState === "copied" ? (
            ...
            >
              <div className="flex h-12 shrink-0 items-center gap-3 border-b border-[var(--border-subtle)] bg-[var(--control-bg)] px-4">
                ...
                <span className="hidden shrink-0 text-[11px] font-medium text-[var(--text-muted)] sm:inline">
                  {getTextStats(fullscreenText).characters} 字符 ·{" "}
                  {getTextStats(fullscreenText).lines} 行 · ≈{" "}
                  {getTextStats(fullscreenText).tokens} tokens
                </span>
                <button
                  type="button"
                  onClick={saveFullscreenEditor}
              ...
              <textarea
                ...
                onChange={(event) =>
                  setFullscreenText(event.currentTarget.value)
                }
                autoFocus
