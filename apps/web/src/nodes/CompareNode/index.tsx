...
function decodeCompareImageSource(
  key: string,
  fallbackSlot: CompareImageSlot,
): CompareImageSource {
  const [
    slot,
    imageUrl = "",
    label = slotLabel(fallbackSlot),
    thumbnailRelativePath = "",
  ] = key.split(COMPARE_IMAGE_KEY_SEPARATOR);
  return {
...
function buildCompareImageAsset(
...
export const CompareNode = memo(function CompareNode({
  ...
  const connectedImageKeys = useCanvasStore(
    useShallow((s) => {
      ...
      for (const edge of s.edges) {
        if (
          edge.target !== id ||
          (edge.targetHandle !== "image1" && edge.targetHandle !== "image2")
        ) {
          continue;
        }
      ...
      return [
        encodeCompareImageSource(
          bySlot.get("image1") ?? {
            slot: "image1",
            imageUrl: null,
            label: UI_TEXT.image1,
          },
        ),
        encodeCompareImageSource(
          bySlot.get("image2") ?? {
  ...
  const activeSlot: CompareImageSlot =
    data.activeSlot === "image2" ? "image2" : "image1";
  const sliderPosition = clampSliderPosition(
    typeof data.sliderPosition === "number" ? data.sliderPosition : 50,
  );
  ...
  const activeImage =
    activeSlot === "image2" ? connectedImages.image2 : connectedImages.image1;
    ...
    >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 rounded-md px-2 py-1 text-[10px] transition ${
                mode === "slider"
                  ...
                  : themeClasses.nodeSegmentButton
              }`}
      ...
      <div className="flex min-h-0 flex-1 flex-col p-2">
        <div
          ...
          style={
            {
              "--compare-slider-position": `${sliderPosition}%`,
            } as CSSProperties
          }
          onPointerMove={
          ...
          {!hasImage1 && !hasImage2 ? (
            ...
            <>
              ...
              <div
                ...
                style={{
                  clipPath:
                    "inset(0 calc(100% - var(--compare-slider-position)) 0 0)",
                }}
              >
                <CanvasImagePreview
            ...
            >
              {activeImage.imageUrl ? (
                ...
                <div className="flex h-full items-center justify-center px-6 text-center text-xs text-[var(--text-muted)]">
                  {activeSlot === "image1"
                    ? UI_TEXT.missingImage1
                    : UI_TEXT.missingImage2}
                </div>
              )}
