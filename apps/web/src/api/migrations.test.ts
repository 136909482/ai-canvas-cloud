...
test("migration package parser reads fixed package files and asset bytes from a stored ZIP", async () => {
  const zip = storedZip({
    ...
    "assets.json": JSON.stringify({
      schemaVersion: 1,
      assets: [{ filePath: "assets/image.png" }],
    }),
    "assets/image.png": "image-body",
  });

  const parsed = await parseMigrationPackage(
    new Blob([toOwnedArrayBuffer(zip)], { type: "application/zip" }),
  );

  assert.equal(parsed.manifest.packageId, "package-a");
  assert.equal(parsed.projectRecord.id, "project-a");
  assert.equal(
    new TextDecoder().decode(parsed.files.get("assets/image.png")),
    "image-body",
  );
  assert.deepEqual(
    parsed.archiveEntries.map((entry) => entry.path),
    [
      "manifest.json",
      "project.json",
      "graph.json",
...
test("migration package parser rejects archives without a central directory", async () => {
  await assert.rejects(
    () => parseMigrationPackage(new Blob(["not-a-zip"])),
    /ZIP 目录损坏/,
  );
});
