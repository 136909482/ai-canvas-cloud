...
export async function prepareMigrationImport(
  input: PrepareMigrationImportRequest,
) {
  return requestCloudJson<MigrationImportResponse>(
    "/migrations/imports/prepare",
    {
      method: "POST",
...
export async function getMigrationImport(importId: string) {
  return requestCloudJson<MigrationImportResponse>(
    `/migrations/imports/${encodeURIComponent(importId)}`,
  );
}

export async function cancelMigrationImport(importId: string) {
  return requestCloudJson<MigrationImportResponse>(
    `/migrations/imports/${encodeURIComponent(importId)}/cancel`,
    {
      method: "POST",
      body: JSON.stringify({}),
    },
...
export async function commitMigrationImport(
  importId: string,
  input: CommitMigrationImportRequest,
) {
  return requestCloudJson<{
    importId: string;
    status: "completed";
    strategy: "copy" | "replace";
    project: { id: string; name: string; version: number; sequence: number };
    assetCount: number;
    checkpoint: { id: string; projectVersion: number; sequence: number } | null;
...
export async function getMigrationAssetUpload(
  importId: string,
  logicalAssetId: string,
) {
  return requestCloudJson<MigrationImportAssetUploadResponse>(
...
export async function completeMigrationAssetUpload(
  importId: string,
  logicalAssetId: string,
  parts?: Record<string, { etag: string; byteSize: number }>,
) {
  return requestCloudJson<MigrationImportAssetUploadResponse>(
...
export async function completeMigrationAssetPart(
  importId: string,
  logicalAssetId: string,
  partNumber: number,
  input: { etag: string; byteSize: number },
) {
  return requestCloudJson<MigrationImportAssetUploadResponse>(
...
export async function prepareMigrationExport(
  projectId: string,
  input: PrepareMigrationExportRequest,
) {
  return requestCloudJson<MigrationExportResponse>(
    `/projects/${encodeURIComponent(projectId)}/exports/prepare`,
    {
      method: "POST",
      body: JSON.stringify(input),
    },
...
export async function downloadMigrationExport(
  ...
  return requestCloudJson<MigrationExportDownloadResponse>(
    `/projects/${encodeURIComponent(projectId)}/exports/${encodeURIComponent(exportId)}/download`,
  );
}

export async function cancelMigrationExport(
  projectId: string,
  exportId: string,
) {
  return requestCloudJson<MigrationExportResponse>(
    `/projects/${encodeURIComponent(projectId)}/exports/${encodeURIComponent(exportId)}/cancel`,
    { method: "POST", body: JSON.stringify({}) },
  );
}

export async function retryMigrationExport(
  projectId: string,
  exportId: string,
) {
  return requestCloudJson<MigrationExportResponse>(
    `/projects/${encodeURIComponent(projectId)}/exports/${encodeURIComponent(exportId)}/retry`,
    { method: "POST", body: JSON.stringify({}) },
  );
}
...
export async function parseMigrationPackage(
  blob: Blob,
): Promise<ParsedMigrationPackage> {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    if (readU32(view, cursor) !== 0x02014b50)
      throw new Error("无法读取目录包：条目目录损坏");
    ...
    const body =
      method === 0
        ...
        : method === 8
          ...
          : (() => {
              throw new Error(`不支持 ZIP 压缩方式：${method}`);
            })();
    if (body.byteLength !== uncompressedSize)
      throw new Error(`目录包条目大小不一致：${path}`);
    files.set(path, body);
    archiveEntries.push({
      path,
      kind: "file",
  ...
  return {
    ...
    assetManifest: readJson<{
      schemaVersion: 1;
      assets: MigrationPackageAsset[];
    }>("assets.json"),
    checkpoint: files.has("checkpoint.json")
      ? readJson<MigrationPackageCheckpoint>("checkpoint.json")
      : null,
    archiveEntries,
    files,
  };
