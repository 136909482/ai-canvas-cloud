...
function orderLayerGroups(
  ...
  for (const layer of sortedLayers) {
    layerGroups
      ...
      .sort(
        (left, right) =>
          getSecondaryPosition(left, direction) -
          getSecondaryPosition(right, direction),
      );
    layerGroups
      .get(layer)!
      .forEach((node, index) => originalOrder.set(node.id, index));
  }
  ...
  const getOrderByNodeId = () => {
    ...
    for (const layer of sortedLayers) {
      layerGroups
        .get(layer)!
        .forEach((node, index) => orderByNodeId.set(node.id, index));
    }
  ...
  const sortLayer = (layer: number, scoreByNodeId: Map<string, number>) => {
    ...
    layerGroups.get(layer)!.sort((left, right) => {
      ...
      return (
        (previousOrder.get(left.id) ?? originalOrder.get(left.id) ?? 0) -
        (previousOrder.get(right.id) ?? originalOrder.get(right.id) ?? 0)
      );
    });
  };
  ...
  for (let iteration = 0; iteration < 4; iteration += 1) {
    for (const layer of sortedLayers.slice(1)) {
      ...
      for (const node of layerGroups.get(layer) ?? []) {
        const score = averageNeighborOrder(
...
function computeLayeredAutoLayout(
  ...
  const { sortedLayers, layerGroups } = orderLayerGroups(
    nodes,
    edges,
    direction,
  );
...
export function computeFocusedAutoLayout(
  ...
  const negativeLayers = sortedLayers
  ...
  for (const layer of positiveLayers) {
    ...
    const previousWidth = Math.max(
      ...(layerGroups.get(previousPositiveLayer) ?? [centerNode]).map(
        (node) => getSize(node).width,
      ),
    );
  ...
  for (const layer of negativeLayers) {
    ...
    const currentWidth = Math.max(
      ...(layerGroups.get(layer) ?? [centerNode]).map(
        (node) => getSize(node).width,
      ),
    );
  ...
  for (const layer of sortedLayers) {
    const layerNodes = [...(layerGroups.get(layer) ?? [])].sort(
      (left, right) => left.position.y - right.position.y,
    );
    const x = xByLayer.get(layer) ?? centerX;

    if (layer === 0) {
      const anchorIndex = layerNodes.findIndex(
    ...
    const totalHeight =
      layerNodes.reduce((sum, node) => sum + getSize(node).height, 0) +
