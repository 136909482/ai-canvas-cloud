...
test(
  "two API limiter instances share Redis limits and serialize concurrent bursts",
  {
  ...
  async (context) => {
    ...
    try {
      try {
        ...
        context.skip(
          "The configured Redis server is unavailable or rejected REDIS_URL credentials",
        );
        return;
      }
      ...
      for (
        ...
        assert.equal(
          (await limiter.consume("auth_attempt", [sharedScope])).allowed,
          true,
        );
      }
      ...
      const burst = await Promise.all(
        Array.from({ length: 20 }, (_, index) =>
          (index % 2 === 0 ? first : second).consume("auth_attempt", [
            burstScope,
          ]),
        ),
      );
      assert.equal(
        burst.filter((decision) => decision.allowed).length,
        RATE_LIMIT_POLICIES.auth_attempt.maxRequests,
      );
    } finally {
