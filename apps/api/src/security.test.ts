...
test("security boundary exposes restrictive headers and allows an explicit origin preflight", () => {
  ...
  const handled = handleSecurityBoundary(
    request("OPTIONS", "https://cloud.example.com"),
    response,
    {
      env: "staging",
...
test("protected auth cookie writes require an origin even before a session exists", () => {
  const response = responseDouble();
  const handled = handleSecurityBoundary(
    request("POST", undefined, {}, "/api/v1/auth/login"),
    response,
    {
      env: "production",
      webAllowedOrigins: ["https://cloud.example.com"],
...
test("protected cross-site fetch metadata is rejected", () => {
  const response = responseDouble();
  const handled = handleSecurityBoundary(
    request("POST", "https://cloud.example.com", {
