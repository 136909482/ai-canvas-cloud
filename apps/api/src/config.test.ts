...
test("API config reads development admin seed options outside production", () => {
  ...
  assert.equal(
    loadApiConfig({
      ...
      DATABASE_URL:
        "postgres://prod-user:random-password@prod-postgres:5432/ai_canvas_cloud_production",
      ...
      ...Object.fromEntries(
        [
          ...
          key,
          `ai-canvas-cloud-production-${key.toLowerCase()}`,
        ]),
      ),
    }).devSeedAdmin,
    false,
...
test("API config normalizes and validates the explicit web origin allowlist", () => {
  const config = loadApiConfig({
    ...
    WEB_PUBLIC_URL: "https://cloud.example.com",
    WEB_ALLOWED_ORIGINS:
      "https://cloud.example.com, https://studio.example.com/,https://cloud.example.com",
  });
  assert.deepEqual(config.webAllowedOrigins, [
  ...
  assert.throws(
    () =>
      loadApiConfig({
        ...baseEnv,
        WEB_ALLOWED_ORIGINS: "https://cloud.example.com/app",
      }),
    /without paths/,
  );
  assert.throws(
    () =>
      loadApiConfig({
        ...baseEnv,
        WEB_ALLOWED_ORIGINS: "http://user:pass@example.com",
      }),
    /Invalid WEB_ALLOWED_ORIGINS/,
  );
});
