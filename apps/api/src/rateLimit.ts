...
function normalizeRetryAfter(value: unknown, fallback: number) {
  ...
  return Number.isFinite(parsed) && parsed > 0
    ? Math.min(Math.ceil(parsed), 86_400)
    : fallback;
}
...
export function createMemoryRateLimiter(now = () => Date.now()): RateLimiter {
  ...
  return {
    async consume(bucket, scopes) {
      ...
      for (const scope of [
        ...new Set(scopes.filter((value) => value.trim())),
      ]) {
        ...
        if (!current || current.expiresAt <= timestamp) {
          entries.set(key, {
            count: 1,
            expiresAt: timestamp + policy.windowSeconds * 1000,
          });
          continue;
        }
        current.count += 1;
        if (current.count > policy.maxRequests) {
          retryAfterSeconds = Math.max(
            retryAfterSeconds,
            Math.ceil((current.expiresAt - timestamp) / 1000),
          );
        }
      }
      return {
