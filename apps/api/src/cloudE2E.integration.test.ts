...
class BrowserContext {
  ...
  async request(port: number, method: string, path: string, body?: unknown): Promise<JsonResponse> {
    ...
    return new Promise((resolve, reject) => {
      const request = http.request({
        ...
        headers: {
          ...
          ...(this.cookies.size > 0 ? { cookie: [...this.cookies].map(([key, value]) => `${key}=${value}`).join('; ') } : {}),
          ...(bodyText === undefined ? {} : {
        ...
        response.on('end', () => {
          ...
          for (const header of setCookies) {
            ...
            if (/max-age=0|expires=Thu, 01 Jan 1970/i.test(header)) this.cookies.delete(key)
  ...
  loginBody(email: string, password: string, force = false) {
    return { email, password, deviceId: this.deviceId, ...(force ? { force: true } : {}) }
  }
}
...
test('cloud API two-account E2E keeps projects, graph, assets, tasks, providers, sessions and devices isolated', {
  ...
  try {
    ...
    const assetService = createPostgresAssetService(pool, { authorizationService: authorization, objectStorage })
    const projectService = createPostgresProjectService(pool, { authorizationService: authorization })
    ...
    const createServer = () => createApiServer({
      config,
      authService,
      assetService,
      projectService,
    ...
    const accountA = new BrowserContext(`cloud-e2e-a/${runId}`, `device-a-${runId}`)
    const accountB = new BrowserContext(`cloud-e2e-b/${runId}`, `device-b-${runId}`)
    const emailA = `p7-6-a-${runId}@example.invalid`
    const emailB = `p7-6-b-${runId}@example.invalid`
    const password = `p7-6-password-${runId}`

    const registeredA = await accountA.request(port, 'POST', '/api/v1/auth/register', accountA.registerBody(emailA, password))
    const registeredB = await accountB.request(port, 'POST', '/api/v1/auth/register', accountB.registerBody(emailB, password))
    ...
    const projectA = (createdProject.body as { project: { id: string } }).project.id
    const projectBRead = await accountB.request(port, 'GET', `/api/v1/projects/${projectA}`)
    const missingRead = await accountB.request(port, 'GET', `/api/v1/projects/${randomUUID()}`)
    ...
    assert.equal((await accountB.request(port, 'PATCH', `/api/v1/projects/${projectA}`, { name: 'forged' })).statusCode, 404)
    assert.equal((await accountB.request(port, 'POST', `/api/v1/projects/${projectA}/archive`, {})).statusCode, 404)
    assert.equal((await accountA.request(port, 'GET', `/api/v1/projects/${projectA}`)).statusCode, 200)

    const sourceNodeId = `source-${runId}`
    const previewNodeId = `preview-${runId}`
    const graphPath = `/api/v1/projects/${projectA}/graph`
    const graphBody = (baseVersion: number, batchId: string, operations: unknown[]) => ({
      baseVersion,
    ...
    const nodes = [
      { id: sourceNodeId, nodeType: 'generate', position: { x: 0, y: 0 }, dataSchemaVersion: 1, data: {} },
      { id: previewNodeId, nodeType: 'preview', position: { x: 240, y: 0 }, dataSchemaVersion: 1, data: {} },
    ]
    const graphCreated = await accountA.request(port, 'PATCH', graphPath, graphBody(0, 'initial', nodes.map((node) => ({ type: 'upsertNode', node }))))
    ...
    assert.equal((await accountB.request(port, 'GET', graphPath)).statusCode, 404)

    const concurrent = await Promise.all([
      accountA.request(port, 'PATCH', graphPath, graphBody(graphVersion, 'tab-a', [{
        type: 'upsertNode',
        node: { id: `tab-a-${runId}`, nodeType: 'text', position: { x: 0, y: 240 }, dataSchemaVersion: 1, data: {} },
      }])),
      accountA.request(port, 'PATCH', graphPath, graphBody(graphVersion, 'tab-b', [{
        type: 'upsertNode',
        node: { id: `tab-b-${runId}`, nodeType: 'text', position: { x: 0, y: 360 }, dataSchemaVersion: 1, data: {} },
      }])),
    ])
    assert.equal(concurrent.filter((response) => response.statusCode === 200).length, 1)
    assert.equal(concurrent.filter((response) => response.statusCode === 409).length, 1)
    const currentGraph = await accountA.request(port, 'GET', graphPath)
    assert.equal(currentGraph.statusCode, 200)
    const currentVersion = (currentGraph.body as { version: number; sequence: number }).version
    const currentSequence = (currentGraph.body as { version: number; sequence: number }).sequence

    const checkpoint = await accountA.request(port, 'POST', `/api/v1/projects/${projectA}/checkpoints`, {
    ...
    const assetBody = asset.body as { asset: { id: string }; directUpload: { url: string; method: string; headers: Record<string, string> } }
    assert.equal(assetBody.directUpload.method, 'PUT')
    const uploadResponse = await fetch(assetBody.directUpload.url, { method: 'PUT', headers: assetBody.directUpload.headers, body: png, redirect: 'error' })
    assert(uploadResponse.ok)
    const completedAsset = await accountA.request(port, 'POST', `/api/v1/assets/uploads/${(asset.body as { upload: { id: string } }).upload.id}/complete`)
    assert.equal(completedAsset.statusCode, 200)
    const assetId = assetBody.asset.id
    assert.equal((await accountA.request(port, 'GET', `/api/v1/assets/${assetId}/url`)).statusCode, 200)
    const assetForbidden = await accountB.request(port, 'GET', `/api/v1/assets/${assetId}/url`)
    assert.equal(assetForbidden.statusCode, 404)
    assert.equal('url' in (assetForbidden.body as object), false)

    const sessionsA = await accountA.request(port, 'GET', '/api/v1/auth/sessions')
    const devicesA = await accountA.request(port, 'GET', '/api/v1/auth/devices')
    assert.equal(sessionsA.statusCode, 200)
    assert.equal(devicesA.statusCode, 200)
    const sessionA = (sessionsA.body as { sessions: Array<{ id: string }> }).sessions.find((session) => session.current)
    const deviceA = (devicesA.body as { devices: Array<{ id: string }> }).devices.find((device) => device.current)
    assert(sessionA && deviceA)
    assert.equal((await accountB.request(port, 'DELETE', `/api/v1/auth/sessions/${sessionA.id}`)).statusCode, 404)
    assert.equal((await accountB.request(port, 'DELETE', `/api/v1/auth/devices/${deviceA.id}`)).statusCode, 404)
    ...
    assert.equal((await takeover.request(port, 'POST', '/api/v1/auth/login', takeover.loginBody(emailA, password))).statusCode, 409)
    assert.equal((await takeover.request(port, 'POST', '/api/v1/auth/login', takeover.loginBody(emailA, password, true))).statusCode, 200)
    assert.notEqual((await accountA.request(port, 'GET', '/api/v1/auth/session')).statusCode, 200)
  } finally {
    if (server) await closeApiServer(server, 2_000).catch(() => undefined)
    if (pool) {
      if (objectStorage) {
        const managedObjects = await pool.query<{ object_key: string }>(`
