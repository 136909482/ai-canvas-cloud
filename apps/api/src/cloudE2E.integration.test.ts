  ...
  async () => {
    ...
    try {
      ...
      const migrations = (
        ...
        .filter(
          (fileName) =>
            fileName.endsWith(".sql") && !/^002[5-9]_/.test(fileName),
        )
        .sort();
      ...
      const projectSnapshotService = createPostgresProjectSnapshotService(
        pool,
        { authorizationService: authorization },
      );
      ...
      const migrationImportService = createPostgresMigrationImportService(
        pool,
        { authorizationService: authorization },
      );
