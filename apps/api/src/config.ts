...
function readAllowedOrigins(env: NodeJS.ProcessEnv, fallback: string) {
  ...
  for (const origin of origins) {
    ...
    if (parsed.pathname !== '/') {
      throw new Error(`WEB_ALLOWED_ORIGINS must contain origins without paths: ${origin}`)
    }
  }
...
export function loadApiConfig(env: NodeJS.ProcessEnv = process.env): ApiConfig {
  ...
  const authEmailTransport = readOptionalEnv(env, 'AUTH_EMAIL_TRANSPORT', 'development').toLowerCase()
  ...
  return {
    ...
    webAllowedOrigins: readAllowedOrigins(env, readOptionalEnv(env, 'WEB_PUBLIC_URL', 'http://localhost:5173')),
