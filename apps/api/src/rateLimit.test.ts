...
test("memory rate limiter enforces a window and recovers after expiry", async () => {
  ...
  for (
    let attempt = 0;
    attempt < RATE_LIMIT_POLICIES.auth_attempt.maxRequests;
    attempt += 1
  ) {
    assert.equal(
      (await limiter.consume("auth_attempt", ["ip:198.51.100.10"])).allowed,
      true,
    );
  }
...
test("session limits follow one account across IP changes", async () => {
  ...
  for (
    let attempt = 0;
    attempt < RATE_LIMIT_POLICIES.asset_prepare.maxRequests;
    attempt += 1
  ) {
    assert.equal(
      (
        await limiter.consume("asset_prepare", [
          "session:session-a",
...
test("Redis outages fail closed for high-risk writes and recover without restarting the API", async () => {
  ...
  const closed = await limiter.consume("asset_prepare", [
  ...
  client.status = "ready";
  const recovered = await limiter.consume("asset_prepare", [
    "session:trusted-session",
  ]);
...
test("Redis readiness preserves the latest low-level connection failure for stable classification", async () => {
  const connectionError = Object.assign(new Error("connect failed"), {
    code: "ECONNREFUSED",
  });
  const client: RedisRateLimitClient = {
    status: "ready",
  ...
  const limiter = createRateLimiterWithRedisClient(
    client,
    "development",
    () => connectionError,
  );
  ...
  await assert.rejects(
    limiter.ping(),
    (error: Error & { code?: string }) => error.code === "ECONNREFUSED",
  );
});
