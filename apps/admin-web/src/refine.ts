import type { AdminAuditEvent, AdminRole, AdminSessionResponse } from '@ai-canvas-cloud/contracts'
import type { AccessControlProvider, AuthProvider, BaseRecord, DataProvider, GetListParams } from '@refinedev/core'
...
export const adminDataProvider: DataProvider = {
  async getList<TData extends BaseRecord>({ resource, pagination, filters }: GetListParams) {
    ...
    const actionFilter = filters?.find((filter) => 'field' in filter && filter.field === 'action')
    const resultFilter = filters?.find((filter) => 'field' in filter && filter.field === 'result')
    const payload = await adminApi.auditEvents({
      limit: pagination?.pageSize ?? 50,
      action: actionFilter && 'value' in actionFilter && typeof actionFilter.value === 'string' ? actionFilter.value : undefined,
      result: resultFilter && 'value' in resultFilter && typeof resultFilter.value === 'string' ? resultFilter.value : undefined,
    })
    return { data: payload.items as unknown as TData[], total: payload.items.length }
  },
  async getOne<TData extends BaseRecord>() { return { data: unsupported() as TData } },
  async create<TData extends BaseRecord>() { return { data: unsupported() as TData } },
  async update<TData extends BaseRecord>() { return { data: unsupported() as TData } },
  async deleteOne<TData extends BaseRecord>() { return { data: unsupported() as TData } },
  getApiUrl: () => '/admin/v1',
}
...
export const adminAccessControlProvider: AccessControlProvider = {
  async can({ action }) {
    ...
    return { can: Boolean(role && PERMISSIONS[role].has(action)), reason: '当前管理员角色无此权限' }
  },
