        ...
        <form className="auth-form" onSubmit={submit}>
            ...
            <Field label="验证码">
              <div className="captcha-row">
                ...
                <button
                  ...
                  aria-label="刷新验证码"
                >
                  <RefreshCw className={captchaLoading ? "spin" : ""} />
                </button>
              </div>
            </Field>
          ) : null}
          <button
            ...
            disabled={
              ...
              captchaLoading ||
              !captchaReady ||
              (captcha?.enabled === true && captchaCode.length !== 5)
            }
          >
            {busy ? (
...
function AuditView() {
  ...
  if (access && !access.can)
    return <div className="empty-state">当前角色无权读取审计事件。</div>;
  return (
          ...
          <tbody>
            {result.data.map((event) => (
              <tr key={event.id}>
                ...
                <td>
                  <code className="request-id">{event.requestId}</code>
                </td>
              </tr>
            ))}
          </tbody>
...
function SecurityView({
  ...
  async function saveUsername(event: FormEvent) {
    ...
    try {
      ...
      setUsername(updated.admin.username);
  ...
  return (
    <section className="workspace-view">
      ...
      <div className="posture-list">
        {controls.map(([label, value, state]) => (
          <div className="posture-row" key={label}>
            <span>{label}</span>
            <strong>{value}</strong>
            <i className={state} />
          </div>
      ...
      <div className="credential-grid">
        {session.admin.role === "super_admin" ? (
          <section className="credential-panel credential-panel--security">
            <div className="credential-panel__heading">
              ...
              <div>
                <h2>登录验证码</h2>
                <p>开启后，管理员登录时需要输入页面展示的 5 位图片验证码。</p>
              </div>
            </div>
            <label className="security-toggle">
              <span>
                <strong>要求图片验证码</strong>
                <small>默认关闭，修改后对下一次登录生效</small>
              </span>
              <input
                type="checkbox"
                checked={captchaEnabled ?? false}
                disabled={captchaEnabled === null || busy !== null}
                onChange={(event) => void toggleCaptcha(event.target.checked)}
              />
              <i />
            </label>
          </section>
        ...
        <form className="credential-panel" onSubmit={saveUsername}>
          ...
          <button
            ...
            type="submit"
            disabled={busy !== null || username === session.admin.username}
          >
            {busy === "username" ? (
