...
export const DEFAULT_SITE_CONFIG: SiteConfigDocument = {
  ...
  home: {
    headline: 'AI Canvas',
    lead: '让创意，在画布上自然生长',
    description: '把灵感、素材与 AI 生成工作流放进同一张画布，随时回来，继续创作。',
...
const HTML_PATTERN = /<\/?[a-z][^>]*>|javascript:|data:text\/html|\{\s*[^}]*:[^}]*\}/i
...
function text(value: unknown, field: string, minimum: number, maximum: number) {
  ...
  if (normalized.length < minimum || normalized.length > maximum || HTML_PATTERN.test(normalized)) {
...
function nullableUrl(value: unknown, field: string) {
  ...
  const hasControlOrSpace = [...normalized].some((character) => character.charCodeAt(0) <= 0x20)
...
export function validateSiteConfigDocument(value: unknown): SiteConfigDocument {
  ...
  exactKeys(root, ['schemaVersion', 'siteName', 'shortName', 'home', 'footer', 'records', 'links', 'themePreset', 'navigation', 'features', 'logoAssetId', 'faviconAssetId'], 'config')
  if (root.schemaVersion !== SITE_CONFIG_SCHEMA_VERSION) throw new Error('config schema version is unsupported')

  const home = object(root.home, 'home')
  exactKeys(home, ['headline', 'lead', 'description', 'primaryActionLabel'], 'home')
  ...
  exactKeys(records, ['companyName', 'icpNumber', 'publicSecurityNumber'], 'records')
  const links = object(root.links, 'links')
  exactKeys(links, ['helpUrl', 'feedbackUrl', 'termsUrl', 'privacyUrl', 'accountDeletionUrl'], 'links')
  const features = object(root.features, 'features')
  exactKeys(features, ['registrationEnabled', 'feedbackEnabled'], 'features')

  if (typeof root.themePreset !== 'string' || !THEMES.has(root.themePreset as SiteThemePreset)) {
    throw new Error('themePreset is invalid')
  }
  if (!Array.isArray(root.navigation) || root.navigation.length === 0 || root.navigation.length > NAVIGATION.size) {
    throw new Error('navigation is invalid')
  }
  const navigation = root.navigation.map((item) => {
    if (typeof item !== 'string' || !NAVIGATION.has(item as SiteNavigationItem)) throw new Error('navigation is invalid')
    return item as SiteNavigationItem
  })
  if (new Set(navigation).size !== navigation.length) throw new Error('navigation contains duplicates')
  ...
  return {
    ...
    home: {
      ...
      primaryActionLabel: text(home.primaryActionLabel, 'home.primaryActionLabel', 1, 40),
    },
    footer: {
    ...
    records: {
      companyName: nullableText(records.companyName, 'records.companyName', 120),
      icpNumber: nullableText(records.icpNumber, 'records.icpNumber', 120),
      publicSecurityNumber: nullableText(records.publicSecurityNumber, 'records.publicSecurityNumber', 120),
    },
    links: {
      ...
      accountDeletionUrl: nullableUrl(links.accountDeletionUrl, 'links.accountDeletionUrl'),
    },
    themePreset: root.themePreset as SiteThemePreset,
    navigation,
    features: {
      registrationEnabled: boolean(features.registrationEnabled, 'features.registrationEnabled'),
      feedbackEnabled: boolean(features.feedbackEnabled, 'features.feedbackEnabled'),
    },
