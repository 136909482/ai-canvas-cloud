...
export function validateAdminUserListQuery(value: unknown): AdminUserListQuery {
  ...
  rejectUnknownKeys(input, new Set(['cursor', 'limit', 'status', 'verification', 'search']))

  const cursor = optionalTrimmedString(input.cursor, 'cursor', 1_024)
  if (cursor && !/^[A-Za-z0-9_-]+$/.test(cursor)) throw new Error('cursor is invalid')

  const rawLimit = input.limit ?? ADMIN_USER_LIST_DEFAULT_LIMIT
  const limit = typeof rawLimit === 'string' && /^\d+$/.test(rawLimit)
    ? Number(rawLimit)
    : rawLimit
  if (!Number.isInteger(limit) || Number(limit) < 1 || Number(limit) > ADMIN_USER_LIST_MAX_LIMIT) {
  ...
  if (status && !(['active', 'disabled', 'deleted'] as const).includes(status as AdminManagedUserStatus)) {
    throw new Error('status is invalid')
  }

  const verification = optionalTrimmedString(input.verification, 'verification', 16)
  if (verification && !(['verified', 'unverified'] as const).includes(verification as AdminUserVerificationFilter)) {
  ...
  return {
    ...
    ...(verification ? { verification: verification as AdminUserVerificationFilter } : {}),
    ...(search ? { search } : {}),
  }
}

export function validateAdminUserActionRequest(value: unknown): AdminUserActionRequest {
  const input = requireRecord(value)
  rejectUnknownKeys(input, new Set(['reason']))
  if (typeof input.reason !== 'string') throw new Error('reason is required')
  const reason = input.reason.trim()
  if (reason.length < 3 || reason.length > ADMIN_USER_ACTION_REASON_MAX_LENGTH) {
    throw new Error(`reason must contain between 3 and ${ADMIN_USER_ACTION_REASON_MAX_LENGTH} characters`)
  }
  return { reason }
}
