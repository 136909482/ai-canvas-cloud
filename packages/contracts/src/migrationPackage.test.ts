...
function packageInput(
  overrides: Partial<MigrationPackageContractInput> = {},
): MigrationPackageContractInput {
  const files = [
...
test("migration asset completion accepts empty bodies but rejects unsafe part fields", () => {
  assert.deepEqual(
    validateCompleteMigrationImportAssetUploadRequest(undefined),
    {},
  );
...
test("rejects duplicate logical assets, dangling references, and mismatched metadata", () => {
  ...
  assert.throws(
    () => validateMigrationPackageContract(duplicateAssets),
    /duplicate value/i,
  );
  ...
  assert.throws(
    () => validateMigrationPackageContract(dangling),
    /referenced but not declared/i,
  );

  const mismatched = packageInput();
  mismatched.manifest = {
    ...packageInput().manifest,
    files: packageInput().manifest.files.map((file) =>
      file.path === "assets/asset-1.png" ? { ...file, byteSize: 101 } : file,
    ),
    totalByteSize: 191,
  };
  assert.throws(
    () => validateMigrationPackageContract(mismatched),
    /Asset metadata does not match/i,
  );
  ...
  assert.throws(
    () => validateMigrationPackageContract(unsafeFileName),
    /must be a file name/i,
  );
});
...
test("rejects credentials, tenant internals, and persistent URLs in package JSON", () => {
  ...
  assert.throws(
    () => validateMigrationPackageContract(sensitive),
    /forbidden internal or credential field/i,
  );
  ...
  assert.throws(
    () => validateMigrationPackageContract(url),
    /persistent external/i,
  );
});
...
test("rejects non-canonical manifest ordering and dangling checkpoint assets", () => {
  ...
  assert.throws(
    () => validateMigrationPackageContract(unordered),
  ...
  assert.throws(
    () => validateMigrationPackageContract(checkpoint),
    /Checkpoint asset IDs must match/i,
  );
});
