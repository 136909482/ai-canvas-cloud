...
test("workspace usage contract separates stored and reserved bytes", () => {
  ...
  assert.equal(
    ...
    response.storage.usedBytes + response.storage.reservedBytes,
  );
...
test("project graph contracts keep versioned operations explicit", () => {
  ...
  const graph: ProjectGraphResponse = {
    ...
    sequence: 1,
    nodes: [
      request.operations[0]!.type === "upsertNode"
        ? request.operations[0]!.node
        : neverValue(),
    ],
    edges: [],
  };
...
test("project revision detail contract exposes the saved record for a version", () => {
  ...
  assert.equal(
    response.record.project.version,
    response.checkpoint.projectVersion,
  );
