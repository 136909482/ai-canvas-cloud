...
const SENSITIVE_LOG_KEY_PATTERN =
  /(?:authorization|cookie|password|secret|token|api[_-]?key|access[_-]?key|signed[_-]?url|upload[_-]?url|download[_-]?url|object[_-]?key|provider[_-]?(?:response|body)|request[_-]?body|response[_-]?body)/i;
const SENSITIVE_LOG_TEXT_PATTERN =
  /\b(?:bearer|basic)\s+[a-z0-9._~+/=-]+|\b(?:password|pass|token|secret|api[_-]?key|authorization)\s*[:=]\s*[^\s,;]+/gi;
const URL_PATTERN = /\b(?:https?|s?3|rediss?):\/\/[^\s"'<>]+/gi;

function redactLogString(value: string) {
  const withoutCredentials = value.replace(
    SENSITIVE_LOG_TEXT_PATTERN,
    (match) => {
      const separator = match.match(/\s*[:=]\s*/)?.[0];
      return separator
        ? `${match.slice(0, match.indexOf(separator))}${separator}[redacted]`
        : "[redacted]";
    },
  );
...
export function redactSensitiveLogContext(
  value: unknown,
  key?: string,
  ...
  if (Array.isArray(value)) {
    const output = value.map((item) =>
      redactSensitiveLogContext(item, undefined, seen),
    );
    seen.delete(value);
    return output;
  }
