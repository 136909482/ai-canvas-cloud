...
test("dependency failures map to stable redacted categories", () => {
  ...
  assert.equal(
    classifyDependencyFailure(new Error("redis://user:secret@private.example")),
    "unknown",
  );
});
...
test("dependency checks enforce a bounded timeout without exposing error text", async () => {
  const status = await measureDependencyCheck(
  ...
  assert.equal(
    JSON.stringify(status).includes("Dependency health check timed out"),
    false,
  );
});
