...
export type DependencyFailureCategory =
  (typeof dependencyFailureCategories)[number];
...
export function classifyDependencyFailure(
  error: unknown,
  ...
  if (fields.some((value) => value === "ECONNREFUSED"))
    return "connection_refused";
  if (
    fields.some(
      (value) =>
        TIMEOUT_NAMES.has(value) ||
        value === "ETIMEDOUT" ||
  ...
  if (
    ...
    fields.some(
      (value) =>
        AUTHENTICATION_NAMES.has(value) ||
        value === "28P01" ||
        value === "28000",
    ) ||
    normalizedFields.some((value) =>
      /\b(?:wrongpass|noauth|authentication failed|password authentication failed)\b/.test(
        value,
      ),
    )
  ) {
  ...
  if (
    ...
    normalizedFields.some((value) =>
      /\b(?:access denied|permission denied|not authorized|noperm)\b/.test(
        value,
      ),
    )
  ) {
  ...
  if (
    ...
    normalizedFields.some((value) =>
      /\b(?:no such bucket|bucket unavailable)\b/.test(value),
    )
  ) {
